...
public class GT_NEIInputHandler implements IContainerInputHandler {
    ...
    public boolean lastKeyTyped(GuiContainer gui, char keyChar, int keyID) {
        if (keyID == NEIClientConfig.getKeyBinding("gui.usage")
            || keyID == NEIClientConfig.getKeyBinding("gui.recipe")) {
            ...
            if (slot != null && slot instanceof GT_FluidSlot) {
                ...
                if (GT_Utility.isFluidStackValid(fluid)) {
                    return keyID == NEIClientConfig.getKeyBinding("gui.usage")
                        ? GuiUsageRecipe.openRecipeGui("liquid", fluid.copy())
                        : GuiCraftingRecipe.openRecipeGui("liquid", fluid.copy());
                }
            }
        }
