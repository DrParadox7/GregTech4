...
public abstract class GT_RecipeHandler extends TemplateRecipeHandler {
    ...
    public GT_RecipeHandler() {
        if (!NEI_GregTech_Config.sIsAdded) {
            FMLInterModComms.sendRuntimeMessage(
                GregTech_API.gregtechmod,
                "NEIPlugins",
                "register-crafting-handler",
                GregTech_API.MOD_ID + "@" + getRecipeName() + "@" + getRecipeId());
            // API.registerRecipeHandler(this); // Oh nice, NEI checks if class not same... Scroll this, I'll just add
            // it to fucking list
        ...
        public CachedGT_Recipe(Recipe recipe, ItemStack activatedStack, boolean crafting) {
            ...
            for (int i = 0; i < inputs.size(); i++) {
                ...
                List<ItemStack> items = !crafting && activatedStack != null && input.match(activatedStack)
                    ? Lists.newArrayList(
                        GT_Utility.copyAmount(input.getCount() > 0 ? input.getCount() : 1, activatedStack))
                    : input.getVariants();
    ...
    protected void provideTooltip(GuiRecipe<?> gui, ItemStack stack, List<String> currenttip, CachedGT_Recipe recipe,
        Point relMouse) {
        for (PositionedFluidStack fluid : recipe.fluidResources) {
            if (fluid.fits(relMouse)) {
                currenttip.add(fluid.fluid.getLocalizedName());
                currenttip.add(
                    EnumChatFormatting.GRAY.toString() + I18n
                        .format("metatileentity.fluid.amount", GT_Utility.parseNumberToString(fluid.fluid.amount)));
            }
        }
    ...
    public void drawFluidStacks(int recipe) {
        ...
        for (PositionedFluidStack fluid : cached.fluidResources) fluid.draw();
        for (PositionedFluidStack fluid : cached.fluidProducts) fluid.draw();
    }
    ...
    public void loadCraftingRecipes(ItemStack result) {
        ...
        for (Recipe irecipe : getRecipeList().getRecipes()) {
            ...
            if (recipe.contains(recipe.products, result) || irecipe.getFluidOutputs()
                .contains(fluid)) {
                arecipes.add(recipe);
            }
        }
    ...
    public void loadUsageRecipes(ItemStack ingredient) {
        ...
        for (Recipe irecipe : getRecipeList().getRecipes()) {
            ...
            if (recipe.contains(recipe.resources, ingredient) || irecipe.getFluidInputs()
                .contains(fluid)) {
                arecipes.add(recipe);
            }
        }
