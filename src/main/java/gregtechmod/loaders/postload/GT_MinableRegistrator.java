...
public class GT_MinableRegistrator implements Runnable {
    ...
    public void run() {
        ...
        if (tStack != null)
            GregTech_API.sRecipeAdder.addJackHammerMinableBlock(Block.getBlockFromItem(tStack.getItem()), false);
        tStack = GT_ModHandler.getIC2Item("constructionFoamWall", 1);
        if (tStack != null)
            GregTech_API.sRecipeAdder.addJackHammerMinableBlock(Block.getBlockFromItem(tStack.getItem()), false);
        tStack = GT_ModHandler.getIC2Item("reinforcedStone", 1);
        if (tStack != null)
            GregTech_API.sRecipeAdder.addJackHammerMinableBlock(Block.getBlockFromItem(tStack.getItem()), true);
