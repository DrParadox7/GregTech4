...
public class ProcessingRecycling implements IOreRecipeRegistrator {
    ...
    public ProcessingRecycling() {
        for (OrePrefixes tPrefix : OrePrefixes.values()) {
            if (tPrefix.mIsMaterialBased && tPrefix.mMaterialAmount > 0L
                && tPrefix != OrePrefixes.ingotHot
                                    ...
                                    RecipeMaps.CANNING.factory()
                                        ...
                                        .output(
                                            GT_OreDictUnificator
                                                .get(OrePrefixes.gem, aMaterial, aPrefix.mMaterialAmount / 3628800L))
                                        .buildAndRegister();
                                }

                                if ((aMaterial.mTypes & 15) != 0 && aMaterial != Materials.Creosote
                                    && aMaterial != Materials.Mercury
                                    && aMaterial != Materials.SulfuricAcid
                                    && aMaterial != Materials.BioFuel
                                    && aMaterial != Materials.Water
                                    ...
                                    RecipeMaps.CANNING.factory()
                                        ...
                                        .output(
                                            GT_OreDictUnificator
                                                .get(OrePrefixes.dust, aMaterial, aPrefix.mMaterialAmount / 3628800L))
                                        .buildAndRegister();
                                }
                            }
                        } else {
                            GT_RecipeRegistrator
                                .registerBasicReverseMacerating(aStack, aMaterial, aPrefix.mMaterialAmount);
                            if (GT_OreDictUnificator.get(OrePrefixes.ingot, aMaterial) != null
                                && !aMaterial.contains(SubTag.NO_SMELTING)
                                && aPrefix != OrePrefixes.ingot
                                && aPrefix != OrePrefixes.nugget) {
                                GT_RecipeRegistrator
                                    .registerBasicReverseSmelting(aStack, aMaterial, aPrefix.mMaterialAmount, true);
                            }
                        }
                    }
                }
