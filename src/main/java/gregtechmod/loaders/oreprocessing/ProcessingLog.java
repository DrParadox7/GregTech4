    ...
    public void registerOre(OrePrefixes aPrefix, List<OreDictEntry> entries) {
        ...
        if (GregTech_API.sRecipeFile.get(GT_ConfigCategories.Recipes.disabledrecipes, "wood2charcoalsmelting", false)) {
            RecipeHandler.scheduleSmeltingToRemove(
                (in, out) -> out.isItemEqual(new ItemStack(Items.coal, 1, 1))
            ...
            if (this.isExecutable(aPrefix, aMaterial)) {
                ...
                for (ItemStack aStack : entry.ores) {
                    if (aStack.getItem() instanceof ItemBlock && GT_Mod.sWoodStackSize < aStack.getMaxStackSize()) {
                        aStack.getItem()
                            .setMaxStackSize(GT_Mod.sWoodStackSize);
                    }
                    ...
                    if (aMeta == 32767) {
                            ...
                            if (tStack != null) {
                                ...
                                RecipeHandler.executeOnFinish(() -> {
                                    GT_ModHandler.addCraftingRecipe(
                                        GT_Utility.copyAmount(
                                            GT_Mod.sNerfedWoodPlank ? tStack1.stackSize : (tStack1.stackSize * 5 / 4),
                                            tStack1),
                                        new Object[] { "S", "L", 'S', GT_ToolDictNames.craftingToolSaw, 'L', planks });
                                    GT_ModHandler.addShapelessCraftingRecipe(
                                        GT_Utility.copyAmount(
                                            (tStack1.stackSize / (GT_Mod.sNerfedWoodPlank ? 2 : 1)),
                                            tStack1),
                                        new Object[] { planks });
                                });
                            } else break;
                        ...
                        if (tStack != null) {
                            ...
                            RecipeHandler.executeOnFinish(() -> {
                                ...
                                GT_ModHandler.addShapelessCraftingRecipe(
                                    GT_Utility
                                        .copyAmount((tStack1.stackSize / (GT_Mod.sNerfedWoodPlank ? 2 : 1)), tStack1),
