                ...
                if (this.isExecutable(aPrefix, aMaterial) && !aMaterial.contains(SubTag.NO_SMASHING)) {
                    for (ItemStack aStack : entry.ores) {
                        if (GregTech_API.sRecipeFile
                            .get(GT_ConfigCategories.Tools.hammerrings, OrePrefixes.ring.get(aMaterial), true)) {
                            RecipeHandler.executeOnFinish(() -> {
                                GT_ModHandler.addCraftingRecipe(
                                    GT_Utility.copyAmount(1L, new Object[] { aStack }),
                                    new Object[] { "H ", " S", Character.valueOf('H'),
                                        GT_ToolDictNames.craftingToolHardHammer, Character.valueOf('S'),
