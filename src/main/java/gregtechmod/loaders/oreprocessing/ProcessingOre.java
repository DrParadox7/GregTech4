...
public class ProcessingOre implements IOreRecipeRegistrator {
        ...
        for (OreDictEntry entry : entries) {
            ...
            if (this.isExecutable(aPrefix, aMaterial)) {
                ...
                ItemStack tSmeltInto = tIngot == null
                    ? (aMaterial.contains(SubTag.SMELTING_TO_GEM) ? GT_OreDictUnificator.get(
                        OrePrefixes.gem,
                        aMaterial.mOreReplacement.mDirectSmelting,
                        GT_OreDictUnificator.get(
                            OrePrefixes.crystal,
                            aMaterial.mOreReplacement.mDirectSmelting,
                            GT_OreDictUnificator.get(
                                OrePrefixes.gem,
                                aMaterial.mOreReplacement,
                                GT_OreDictUnificator.get(OrePrefixes.crystal, aMaterial.mOreReplacement, 1L),
                                1L),
                            1L),
                        1L) : null)
                    : tIngot;
                ...
                for (ItemStack aStack : entry.ores) {
                    ...
                    if (aStack.getItem() instanceof ItemBlock && aStack.getMaxStackSize() > GT_Mod.sOreStackSize)
                        aStack.getItem()
                            .setMaxStackSize(GT_Mod.sOreStackSize);
                }
            }
        }
    }

    private static void defaultOreRegistration(OrePrefixes aPrefix, Materials aMaterial, ItemStack aOreStack,
        int aMultiplier, ItemStack tSmeltInto) {
    ...
    private static boolean registerStandardOreRecipes(OrePrefixes aPrefix, Materials aMaterial, OreDictEntry entry,
        int aMultiplier) {
        if (aMaterial != null) {
            ...
            if (tCrushed == null) {
                tCrushed = GT_OreDictUnificator.get(
                    ...
                    tMaterial,
                    GT_Utility
                        .copyAmount((aMaterial.mOreMultiplier * aMultiplier), new Object[] { tCleaned, tDust, tGem }),
            ...
            while (iterator.hasNext()) {
                ...
                if (tPrimaryByProduct == null) {
                    ...
                    if (tPrimaryByProductSmall == null) {
                        tPrimaryByProductSmall = GT_OreDictUnificator.get(
                ...
                if (tSecondaryByProduct == null || tSecondaryByMaterial == tPrimaryByMaterial) {
                    ...
                    if (tSecondaryByProductSmall == null) {
                        tSecondaryByProductSmall = GT_OreDictUnificator.get(
                            OrePrefixes.dustTiny,
                            tMat,
                            GT_OreDictUnificator.get(OrePrefixes.nugget, tMat, 2L),
                            2L);
            ...
            if (tSmeltInto != null) {
                    ...
                    GT_ModHandler.addInductionSmelterRecipe(
                        ...
                        GT_Utility.mul(
                            (aMultiplier * (aMaterial.contains(SubTag.INDUCTIONSMELTING_LOW_OUTPUT) ? 2 : 3)
                                * aMaterial.mSmeltingMultiplier),
                            tSmeltInto),
                        GT_Items.TE_Slag.get(aMultiplier),
                        300 * aMultiplier,
                        95);
                }
                ...
                if (aMaterial.contains(SubTag.BLASTFURNACE_CALCITE_TRIPLE)) {
                    RecipeMaps.BLAST_FURNACE.factory()
                        ...
                        .output(
                            GT_Items.TE_Slag.get(1L, GT_OreDictUnificator.get(OrePrefixes.dust, Materials.DarkAsh, 1L)))
                        .buildAndRegister();
            ...
            if (tCrushed != null) {
                ...
                if (GregTech_API.sRecipeFile
                    .get(GT_ConfigCategories.Tools.hammercrushing, aPrefix.get(aMaterial), true)) {
                    RecipeHandler.executeOnFinish(
                        () -> GT_ModHandler.addCraftingRecipe(
                            copy,
                            new Object[] { "T", "O", 'T', GT_ToolDictNames.craftingToolHardHammer, 'O',
                                aPrefix.get(aMaterial) }));
                }

                RecipeMaps.HAMMER.factory()
                    .EUt(10)
                    .duration(16)
                    .input(ingr)
                    .output(tCrushed)
                    .buildAndRegister();
                RecipeHandler.scheduleIC2RecipeToRemove(
                    GT_ModHandler.getMaceratorRecipeList(),
                    (in, out) -> in.matches(entry.ores.get(0)));
                final ItemStack out1 = tMaterial.contains(SubTag.PULVERIZING_CINNABAR)
                    ? GT_OreDictUnificator
                        .get(OrePrefixes.crystal, Materials.Cinnabar, GT_Utility.copyAmount(1, tPrimaryByProduct), 1L)
                    : GT_Utility.copyAmount(1, tPrimaryByProduct);
                final int val = tPrimaryByProduct == null ? 0
                    : tPrimaryByProduct.stackSize * 10 * aMultiplier * aMaterial.mByProductMultiplier;
                RecipeHandler.executeOnFinish(
                    () -> GT_ModHandler.addPulverisationRecipe(entry, 1, GT_Utility.mul(2L, copy), out1, val));
                ...
                if (tGem != null) {
                    factory = RecipeMaps.GRINDER.factory()
                        ...
                        .output(
                            tSmall == null ? GT_Utility.mul(aMultiplier * aMaterial.mOreMultiplier * 2, tCleaned)
                                : GT_Utility.mul(aMultiplier * aMaterial.mOreMultiplier * 6, tSmall));
                    if (tPrimaryByProductSmall != null) factory.output(
                        GT_Utility.mul(aMultiplier * 2 * aMaterial.mByProductMultiplier, tPrimaryByProductSmall));
                    ...
                    if (tMaterial.contains(SubTag.WASHING_MERCURY)) {
                        if (tSmall == null) {
                            ...
                            if (tPrimaryByProductSmall != null) factory.output(
                                GT_Utility
                                    .mul(aMultiplier * 2 * aMaterial.mByProductMultiplier, tPrimaryByProductSmall));

                            factory.output(GT_Items.Cell_Empty.get(aMultiplier))
                                .buildAndRegister();
                        }
                    ...
                    if (tPrimaryByMaterial.contains(SubTag.WASHING_MERCURY)) {
                        factory = RecipeMaps.GRINDER.factory()
                            ...
                            .input(GT_OreDictUnificator.get(OrePrefixes.cell, Materials.Mercury, aMultiplier))
                            .output(GT_Utility.mul(aMultiplier * aMaterial.mOreMultiplier, tGem));
                        if (tCleaned != null || tSmall != null) factory.output(
                            tSmall == null ? GT_Utility.mul((aMultiplier * 2 * aMaterial.mOreMultiplier), tCleaned)
                                : GT_Utility.mul((aMultiplier * 6 * aMaterial.mOreMultiplier), tSmall));
                        if (tPrimaryByProductSmall != null) factory.output(
                            GT_Utility.mul(aMultiplier * 2 * aMaterial.mByProductMultiplier, tPrimaryByProduct));
                        factory.output(GT_Items.Cell_Empty.get(aMultiplier))
                            .buildAndRegister();
                    }
                    ...
                    if (tMaterial.contains(SubTag.WASHING_SODIUMPERSULFATE)) {
                        if (tSmall == null) {
                            factory = RecipeMaps.GRINDER.factory()
                                ...
                                .input(ingr)
                                .input(
                                    GT_OreDictUnificator.get(OrePrefixes.cell, Materials.SodiumPersulfate, aMultiplier))
                                .output(GT_Utility.mul(aMultiplier * aMaterial.mOreMultiplier, tGem));
                            if (tCleaned != null)
                                factory.output(GT_Utility.mul(aMultiplier * 3 * aMaterial.mOreMultiplier, tCleaned));
                            if (tPrimaryByProductSmall != null) factory.output(
                    ...
                    if (tPrimaryByMaterial.contains(SubTag.WASHING_SODIUMPERSULFATE)) {
                        factory = RecipeMaps.GRINDER.factory()
                            ...
                            .input(GT_OreDictUnificator.get(OrePrefixes.cell, Materials.SodiumPersulfate, aMultiplier))
                            .output(GT_Utility.mul(aMultiplier * aMaterial.mOreMultiplier, tGem));
                        if (tSmall != null || tCleaned != null) factory.output(
                            tSmall == null ? GT_Utility.mul((aMultiplier * 2 * aMaterial.mOreMultiplier), tCleaned)
                                : GT_Utility.mul((aMultiplier * 6 * aMaterial.mOreMultiplier), tSmall));
                        if (tPrimaryByProduct != null) factory.output(
                            GT_Utility.mul(aMultiplier * 2 * aMaterial.mByProductMultiplier, tPrimaryByProduct));
                        factory.output(GT_Items.Cell_Empty.get(aMultiplier))
                            .buildAndRegister();
                    }
                    ...
                    if (tMaterial.contains(SubTag.WASHING_MERCURY)) {
                        ...
                        if (tPrimaryByProductSmall != null) factory.output(
                            GT_Utility.mul(aMultiplier * aMaterial.mByProductMultiplier, tPrimaryByProductSmall));
                        if (tSecondaryByProductSmall != null) factory.output(
                            GT_Utility.mul(aMultiplier * aMaterial.mByProductMultiplier, tSecondaryByProductSmall));
                        factory.output(GT_Items.Cell_Empty.get(aMultiplier))
                            .buildAndRegister();
                    }
