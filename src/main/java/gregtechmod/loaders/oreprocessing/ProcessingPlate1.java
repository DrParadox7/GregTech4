    ...
    public void registerOre(OrePrefixes aPrefix, List<OreDictEntry> entries) {
        for (OreDictEntry entry : entries) {
            ...
            if (this.isExecutable(aPrefix, aMaterial)) {
                RecipeHandler
                    .scheduleCraftingToRemove(new RecipeHandler.InventoryRecipeMatcher(true, entry.ores.get(0)));
                ...
                if (!aMaterial.contains(SubTag.NO_SMASHING)) {
                    ...
                    if (GregTech_API.sRecipeFile
                        .get(GT_ConfigCategories.Tools.hammerplating, OrePrefixes.ingot.get(aMaterial), true)) {
                    ...
                    if (GregTech_API.sRecipeFile
                        .get(GT_ConfigCategories.Tools.hammerplating, OrePrefixes.ingotDouble.get(aMaterial), true)) {
                        RecipeHandler.executeOnFinish(() -> {
                            GT_ModHandler.addCraftingRecipe(
                                GT_Utility.copyAmount(1, aStack),
                ...
                for (ItemStack aStack : entry.ores) {
                    RecipeHandler.scheduleCraftingToRemove(
                        rec -> rec != null && rec.getRecipeOutput() != null
                            && rec.getRecipeOutput()
                                .isItemEqual(aStack));
