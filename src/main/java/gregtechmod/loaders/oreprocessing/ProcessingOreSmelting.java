...
public class ProcessingOreSmelting implements IOreRecipeRegistrator {
    ...
    private final static OrePrefixes[] mSmeltingPrefixes = new OrePrefixes[] { OrePrefixes.crushed,
        OrePrefixes.crushedPurified, OrePrefixes.crushedCentrifuged, OrePrefixes.dustImpure, OrePrefixes.dustPure,
        ...
        for (OreDictEntry entry : entries) {
            ...
            if (this.isExecutable(aPrefix, aMaterial) && !aMaterial.contains(SubTag.NO_SMELTING)) {
                if (!aMaterial.mBlastFurnaceRequired && !aMaterial.mDirectSmelting.mBlastFurnaceRequired) {
                    switch (aPrefix) {
                        ...
                        case crushedCentrifuged:
                            ItemStack tStack = GT_OreDictUnificator
                                .get(OrePrefixes.nugget, aMaterial.mDirectSmelting, 10L);
                            if (tStack == null)
                                tStack = GT_OreDictUnificator.get(OrePrefixes.ingot, aMaterial.mDirectSmelting, 1L);

                            for (ItemStack aStack : entry.ores) GT_ModHandler.addSmeltingRecipe(aStack, tStack);
                            break;
                        default:
                            for (ItemStack aStack : entry.ores) GT_ModHandler.addSmeltingRecipe(
                                aStack,
                    ...
                    ItemStack out = aMaterial.mBlastFurnaceTemp > 1750 ? GT_OreDictUnificator.get(
                    ...
                    RecipeMaps.BLAST_FURNACE.factory()
                        ...
                        .EUt(120)
                        .duration(Math.max(aMaterial.getMass() / 4, 1) * aMaterial.mBlastFurnaceTemp)
                        .input(RecipeEntry.fromStacks(entry.ores, Match.STRICT))
                        .output(out)
                        .buildAndRegister();
                    if (aMaterial.mBlastFurnaceTemp <= 1000) {
                        for (ItemStack aStack : entry.ores) GT_ModHandler.addRCBlastFurnaceRecipe(
