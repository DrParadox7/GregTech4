            ...
            if (this.isExecutable(aPrefix, aMaterial)) {
                if (aMaterial != Materials.Empty) {
                    ...
                    if (aMaterial.mMaterialList.size() > 0 && (aMaterial.mExtraData & 3) != 0) {
                        ...
                        for (MaterialStack tMat : aMaterial.mMaterialList) totalMaterialAmount += tMat.mAmount;
                        int tItemAmount1 = 0;
                                ...
                                if (tMat1.mMaterial == Materials.Oxygen) {
                                    ...
                                    if (tStack == null) {
                                        tStack = GT_OreDictUnificator
                                            .get(OrePrefixes.cell, tMat1.mMaterial, tMat1.mAmount);
                                    }
                                }

                                if (tItemAmount1 + (tMat1.mAmount * GregTech_API.MATERIAL_UNIT)
                                    <= 64 * aMaterial.getDensity()) {
                                    tItemAmount1 += tMat1.mAmount * GregTech_API.MATERIAL_UNIT;
                                    if (tStack != null) {
                                        for (tStack.stackSize *= tDensityMultiplier; tStack.stackSize
                                            > 64; tStack.stackSize -= 64) {
                                            if (capsuleCount + GT_ModHandler.getCapsuleCellContainerCount(tStack) * 64
                                                < 0) {
                                                if (tList.size() >= 3) {
                                                    break;
                                                }
                                        ...
                                        if (tStack.stackSize > 0 && capsuleCount
                                            + GT_ModHandler.getCapsuleCellContainerCountMultipliedWithStackSize(tStack)
                                            <= 64) {
                                            if (capsuleCount + GT_ModHandler
                                                .getCapsuleCellContainerCountMultipliedWithStackSize(tStack) < 0) {
                                                if (tList.size() >= 3) {
                                                    continue;
                                                }
                                            } else if (tList.size() >= 4) {
                        ...
                        tItemAmount1 = (int) ((tItemAmount1 * tDensityMultiplier % aMaterial.getDensity() > 0L ? 1 : 0)
                            + tItemAmount1 * tDensityMultiplier / aMaterial.getDensity());
                        if (tList.size() > 0) {
                            if ((aMaterial.mExtraData & 1) != 0) {
                                ...
                                if (capsuleCount > 0) factory.input(GT_Items.Cell_Empty.get(capsuleCount));
                                else if (capsuleCount != 0) factory.output(GT_Items.Cell_Empty.get(-capsuleCount));
                    ...
                    for (ItemStack aStack : entry.ores) {
                        if (aMaterial.mFuelPower > 0) {
                            ...
                            if (fuel != null) {
                                factory.input(new FluidStack(fuel, 1))
                                    .duration(aMaterial.mFuelPower);
                            } else {
                                ItemStack container = GT_Utility.getContainerItem(aStack);
                                factory.input(aStack)
                                    .duration(aMaterial.mFuelPower * 1000);
