...
public class GT_Mod implements IGT_Mod {
    ...
    public static boolean sThaumiumObtainable = false, sNerfDustCrafting = true, sMinimalItemGeneration = false,
        sSortToTheEnd = true, sCraftingUnification = true, sInventoryUnification = true, sIncreaseDungeonLoot = true,
        sBookDungeonLoot = true, sAxeWhenAdventure = true, sSurvivalIntoAdventure = false, sNerfedWoodPlank = true,
        sNerfedWoodenTools = true, sNerfedStoneTools = true, sInvisibleOres = false, sHardRock = false,
        sHungerEffect = true, mOnline = true, mAlreadyPlayed = false, mDoNotInit = false;
    public static int sItemDespawnTime = 6000, sUpgradeCount = 4, sBlockStackSize = 64, sOreStackSize = 64,
        sWoodStackSize = 64, sPlankStackSize = 64;
    ...
    public static final ArrayList<String> mGregTechCapeList = new ArrayList<>(
        Arrays.asList(
            new String[] { "Plem", "invultri", "Malevolence_", "Archibald_McShane", "Sirbab", "kehaan", "bpgames123",
                "semig0d", "9000bowser", "Sovereignty89", "Kris1432", "xander_cage_", "samuraijp", "bsaa", "SpwnX",
                "tworf", "Kadah", "kanni", "Stute", "Hegik", "Onlyme", "t3hero", "Hotchi", "jagoly", "Nullav", "BH5432",
                "Sibmer", "inceee", "foxxx0", "Hartok", "TMSama", "Shlnen", "Carsso", "zessirb", "meep310", "Seldron",
                "yttr1um", "hohounk", "freebug", "Sylphio", "jmarler", "Saberawr", "r00teniy", "Neonbeta", "yinscape",
                "voooon24", "Quintine", "peach774", "lepthymo", "bildeman", "Kremnari", "Aerosalo", "OndraSter",
                ...
                "Speedynutty68", "GarretSidzaka", "HallowCharm977", "mastermind1919", "The_Hypersonic",
                "diamondguy2798", "zF4ll3nPr3d4t0r", "CrafterOfMines57", "XxELIT3xSNIP3RxX", "adamros", "alexbegt" }));
    public static final ArrayList<String> mBrainTechCapeList = new ArrayList<>(
        Arrays.asList(new String[] { "Friedi4321" }));
    ...
    private static final void checkVersions() {
        if (VERSION != GregTech_API.VERSION || VERSION != BaseMetaTileEntity.VERSION
    ...
    public GT_Mod() {
        checkVersions();
        if (GregTech_API.isGregTechLoaded()) throw new GT_ItsNotMyFaultException(
            "Why did you install my Addon twice? Remove the second gregtechmod.zip out of your mods-Folder, you need only one of them.");
        ...
        for (int i = 0; i < mGregTechCapeList.size(); ++i) {
            mGregTechCapeList.set(
                i,
                mGregTechCapeList.get(i)
                    .toLowerCase());
        }
    ...
    public void preload(FMLPreInitializationEvent aEvent) {
        ...
        try {
            Integer.parseInt(
                ((String) Class.forName("ic2.core.IC2")
                    .getField("VERSION")
                    .get((Object) null)).substring(4, 7));
        } catch (Throwable var16) {
            throw new GT_ItsNotMyFaultException(
                "Ancient IndustrialCraft Version detected, please update your IndustrialCraft here: ic2api.player.to:8080/job/IC2_experimental/?");
        }
        ...
        GregTech_API.sSpecialFile.mConfig.getCategory(GT_ConfigCategories.enchants.toString())
            .setComment(
                "There you can set allowed enchant IDs to specific GT tools\nIt will let you apply choosen enchant in anvil to GT tool");
        GregTech_API.sUnification.mConfig.getCategory(GT_ConfigCategories.forceoredict.toString())
            .setComment(
        ...
        mDoNotInit = (!tFile.getAbsolutePath()
            .toLowerCase()
            .contains("voltz"))
        ...
        if (mDoNotInit) {
             ...
             * Why? Because it's to prevent Kakermix from adding it, without being kind enough to just ask, and not to
             * prevent Tekkit Users from playing.
            ...
            GT_Log.log.error(
                "I see you most likely use Tekkit, but this Mod won't load, until Kakermix asks me PERSONALLY for the inclusion of my Mod. So bug HIM for it.");
            ...
            GT_Log.log.error(
                "PPPS. There has been already so many Discoussions about me detecting Tekkit more or less precicely or even ACCIDENTLY on non-Tekkit Users, and NO YOU WON'T GET ME TO 'JUST' REMOVE THIS, GO LIVE WITH IT OR FIX IT YOURSELF!");
            GT_Log.log.error("*");
            GT_Log.log.error("*");
            GT_Log.log.error("*");
        ...
        GT_Items.IC2_Industrial_Diamond
        ...
        if (tConfig1.get("general", "TooEasyMode", false)
            .getBoolean(false)) GregTech_API.sAfterGTPostload.add(new GT_TooEasyModeLoader());
        GregTech_API.DEBUG_MODE = tConfig1.get("general", "Debug", false)
            .getBoolean(false);
        GregTech_API.SECONDARY_DEBUG_MODE = tConfig1.get("general", "Debug2", false)
            .getBoolean(false);
        GregTech_API.OREDICT_DEBUG_MODE = tConfig1.get("general", "OredictDebug", false)
            .getBoolean(false);
        GregTech_API.TICKS_FOR_LAG_AVERAGING = tConfig1.get("general", "TicksForLagAveragingWithScanner", 25)
            .getInt(25);
        GregTech_API.MILLISECOND_THRESHOLD_UNTIL_LAG_WARNING = tConfig1
            .get("general", "MillisecondsPassedInGTTileEntityUntilLagWarning", 100)
            .getInt(100);

        if (tConfig1.get("general", "disable_STDOUT", false)
            .getBoolean(false)) System.out.close();
        if (tConfig1.get("general", "disable_STDERR", false)
            .getBoolean(false)) System.err.close();

        GregTech_API.IC_ENERGY_COMPATIBILITY = tConfig1.get("compatibility", "Industrialcraft.Energy", true)
            .getBoolean(true);
        GregTech_API.RF_ENERGY_COMPATIBILITY = tConfig1.get("compatibility", "Cofh.Energy", true)
            .getBoolean(true);

        GregTech_API.sMachineExplosions = tConfig1.get("machines", "machines_explosion_damage", true)
            .getBoolean(false);
        GregTech_API.sMachineFlammable = tConfig1.get("machines", "machines_flammable", true)
            .getBoolean(false);
        GregTech_API.sMachineNonWrenchExplosions = tConfig1.get("machines", "explosions_on_nonwrenching", true)
            .getBoolean(false);
        GregTech_API.sMachineWireFire = tConfig1.get("machines", "wirefire_on_explosion", true)
            .getBoolean(false);
        GregTech_API.sMachineFireExplosions = tConfig1.get("machines", "fire_causes_explosions", true)
            .getBoolean(false);
        GregTech_API.sMachineRainExplosions = tConfig1.get("machines", "rain_causes_explosions", true)
            .getBoolean(false);
        GregTech_API.sMachineThunderExplosions = tConfig1.get("machines", "lightning_causes_explosions", true)
            .getBoolean(false);
        GregTech_API.sConstantEnergy = tConfig1.get("machines", "constant_need_of_energy", true)
            .getBoolean(false);
        GregTech_API.sColoredGUI = tConfig1.get("machines", "colored_guis_when_painted", true)
            .getBoolean(false);
        GregTech_API.sBasicBending = tConfig1.get("machines", "plate_bending_uses_no_circuit", false)
            .getBoolean(true);
        GregTech_API.sDoShowAllItemsInCreative = tConfig1
            .get("general", "show_all_metaitems_in_creative_and_NEI", false)
            .getBoolean(false);

        sItemDespawnTime = tConfig1.get("general", "ItemDespawnTime", 6000)
            .getInt(6000);
        sNerfDustCrafting = tConfig1.get("general", "NerfDustCrafting", true)
            .getBoolean(true);
        sMinimalItemGeneration = tConfig1.get("general", "MinimalItemGeneration", false)
            .getBoolean(false);
        sIncreaseDungeonLoot = tConfig1.get("general", "IncreaseDungeonLoot", true)
            .getBoolean(true);
        sBookDungeonLoot = tConfig1.get("general", "BookLoot", true)
            .getBoolean(true);
        sAxeWhenAdventure = tConfig1.get("general", "AdventureModeStartingAxe", true)
            .getBoolean(true);
        sSurvivalIntoAdventure = tConfig1.get("general", "forceAdventureMode", false)
            .getBoolean(false);
        sHungerEffect = tConfig1.get("general", "AFK_Hunger", false)
            .getBoolean(false);
        sHardRock = tConfig1.get("general", "harderstone", false)
            .getBoolean(false);
        sInvisibleOres = tConfig1.get("general", "hiddenores", true)
            .getBoolean(true);
        sInventoryUnification = tConfig1.get("general", "InventoryUnification", true)
            .getBoolean(true);
        sCraftingUnification = tConfig1.get("general", "CraftingUnification", true)
            .getBoolean(true);
        sNerfedWoodPlank = tConfig1.get("general", "WoodNeedsSawForCrafting", true)
            .getBoolean(true);
        sNerfedWoodenTools = tConfig1.get("general", "smallerWoodToolDurability", true)
            .getBoolean(true);
        sNerfedStoneTools = tConfig1.get("general", "smallerStoneToolDurability", true)
            .getBoolean(true);
        sSortToTheEnd = tConfig1.get("general", "EnsureToBeLoadedLast", true)
            .getBoolean(true);
        mOnline = tConfig1.get("general", "online", true)
            .getBoolean(false);

        if (tConfig1.get("general", "hardermobspawners", true)
            .getBoolean(true)) Blocks.mob_spawner.setHardness(2000.0F);
        GT_BlockMetaID_Block.mConnectedMachineTextures = tConfig1.get("general", "ConnectedMachineCasingTextures", true)
            .getBoolean(false);

        sUpgradeCount = Math.min(
            64,
            Math.max(
                1,
                tConfig1.get("features", "UpgradeStacksize", 4)
                    .getInt()));
        sOreStackSize = Math.min(
            64,
            Math.max(
                16,
                tConfig1.get("features", "MaxOreStackSize", 64)
                    .getInt()));
        sWoodStackSize = Math.min(
            64,
            Math.max(
                16,
                tConfig1.get("features", "MaxLogStackSize", 64)
                    .getInt()));
        sPlankStackSize = Math.min(
            64,
            Math.max(
                16,
                tConfig1.get("features", "MaxPlankStackSize", 64)
                    .getInt()));
        sBlockStackSize = Math.min(
            64,
            Math.max(
                16,
                tConfig1.get("features", "MaxOtherBlockStackSize", 64)
                    .getInt()));
        ...
        GT_Config.system = (!GregTech_API.DEBUG_MODE && Calendar.getInstance()
            .get(Calendar.MONTH) + 1 == 4
                    ...
                    public void run() {
                        try {
                            Scanner tScanner = new Scanner(
                                new URL("http://gregtech.mechaenetia.com/com/gregoriust/gregtech/supporterlist.txt")
                                    .openStream());
            ...
            try {
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        try {
                            Scanner tScanner = new Scanner(
                                new URL("http://gregtech.mechaenetia.com/com/gregoriust/gregtech/message.txt")
                                    .openStream());
        ...
        GameRegistry
            .registerBlock(GregTech_API.sBlockList[0] = new GT_BlockMetaID_Block(), GT_MetaBlock_Item.class, "block");
        GameRegistry.registerBlock(
            GregTech_API.sBlockList[1] = new GT_BlockMetaID_Machine(),
            GT_MetaMachine_Item.class,
            "machine");
        GameRegistry.registerBlock(GregTech_API.sBlockList[2] = new GT_BlockMetaID_Ore(), GT_MetaOre_Item.class, "ore");
        GameRegistry
            .registerBlock(GregTech_API.sBlockList[3] = new GT_Block_LightSource(), ItemBlock.class, "light_source");
        GameRegistry.registerBlock(
            GregTech_API.sBlockList[4] = new GT_BlockMetaID_Block2(),
            GT_MetaBlock2_Item.class,
            "block_2");
        GameRegistry
            .registerBlock(GregTech_API.sBlockList[5] = new GT_BlockMetaID_Stone1(), GT_MetaStone1_Item.class, "stone");

        GregTech_API.registerMachineBlock(
            GregTech_API.sBlockList[0],
            new boolean[] { true, true, false, false, false, false, true, false, false, false, true, false, false, true,
                true, true });
        GregTech_API.registerMachineBlock(GregTech_API.sBlockList[1], new boolean[] { true });
        GregTech_API.registerMachineBlock(
            GregTech_API.sBlockList[4],
        ...
        GameRegistry.registerTileEntity(
            GregTech_API.constructBaseMetaTileEntity()
                .getClass(),
            "MetatileEntity");
        ...
        if (GregTech_API.constructBaseMetaTileEntity() == null) {
            ...
            if (sSortToTheEnd) {
                try {
                    ...
                    LoadController controller = (LoadController) GT_Utility
                    ...
                    for (short i = 0; i < mods.size(); ++i) {
                        ...
                        if (tMod.getModId()
                            .equalsIgnoreCase(GregTech_API.MOD_ID)) {
                            mod = tMod;
                        } else {
                            sorted.add(tMod);
                        }
                    ...
                    if (mod != null) sorted.add(mod);

                    GT_Utility.getField(controller, "activeModList", true, true)
                        .set(controller, sorted);
                } catch (Throwable e) {
        ...
        RecipeSorter.register(
            "gregtech_addon:shaped",
            GT_Shaped_Recipe.class,
            RecipeSorter.Category.SHAPED,
            "after:forge:shapedore before:minecraft:shapeless");
        RecipeSorter.register(
            "gregtech_addon:shaped_nbt_keeping",
            GT_Shaped_NBT_Keeping_Recipe.class,
            RecipeSorter.Category.SHAPED,
            "after:gregtech_addon:shaped before:minecraft:shapeless");
        RecipeSorter.register(
            "gregtech_addon:shapeless",
            GT_Shapeless_Recipe.class,
            RecipeSorter.Category.SHAPELESS,
            "after:minecraft:shapeless");
        RecipeSorter.register(
            "gregtech_addon:shapeless_nbt_keeping",
            GT_Shapeless_NBT_Keeping_Recipe.class,
            RecipeSorter.Category.SHAPELESS,
            "after:gregtech_addon:shapeless");
    ...
    public void postload(FMLPostInitializationEvent aEvent) {
        ...
        if (GregTech_API.sRecipeFile.get(GT_ConfigCategories.Recipes.disabledrecipes, "ic2forgehammer", true))
            RecipeHandler.scheduleCraftingToRemove(
                new RecipeHandler.OutputMatcher(false, GT_Items.Tool_Hammer_Forge.getWildcard(1)));
        ...
        if (GregTech_API.sRecipeFile.get(GT_ConfigCategories.Recipes.recipereplacements, "Iron.PressurePlate", true))
            RecipeHandler.scheduleCraftingToRemove(
                new GT_RecipeRegistrator.DoubleMatcher(
        ...
        if (GregTech_API.sRecipeFile.get(GT_ConfigCategories.Recipes.recipereplacements, "Rubber.Sheet", true))
            RecipeHandler.scheduleCraftingToRemove(
                new GT_RecipeRegistrator.DoubleMatcher(
                    GT_ModHandler.getIC2Item("rubberTrampoline", 1),
                    false,
                    tMat,
                    tMat,
                    tMat,
        ...
        RecipeHandler.executeOnFinish(() -> {
            if (GregTech_API.sRecipeFile
                .get(GT_ConfigCategories.Recipes.recipereplacements, "Iron.PressurePlate", true))
                GameRegistry.addRecipe(
                    new ShapedOreRecipe(
                        ...
                        "plateIron",
                        'T',
                        GT_ToolDictNames.craftingToolHardHammer.toString()));
            if (GregTech_API.sRecipeFile.get(GT_ConfigCategories.Recipes.recipereplacements, "Iron.Bucket", true))
                GameRegistry.addRecipe(
                    new ShapedOreRecipe(
                        new ItemStack(Items.bucket),
            ...
            if (GregTech_API.sRecipeFile.get(GT_ConfigCategories.Recipes.recipereplacements, "Iron.Hopper", true))
                GameRegistry.addRecipe(
                    new ShapedOreRecipe(
                        ...
                        "plateIron",
                        'W',
                        GT_ToolDictNames.craftingToolWrench.toString(),
                        'C',
                        "craftingChest"));
            if (GregTech_API.sRecipeFile.get(GT_ConfigCategories.Recipes.recipereplacements, "Iron.Bars", true))
                GameRegistry.addRecipe(
                    new ShapedOreRecipe(
                        new ItemStack(Blocks.iron_bars, 8),
                        " W ",
                        "XXX",
                        "XXX",
                        'X',
                        ...
                        GT_ToolDictNames.craftingToolWrench.toString()));
            if (GregTech_API.sRecipeFile
                .get(GT_ConfigCategories.Recipes.recipereplacements, "Gold.PressurePlate", true))
                GameRegistry.addRecipe(
                    new ShapedOreRecipe(
                        new ItemStack(Blocks.light_weighted_pressure_plate),
                        "XXT",
            ...
            if (GregTech_API.sRecipeFile.get(GT_ConfigCategories.Recipes.recipereplacements, "Rubber.Sheet", true))
                GameRegistry.addRecipe(
                    new ShapedOreRecipe(
                        GT_ModHandler.getIC2Item("rubberTrampoline", 3),
                        "XXX",
                        "XXX",
                        'X',
                        "plateRubber"));
        ...
        GT_Log.log
            .info("Activating OreDictionary Handler, this can take some time, as it scans the whole OreDictionary");
        ...
        GT_ModHandler.addSmeltingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 1, 0),
            new ItemStack(GregTech_API.sBlockList[5], 1, 7));
        GT_ModHandler.addSmeltingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 1, 1),
            new ItemStack(GregTech_API.sBlockList[5], 1, 0));
        GT_ModHandler.addSmeltingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 1, 2),
            new ItemStack(GregTech_API.sBlockList[5], 1, 0));
        GT_ModHandler.addSmeltingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 1, 3),
            new ItemStack(GregTech_API.sBlockList[5], 1, 0));
        GT_ModHandler.addSmeltingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 1, 4),
            new ItemStack(GregTech_API.sBlockList[5], 1, 0));
        GT_ModHandler.addSmeltingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 1, 5),
            new ItemStack(GregTech_API.sBlockList[5], 1, 0));
        GT_ModHandler.addSmeltingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 1, 6),
            new ItemStack(GregTech_API.sBlockList[5], 1, 0));
        GT_ModHandler.addSmeltingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 1, 7),
            new ItemStack(GregTech_API.sBlockList[5], 1, 0));
        GT_ModHandler.addSmeltingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 1, 8),
            new ItemStack(GregTech_API.sBlockList[5], 1, 15));
        GT_ModHandler.addSmeltingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 1, 9),
            new ItemStack(GregTech_API.sBlockList[5], 1, 8));
        GT_ModHandler.addSmeltingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 1, 10),
            new ItemStack(GregTech_API.sBlockList[5], 1, 8));
        GT_ModHandler.addSmeltingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 1, 11),
            new ItemStack(GregTech_API.sBlockList[5], 1, 8));
        GT_ModHandler.addSmeltingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 1, 12),
            new ItemStack(GregTech_API.sBlockList[5], 1, 8));
        GT_ModHandler.addSmeltingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 1, 13),
            new ItemStack(GregTech_API.sBlockList[5], 1, 8));
        GT_ModHandler.addSmeltingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 1, 14),
            new ItemStack(GregTech_API.sBlockList[5], 1, 8));
        GT_ModHandler.addSmeltingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 1, 15),
            new ItemStack(GregTech_API.sBlockList[5], 1, 8));
        ...
        RecipeMaps.ASSEMBLING.factory()
            .EUt(4)
            .duration(200)
            .input(new ItemStack(GregTech_API.sBlockList[5], 1, 3))
            .output(new ItemStack(GregTech_API.sBlockList[5], 1, 4))
            .buildAndRegister();
        RecipeMaps.ASSEMBLING.factory()
            .EUt(4)
            .duration(200)
            .input(new ItemStack(GregTech_API.sBlockList[5], 1, 11))
            .output(new ItemStack(GregTech_API.sBlockList[5], 1, 12))
            .buildAndRegister();
        RecipeMaps.ASSEMBLING.factory()
            .EUt(4)
            .duration(200)
            .input(new ItemStack(GregTech_API.sBlockList[5], 1, 7))
            .output(new ItemStack(GregTech_API.sBlockList[5], 1, 6))
            .buildAndRegister();
        RecipeMaps.ASSEMBLING.factory()
            .EUt(4)
            .duration(200)
            .input(new ItemStack(GregTech_API.sBlockList[5], 1, 15))
            .output(new ItemStack(GregTech_API.sBlockList[5], 1, 14))
            .buildAndRegister();

        GT_ModHandler.addShapelessCraftingRecipe(
            new ItemStack(Blocks.stonebrick, 1, 3),
            new Object[] { new ItemStack(Blocks.double_stone_slab, 1, 8), GT_ToolDictNames.craftingToolFile });
        GT_ModHandler.addShapelessCraftingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 1, 6),
            new Object[] { new ItemStack(GregTech_API.sBlockList[5], 1, 7), GT_ToolDictNames.craftingToolFile });
        GT_ModHandler.addShapelessCraftingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 1, 14),
            new Object[] { new ItemStack(GregTech_API.sBlockList[5], 1, 15), GT_ToolDictNames.craftingToolFile });
        GT_ModHandler.addShapelessCraftingRecipe(
            new ItemStack(Blocks.cobblestone, 1, 0),
            new Object[] { new ItemStack(Blocks.stone, 1, 0), GT_ToolDictNames.craftingToolHardHammer });
        GT_ModHandler.addShapelessCraftingRecipe(
            new ItemStack(Blocks.stonebrick, 1, 2),
            new Object[] { new ItemStack(Blocks.stonebrick, 1, 0), GT_ToolDictNames.craftingToolHardHammer });
        GT_ModHandler.addShapelessCraftingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 1, 4),
            new Object[] { new ItemStack(GregTech_API.sBlockList[5], 1, 3), GT_ToolDictNames.craftingToolHardHammer });
        GT_ModHandler.addShapelessCraftingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 1, 12),
            new Object[] { new ItemStack(GregTech_API.sBlockList[5], 1, 11), GT_ToolDictNames.craftingToolHardHammer });

        GT_ModHandler.addCraftingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 4, 3),
            new Object[] { "XX", "XX", 'X', new ItemStack(GregTech_API.sBlockList[5], 4, 0) });
        GT_ModHandler.addCraftingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 4, 11),
            new Object[] { "XX", "XX", 'X', new ItemStack(GregTech_API.sBlockList[5], 4, 8) });
        GT_ModHandler.addCraftingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 4, 3),
            new Object[] { "XX", "XX", 'X', new ItemStack(GregTech_API.sBlockList[5], 4, 7) });
        GT_ModHandler.addCraftingRecipe(
            new ItemStack(GregTech_API.sBlockList[5], 4, 11),
            new Object[] { "XX", "XX", 'X', new ItemStack(GregTech_API.sBlockList[5], 4, 15) });
        ...
        GT_ModHandler.addShapelessCraftingRecipe(
            ...
            false,
            new Object[] { new ItemStack(Blocks.deadbush, 1, 32767) });
        GT_ModHandler.addShapelessCraftingRecipe(
            new ItemStack(Items.stick, 2, 0),
            false,
            new Object[] { new ItemStack(Blocks.tallgrass, 1, 0) });
        ...
        if (tStack != null) {
            GameRegistry.addRecipe(
                new ShapedOreRecipe(
                    ...
                    "plankWood",
                    'S',
                    GT_ToolDictNames.craftingToolSaw.toString()));
            GameRegistry.addRecipe(
                new ShapedOreRecipe(
                    GT_Utility
                        .copyAmount(GT_Mod.sNerfedWoodPlank ? ((tStack.stackSize / 2)) : (tStack.stackSize), tStack),
                    "P",
                    "P",
                    'P',
                    "plankWood"));
        ...
        GT_ModHandler.addCraftingRecipe(
            ...
            false,
            false,
            new Object[] { " T ", "TQT", "SSS", 'Q', "craftingQuartz", 'S', "stoneSmooth", 'T',
        ...
        if (GregTech_API.DEBUG_MODE) {
            try {
                ...
                EnumMap<Side, Map<String, FMLEmbeddedChannel>> channels = ReflectionHelper
                    .getPrivateValue(NetworkRegistry.class, NetworkRegistry.INSTANCE, new String[] { "channels" });
                if (channels != null) {
                    Map<String, FMLEmbeddedChannel> serverChannel = channels.get(Side.SERVER);
                    serverChannel.keySet()
                        .forEach(key -> GT_Log.log.info("  " + key));
                }
            } catch (Throwable e) {
                GT_Log.log.catching(e);
            }
        ...
        try {
            if (GregTech_API.DEBUG_MODE) {
                ...
                for (String name : names) {
                    int tAmount = OreDictionary.getOres(name)
                        .size();
                    if (tAmount > 0) {
    ...
    public static File getSaveDirectory() {
        return mUniverse != null ? mUniverse.getSaveHandler()
