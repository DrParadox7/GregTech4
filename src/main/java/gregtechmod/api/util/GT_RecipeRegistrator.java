...
public class GT_RecipeRegistrator {
     ...
     */
    public static void registerBasicReverseMaceratingAndSmelting(ItemStack aStack, Materials aMaterial,
        long aMaterialAmount, boolean aAllowAlloySmelter) {
    ...
    public static void registerBasicReverseSmelting(ItemStack aStack, Materials aMaterial, long aMaterialAmount,
        ...
        ItemStack tStack1 = GT_OreDictUnificator.get(OrePrefixes.ingot, aMaterial, 1),
            tStack2 = GT_OreDictUnificator.get(OrePrefixes.nugget, aMaterial, 1);
        if (tStack1 != null && aMaterialAmount % GregTech_API.MATERIAL_UNIT == 0) {
            if (aAllowAlloySmelter) GT_ModHandler.addSmeltingAndAlloySmeltingRecipe(
                ...
                GT_OreDictUnificator
                    .get(OrePrefixes.ingot, aMaterial, (int) (aMaterialAmount / GregTech_API.MATERIAL_UNIT)));
            ...
            if (tStack2 != null && temp)
                for (byte i = 1; i <= tLimit && (aMaterialAmount * 9 * i) / GregTech_API.MATERIAL_UNIT <= 64; i++)
                    if (aMaterialAmount * 9 * i >= GregTech_API.MATERIAL_UNIT
                        && (aMaterialAmount * 9 * i) % GregTech_API.MATERIAL_UNIT == 0
                        && (tStack1 == null || (aMaterialAmount * i) % GregTech_API.MATERIAL_UNIT != 0)) {
                            if (aAllowAlloySmelter) GT_ModHandler.addSmeltingAndAlloySmeltingRecipe(
                                GT_Utility.copyAmount(i, aStack),
                                GT_Utility.copyAmount((aMaterialAmount * 9 * i) / GregTech_API.MATERIAL_UNIT, tStack2));
                            else GT_ModHandler.addSmeltingRecipe(
                                GT_Utility.copyAmount(i, aStack),
                                GT_Utility.copyAmount((aMaterialAmount * 9 * i) / GregTech_API.MATERIAL_UNIT, tStack2));
                            temp = false;
            ...
            if (tStack1 != null && temp)
                for (byte i = 1; i <= tLimit && (aMaterialAmount * i) / GregTech_API.MATERIAL_UNIT <= 64; i++)
                    if (aMaterialAmount * i >= GregTech_API.MATERIAL_UNIT
                        && (aMaterialAmount * i) % GregTech_API.MATERIAL_UNIT == 0) {
                            if (aAllowAlloySmelter) GT_ModHandler.addSmeltingAndAlloySmeltingRecipe(
                                GT_Utility.copyAmount(i, aStack),
                                GT_Utility.copyAmount((aMaterialAmount * i) / GregTech_API.MATERIAL_UNIT, tStack1));
                            else GT_ModHandler.addSmeltingRecipe(
                                GT_Utility.copyAmount(i, aStack),
                                GT_Utility.copyAmount((aMaterialAmount * i) / GregTech_API.MATERIAL_UNIT, tStack1));
                            temp = false;
    ...
    public static void registerBasicReverseMacerating(ItemStack aStack, Materials aMaterial, long aMaterialAmount) {
        ...
        if (aMaterialAmount % GregTech_API.MATERIAL_UNIT == 0) {
            GT_ModHandler.addPulverisationRecipe(
                GT_Utility.copyAmount(1, aStack),
                GT_OreDictUnificator
                    .get(OrePrefixes.dust, aMaterial, (int) (aMaterialAmount / GregTech_API.MATERIAL_UNIT)),
                null,
                0,
                true);
        } else if ((aMaterialAmount * 4) % GregTech_API.MATERIAL_UNIT == 0) {
            GT_ModHandler.addPulverisationRecipe(
                GT_Utility.copyAmount(1, aStack),
                GT_OreDictUnificator
                    .get(OrePrefixes.dustSmall, aMaterial, (int) ((aMaterialAmount * 4) / GregTech_API.MATERIAL_UNIT)),
                null,
                0,
                true);
        } else if ((aMaterialAmount * 9) >= GregTech_API.MATERIAL_UNIT) {
            if (!GT_ModHandler.addPulverisationRecipe(
                GT_Utility.copyAmount(1, aStack),
                GT_OreDictUnificator
                    .get(OrePrefixes.dustTiny, aMaterial, (int) ((aMaterialAmount * 9) / GregTech_API.MATERIAL_UNIT)),
                null,
                0,
                true)) {
                if (!GT_ModHandler.addPulverisationRecipe(
                    ...
                    GT_OreDictUnificator.get(
                        ...
                        aMaterial,
                        (int) ((aMaterialAmount * 4) / GregTech_API.MATERIAL_UNIT)),
                    null,
                    0,
                    true)) {
                    GT_ModHandler.addPulverisationRecipe(
     ...
     */
    public static void registerUsagesForMaterials(ItemStack aMat, ItemStack aOutput, String aPlate,
        boolean aBackSmelting, boolean aBackMacerating, boolean aRecipeReplacing) {
        ...
        if (aBackMacerating || aBackSmelting) {
            ...
            for (int i = 0; i < sShapes1.length; i++) {
                ...
                int tAmount1 = 0;
                for (ItemStack tMat : tRecipe) if (tMat == sMt1) tAmount1++;
                for (ItemStack tCrafted : GT_ModHandler.getRecipeOutputs(tRecipe)) {
                    if (aBackMacerating) GT_ModHandler
                        .addPulverisationRecipe(tCrafted, GT_Utility.copyAmount(tAmount1, aOutput), null, 0, false);
                    final ItemStack tmp = GT_Utility.copyAmount(tAmount1, tUnificated);
                    if (aBackSmelting) RecipeHandler
                        .executeOnFinish(() -> GT_ModHandler.addSmeltingAndAlloySmeltingRecipe(tCrafted, tmp));
                }

                for (Materials tMaterial : sRodMaterialList) {
                    ItemStack tMt2 = GT_OreDictUnificator.get(OrePrefixes.stick, tMaterial, 1),
                        tMt3 = GT_OreDictUnificator.get(OrePrefixes.dustSmall, tMaterial, 2);
                    if (tMt2 != null) {
                        ...
                        int tAmount2 = 0;
                        for (ItemStack tMat : tRecipe) if (tMat == sMt2) tAmount2++;
                        for (ItemStack tCrafted : GT_ModHandler.getVanillyToolRecipeOutputs(tRecipe)) {
                            if (aBackMacerating) GT_ModHandler.addPulverisationRecipe(
                                tCrafted,
                                GT_Utility.copyAmount(tAmount1, aOutput),
                                tAmount2 > 0 ? GT_Utility.mul(tAmount2, tMt3) : null,
                                100,
                                false);
                            if (aBackSmelting) GT_ModHandler.addSmeltingAndAlloySmeltingRecipe(
                                tCrafted,
                                GT_Utility.copyAmount(tAmount1, tUnificated));
                            if (aRecipeReplacing && aPlate != null && sShapesA[i] != null && sShapesA[i].length > 1) {
                                if (GregTech_API.sRecipeFile.get(
                                    GT_ConfigCategories.Recipes.recipereplacements,
                                    aAssotiation.replaceFirst(OrePrefixes.ingot.toString(), "") + "." + sShapesA[i][0],
                                    true)) {
                                        ...
                                        switch (sShapesA[i].length) {
                                            case 2:
                                                GT_ModHandler.addCraftingRecipe(
                                                    ...
                                                    false,
                                                    true,
                                                    new Object[] { sShapesA[i][1], s_P.charAt(0), aPlate, s_H.charAt(0),
                                                        GT_ToolDictNames.craftingToolHardHammer, s_R.charAt(0),
                                                        OrePrefixes.stick.toString() + tMaterial, s_I.charAt(0),
                                                        aAssotiation, s_F.charAt(0), GT_ToolDictNames.craftingToolFile,
                                                        s_W.charAt(0), GT_ToolDictNames.craftingToolWrench });
                                                break;
                                            case 3:
                                                GT_ModHandler.addCraftingRecipe(
                                                    tStack,
                                                    ...
                                                    new Object[] { sShapesA[i][1], sShapesA[i][2], s_P.charAt(0),
                                                        aPlate, s_H.charAt(0), GT_ToolDictNames.craftingToolHardHammer,
                                                        s_R.charAt(0), OrePrefixes.stick.toString() + tMaterial,
                                                        s_I.charAt(0), aAssotiation, s_F.charAt(0),
                                                        GT_ToolDictNames.craftingToolFile, s_W.charAt(0),
                                            ...
                                            default:
                                                GT_ModHandler.addCraftingRecipe(
                                                    ...
                                                    new Object[] { sShapesA[i][1], sShapesA[i][2], sShapesA[i][3],
                                                        s_P.charAt(0), aPlate, s_H.charAt(0),
                                                        GT_ToolDictNames.craftingToolHardHammer, s_R.charAt(0),
                                                        OrePrefixes.stick.toString() + tMaterial, s_I.charAt(0),
                                                        aAssotiation, s_F.charAt(0), GT_ToolDictNames.craftingToolFile,
                                                        s_W.charAt(0), GT_ToolDictNames.craftingToolWrench });
    ...
    public static void registerBlockForcibly(OrePrefixes aPrefix, Materials aMaterial, List<ItemStack> unifiedStacks) {
        ...
        for (ItemStack a : unifiedStacks) {
            ItemStack block = aMaterial.mSmallBlock ? GT_ModHandler.getRecipeOutput(a, a, null, a, a, null)
                : GT_ModHandler.getRecipeOutput(a, a, a, a, a, a, a, a, a);
            if (block != null) {
                ...
                if (!optional.isPresent()
                    && !GregTech_API.sUnification.get(GT_ConfigCategories.forceoredict, block, dictName)
                        .equals("false")) {
    ...
    private static final String[][] sShapesA = new String[][] { null, null,
    ...
    public static class DoubleMatcher implements IRecipeMatcher {
        ...
        public boolean matches(IRecipe recipe) {
            return recipe.getRecipeOutput() != null && output.isItemEqual(recipe.getRecipeOutput())
                && matcher.matches(recipe);
        }
