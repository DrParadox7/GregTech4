...
public class GT_OreDictUnificator {
     ...
     * Useful if you have things like the Industrial Diamond, which is better than regular Diamond, but also placeable
     * in absolutely all Diamond Recipes.
     */
    ...
    public static void set(Object aName, ItemStack aStack, boolean aOverwrite, boolean aAlreadyRegistered) {
        if (GT_Utility.isStringInvalid(aName) || GT_Utility.isStackInvalid(aStack)
            || Items.feather.getDamage(aStack) == GregTech_API.ITEM_WILDCARD_DAMAGE) return;
        isAddingOre++;
    ...
    public static ItemStack getFirstOre(Object aName, long aAmount) {
        ...
        if (GT_Utility.isStackValid(sName2OreMap.get(aName.toString())))
            return GT_Utility.copyAmount(aAmount, sName2OreMap.get(aName.toString()));
    ...
    public static ItemStack get(Object aName, ItemStack aReplacement, long aAmount, boolean aMentionPossibleTypos,
        ...
        if (!sName2OreMap.containsKey(aName.toString()) && aMentionPossibleTypos)
            GT_Log.log.error("Unknown Key for Unification, Typo? " + aName);
        return GT_Utility
            .copyAmount(aAmount, sName2OreMap.get(aName.toString()), getFirstOre(aName, aAmount), aReplacement);
    }

    public static ItemStack[] getStackArray(boolean aUseBlackList, Object... aStacks) {
        ItemStack[] rStacks = new ItemStack[aStacks.length];
        for (int i = 0; i < aStacks.length; i++)
            rStacks[i] = get(aUseBlackList, GT_Utility.copy((ItemStack) aStacks[i]));
        return rStacks;
    }
    ...
    public static boolean isItemStackDye(ItemStack aStack) {
        ...
        for (Dyes tDye : Dyes.values())
            if (tDye != Dyes._NULL && isItemStackInstanceOf(aStack, tDye.toString())) return true;
        return false;
    }
