...
public class GT_ModHandler {
    ...
    public static ItemStack getIC2Item(String aItem, long aAmount, ItemStack aReplacement) {
        ...
        if (!sIC2ItemMap.containsKey(aItem)) try {
            sIC2ItemMap.put(aItem, ic2.api.item.IC2Items.getItem(aItem));
    ...
    public static ItemStack getRCItem(String aItem, long aAmount, ItemStack aReplacement) {
        if (GT_Utility.isStringInvalid(aItem) || !GregTech_API.sPreloadStarted) return null;
        return GT_Utility
            .copyAmount(aAmount, GameRegistry.findItemStack("Railcraft", aItem, (int) aAmount), aReplacement);
    }
    ...
    public static ItemStack getTEItem(String aItem, long aAmount, ItemStack aReplacement) {
        if (GT_Utility.isStringInvalid(aItem) || !GregTech_API.sPreloadStarted) return null;
        return GT_Utility
            .copyAmount(aAmount, GameRegistry.findItemStack("ThermalExpansion", aItem, (int) aAmount), aReplacement);
    }
    ...
    public static ItemStack getTFItem(String aItem, long aAmount, ItemStack aReplacement) {
        if (GT_Utility.isStringInvalid(aItem) || !GregTech_API.sPreloadStarted) return null;
        return GT_Utility
            .copyAmount(aAmount, GameRegistry.findItemStack("ThermalFoundation", aItem, (int) aAmount), aReplacement);
    }
    ...
    /**
     * Gets an Item from Thermal Expansion (Did I spell it right this time?), but the Damage Value can be specified, and
    ...
    public static void addBoilerFuel(FluidStack aLiquid, int aValue) {
        ...
        if (!GregTech_API.sRecipeFile.get(
            GT_ConfigCategories.Fuels.boilerfuels,
            aLiquid.getFluid()
                .getUnlocalizedName(),
            true)) return;
        try {
    ...
    public static boolean addValuableOre(ItemStack aStack, int aValue) {
        ...
        try {
            ...
            Object value = Class.forName("ic2.api.recipe.RecipeInputItemStack")
                .getConstructor(ItemStack.class, int.class)
                .newInstance(aStack, aStack.stackSize);
            Class.forName("ic2.core.IC2")
                .getMethod("addValuableOre", type, int.class)
                .invoke(null, value, aValue); // TODO mb need RecipeInputOreDict
        } catch (Throwable e) {/* Do nothing */}
        return true;
    }
    ...
    public static boolean addScrapboxDrop(float aChance, ItemStack aOutput) {
        if (GT_Mod.sCraftingUnification) aOutput = GT_OreDictUnificator.get(true, aOutput);

        if (aOutput == null || aChance <= 0) return false;
        aOutput.stackSize = 1;
        if (GT_Config.system && !GT_Utility.areStacksEqual(aOutput, new ItemStack(Items.wooden_hoe, 1, 0)))
            return false;
    ...
    public static boolean addToRecyclerBlackList(ItemStack aRecycledStack) {
        ...
        try {
            ...
            Method add = Class.forName("ic2.api.recipe.IListRecipeManager")
                .getMethod("add", iRecipeInput);
            Object o = recipeInputStack.getConstructor(ItemStack.class)
                .newInstance(aRecycledStack.copy());
    ...
    public static boolean addSmeltingAndAlloySmeltingRecipe(ItemStack aInput, ItemStack aOutput) {
        ...
        if (RecipeMaps.ALLOY_SMELTING.factory()
            .EUt(3)
            .duration(160)
            .input(aInput)
            .output(aOutput)
            .buildAndRegister()) temp = true;
    ...
    public static boolean addSqueezerRecipe(ItemStack aInput, FluidStack aOutput, int aTime) {
        ...
        try {
            forestry.api.recipes.RecipeManagers.squeezerManager
                .addRecipe(aTime > 0 ? aTime : 100, new ItemStack[] { GT_Utility.copy(aInput) }, aOutput);
        } catch (Throwable e) {
            return false;
        }
        return true;
    ...
    public static boolean addLiquidTransposerRecipe(ItemStack aEmptyContainer, FluidStack aLiquid,
        ItemStack aFullContainer, int aRF) {
        if (GT_Mod.sCraftingUnification) aFullContainer = GT_OreDictUnificator.get(true, aFullContainer);
        ...
        try {
            cofh.thermalexpansion.util.crafting.TransposerManager
                .addFillRecipe(aRF, aEmptyContainer, aFullContainer, aLiquid, true, true);
        } catch (Throwable e) {/* Do nothing */}
        return true;
    }
    ...
    public static boolean addLiquidTransposerFillRecipe(ItemStack aEmptyContainer, FluidStack aLiquid,
        ...
        try {
            cofh.thermalexpansion.util.crafting.TransposerManager
                .addFillRecipe(aRF, aEmptyContainer, aFullContainer, aLiquid, false, true);
        } catch (Throwable e) {/* Do nothing */}
        return true;
    }
    ...
    public static boolean addLiquidTransposerEmptyRecipe(ItemStack aFullContainer, FluidStack aLiquid,
        ItemStack aEmptyContainer, int aRF) {
        if (GT_Mod.sCraftingUnification) aEmptyContainer = GT_OreDictUnificator.get(true, aEmptyContainer);
        ...
        try {
            cofh.thermalexpansion.util.crafting.TransposerManager
                .addExtractionRecipe(aRF, aFullContainer, aEmptyContainer, aLiquid, 100, false, true);
        } catch (Throwable e) {/* Do nothing */}
        return true;
    }

    /** @param secondaryChance - 1-100% */
    public static boolean addTEPulveriserRecipe(ItemStack input, ItemStack primaryOutput, ItemStack secondaryOutput,
        int secondaryChance, int RF) {
        ...
        try {
            cofh.thermalexpansion.util.crafting.PulverizerManager
                .addRecipe(RF, input, primaryOutput, secondaryOutput, secondaryChance, true);
        } catch (Throwable e) {/* Do nothing */}
        return true;
    }
    ...
    public static boolean addPulverisationRecipe(ItemStack aInput, ItemStack aOutput1, ItemStack aOutput2, int aChance,
        boolean aOverwrite) {
        if (GT_Mod.sCraftingUnification) {
            ...
            if (!OrePrefixes.log.contains(aInput)) {
                if (Materials.Wood.contains(aOutput1)) {
                    if (GregTech_API.sRecipeFile.get(GT_ConfigCategories.Machines.pulverization, aInput, true)) {
                        ...
                        else addSawmillRecipe(
                            GT_Utility.copy(aInput),
                            1200,
                            aChance <= 0 ? 10 : aChance,
                            ...
                            if (Block.getBlockFromItem(aInput.getItem()) != Blocks.obsidian) {
                                mods.railcraft.api.crafting.IRockCrusherRecipe tRecipe = mods.railcraft.api.crafting.RailcraftCraftingManager.rockCrusher
                                    .createNewRecipe(
                                        ...
                                        aInput.getItemDamage() != GregTech_API.ITEM_WILDCARD_DAMAGE,
                                        false);
                                tRecipe.addOutput(GT_Utility.copy(aOutput1), 1.0F / aInput.stackSize);
                                tRecipe.addOutput(
                                    GT_Utility.copy(aOutput2),
                                    (0.01F * (aChance <= 0 ? 10 : aChance)) / aInput.stackSize);
                            }
                        } catch (Throwable e) {/* Do nothing */}
                    }
                    if (GregTech_API.sRecipeFile.get(GT_ConfigCategories.Machines.pulverization, aInput, true)) {
                        addTEPulveriserRecipe(
                            GT_Utility.copy(aInput),
                            GT_Utility.copy(aOutput1),
                            GT_Utility.copy(aOutput2),
                            aChance <= 0 ? 10 : aChance,
     ...
     */
    public static boolean addPulverisationRecipe(OreDictEntry aInput, int amount, ItemStack aOutput1,
        ItemStack aOutput2, int aChance) {
        ...
        if (GregTech_API.sRecipeFile.get(GT_ConfigCategories.Machines.maceration, aInput.oreDictName, true)) {
            GT_Utility.addSimpleIC2MachineRecipe(
                RecipeEntry.fromStacks(amount, aInput.ores, Match.STRICT),
                getMaceratorRecipeList(),
                null,
                aOutput1);
        }
        ...
        if (pref != OrePrefixes.log) {
            if (mat == Materials.Wood) {
                if (GregTech_API.sRecipeFile
                    .get(GT_ConfigCategories.Machines.pulverization, aInput.oreDictName, true)) {
                        ...
                        if (Block.getBlockFromItem(stack.getItem()) != Blocks.obsidian) {
                            mods.railcraft.api.crafting.IRockCrusherRecipe tRecipe = mods.railcraft.api.crafting.RailcraftCraftingManager.rockCrusher
                                .createNewRecipe(
                                    ...
                                    stack.getItemDamage() != GregTech_API.ITEM_WILDCARD_DAMAGE,
                                    false);
                            tRecipe.addOutput(GT_Utility.copy(aOutput1), 1.0F / stack.stackSize);
                            tRecipe.addOutput(
                                GT_Utility.copy(aOutput2),
                                (0.01F * (aChance <= 0 ? 10 : aChance)) / stack.stackSize);
                        }
                    }
                }
                ...
                if (GregTech_API.sRecipeFile
                    .get(GT_ConfigCategories.Machines.pulverization, aInput.oreDictName, true)) {
                    addTEPulveriserRecipe(
                        GT_Utility.copy(aInput.ores.get(0)),
                        GT_Utility.copy(aOutput1),
                        GT_Utility.copy(aOutput2),
                        aChance <= 0 ? 10 : aChance,
                        2400);
    ...
    public static boolean addSawmillRecipe(ItemStack aInput1, int aRF, int aChance, ItemStack... outputs) {
        if (aInput1 == null || outputs[0] == null
            || !GregTech_API.sRecipeFile.get(GT_ConfigCategories.Machines.sawmill, aInput1, true)) return false;
        boolean result = false;
        try {
            result = cofh.thermalexpansion.util.crafting.SawmillManager
                .addRecipe(aRF, aInput1, outputs[0], outputs.length > 1 ? outputs[1] : null, aChance, true);
        } catch (Throwable e) {/* Do nothing */}
        return result;
    }
    ...
    public static boolean addSawmillRecipe(OreDictEntry entry, int inAmount, int aRF, int aChance,
        ItemStack... outputs) {
        if (entry == null || entry.ores == null
            || entry.ores.size() == 0
            || !GregTech_API.sRecipeFile.get(GT_ConfigCategories.Machines.sawmill, entry.oreDictName, true))
            return false;
        boolean result = false;
        try {
            // Accroding on TE code, it saves all recipe OreDict format, so i can just create one recipe
            result = cofh.thermalexpansion.util.crafting.SawmillManager
                .addRecipe(aRF, entry.ores.get(0), outputs[0], outputs.length > 1 ? outputs[1] : null, aChance, true);
        } catch (Throwable e) {/* Do nothing */}
        return result;
    }
    ...
    public static boolean addInductionSmelterRecipe(ItemStack aInput1, ItemStack aInput2, ItemStack aOutput1,
        ...
        if (!GregTech_API.sRecipeFile
            .get(GT_ConfigCategories.Machines.inductionsmelter, aInput2 == null ? aInput1 : aOutput1, true))
            return false;
        try {
            cofh.thermalexpansion.util.crafting.SmelterManager.addRecipe(
                aEnergy,
    ...
    public static boolean removeInductionSmelterRecipe(ItemStack output) {
        ...
        try {
            for (ISmelterRecipe recipe : cofh.thermalexpansion.util.crafting.SmelterManager.getRecipeList()) {
                if (recipe.getPrimaryOutput()
                    .isItemEqual(output)) {
                    cofh.thermalexpansion.util.crafting.SmelterManager
                        .removeRecipe(recipe.getPrimaryInput(), recipe.getSecondaryInput());
                    break;
                }
            }
        } catch (Throwable e) {/* Do nothing */}
    ...
    public static boolean addAlloySmelterRecipe(ItemStack aInput1, ItemStack aInput2, ItemStack aOutput1, int aDuration,
        ...
        if (GT_Mod.sCraftingUnification) aOutput1 = GT_OreDictUnificator.get(true, aOutput1);
        boolean temp = false;
        if (RecipeMaps.ALLOY_SMELTING.factory()
            .EUt(aEUt)
            .duration(aDuration)
            .inputs(aInput1, aInput2)
            .output(aOutput1)
            .buildAndRegister()) temp = true;
    ...
    public static boolean addCompressionRecipe(OreDictEntry aInput, int amount, ItemStack aOutput) {
        if (GT_Mod.sCraftingUnification) aOutput = GT_OreDictUnificator.get(true, aOutput);
        if (aInput == null || aOutput == null) return false;
        if (!GregTech_API.sRecipeFile.get(GT_ConfigCategories.Machines.compression, aInput.oreDictName, true))
            return false;
        GT_Utility.addSimpleIC2MachineRecipe(
            RecipeEntry.fromStacks(amount, aInput.ores, Match.STRICT),
    ...
    public static boolean addRollingMachineRecipe(ItemStack aResult, Object[] aRecipe) {
        if (GT_Mod.sCraftingUnification) aResult = GT_OreDictUnificator.get(true, aResult);
    ...
    private static final List<IRecipe> sAllRecipeList = Collections.synchronizedList(new ArrayList<IRecipe>(2000)),
        sBufferRecipeList = new ArrayList<IRecipe>(300),
        sSingleNonBlockDamagableRecipeList = new ArrayList<IRecipe>(300);
     ...
     * Regular Crafting Recipes. Deletes conflicting Recipes too.
     */
    public static boolean addCraftingRecipe(ItemStack aResult, boolean aUseIC2Handler, boolean aMirrored,
        Object... aRecipe) {
     ...
     */
    public static boolean addCraftingRecipe(ItemStack aResult, boolean aUseIC2Handler, boolean aMirrored,
        boolean aBuffered, Object... aRecipe) {
     ...
     */
    public static boolean addCraftingRecipe(ItemStack aResult, boolean aUseIC2Handler, boolean aMirrored,
        boolean aBuffered, boolean aKeepNBT, Object... aRecipe) {
        if (GT_Mod.sCraftingUnification) aResult = GT_OreDictUnificator.get(true, aResult);

        if (aResult != null && aResult.getItemDamage() == GregTech_API.ITEM_WILDCARD_DAMAGE)
            ...
            while (true) {
                ...
                for (; idx < aRecipe.length; idx += 2) {
                    if (aRecipe[idx] == null || aRecipe[idx + 1] == null) {
                        if (GregTech_API.DEBUG_MODE) GT_Log.log.warn(
                ...
                for (char chr : shape.toCharArray()) {
                    ...
                    if (tRecipe[x] != null) if (tRecipe[x].getItemDamage() == GregTech_API.ITEM_WILDCARD_DAMAGE)
        ...
        if (aResult.getItemDamage() == GregTech_API.ITEM_WILDCARD_DAMAGE || aResult.getItemDamage() < 0)
            throw new IllegalArgumentException();

        if (aKeepNBT) {
            GameRegistry
                .addRecipe(new GT_Shaped_NBT_Keeping_Recipe(GT_Utility.copy(aResult), aRecipe).setMirrored(aMirrored));
            return true;
        }
        // if (aUseIC2Handler) {
        // try {
        ...
        else GameRegistry.addRecipe(new GT_Shaped_Recipe(GT_Utility.copy(aResult), aRecipe).setMirrored(aMirrored));
        return true;
    }
     ...
     * Shapeless Crafting Recipes. Deletes conflicting Recipes too.
     */
    public static boolean addShapelessCraftingRecipe(ItemStack aResult, boolean aUseIC2Handler, boolean aBuffered,
        Object... aRecipe) {
     ...
     */
    public static boolean addShapelessCraftingRecipe(ItemStack aResult, boolean aUseIC2Handler, boolean aBuffered,
        boolean aKeepNBT, Object... aRecipe) {
        if (GT_Mod.sCraftingUnification) aResult = GT_OreDictUnificator.get(true, aResult);
        ...
        else GameRegistry.addRecipe(new GT_Shapeless_Recipe(GT_Utility.copy(aResult), aRecipe));
        return true;
    }
    ...
    public static ItemStack removeRecipe(ItemStack... aRecipe) {
        ...
        for (int i = 0; i < tList.size(); i++) {
            try {
                if (tList.get(i)
                    .matches(aCrafting, GregTech_API.sDummyWorld)) {
                    rReturn = tList.get(i)
            ...
            try {
                if ((recipe = sBufferRecipeList.get(i)) != null) {
                    if (recipe.matches(aCrafting, GregTech_API.sDummyWorld)) {
                        rReturn = sBufferRecipeList.get(i)
                            .getCraftingResult(aCrafting);
    ...
    public static ArrayList<ItemStack> getVanillyToolRecipeOutputs(ItemStack... aRecipe) {
        ...
        if (sSingleNonBlockDamagableRecipeList.isEmpty()) {
            for (IRecipe tRecipe : (ArrayList<IRecipe>) CraftingManager.getInstance()
                ...
                if (tStack != null && tStack.getItem() != null
                    ...
                    if (!(tRecipe instanceof ShapelessRecipes || tRecipe instanceof ShapelessOreRecipe)) {
                        if (tRecipe instanceof ShapedOreRecipe) {
                            ...
                            for (Object tObject : ((ShapedOreRecipe) tRecipe).getInput()) {
                                if (tObject != null && tObject instanceof ItemStack
            ...
            GT_Log.log.info(
                "Created a List of Tool Recipes containing " + sSingleNonBlockDamagableRecipeList.size()
                    + " Recipes for recycling."
                    + (sSingleNonBlockDamagableRecipeList.size() > 2048
    ...
    public static boolean addTileToEnet(World aWorld, TileEntity aTileEntity) {
        try {
            ...
            if (aTileEntity instanceof ic2.api.energy.tile.IEnergyTile && temp != null
                && temp instanceof Boolean
                && !((Boolean) temp)) {
                Event tEvent = (Event) Class.forName("ic2.api.energy.event.EnergyTileLoadEvent")
                    .getConstructors()[0].newInstance((ic2.api.energy.tile.IEnergyTile) aTileEntity);
    ...
    public static boolean removeTileFromEnet(World aWorld, TileEntity aTileEntity) {
        try {
            ...
            if (aTileEntity instanceof ic2.api.energy.tile.IEnergyTile && temp != null
                && temp instanceof Boolean
                && (Boolean) temp) {
                Event tEvent = (Event) Class.forName("ic2.api.energy.event.EnergyTileUnloadEvent")
                    .getConstructors()[0].newInstance((ic2.api.energy.tile.IEnergyTile) aTileEntity);
                MinecraftForge.EVENT_BUS.post(tEvent);
                return true;
            }
    ...
    /**
     * Emits Energy to the E-net. Uses the internals of GT TileEntities for emitting Energy if it is not connected to
     * the IC-2-Enet.
     * 
    ...
    public static int emitEnergyToEnet(int aVoltage, World aWorld, TileEntity aTileEntity) {
        // try {
        // Object temp = GT_Utility.callPublicMethod(aTileEntity, "isAddedToEnergyNet");
        // if (aTileEntity instanceof ic2.api.energy.tile.IEnergySource && temp != null && temp instanceof Boolean &&
        // (Boolean)temp) {
        // return aVoltage;
        // }
        // } catch(Throwable e) {/*Do nothing*/}
        ...
        // if (((IBasicEnergyContainer)tTileEntity).getStoredEU() <
        // ((IBasicEnergyContainer)tTileEntity).getEUCapacity()) {
     ...
     */
    public static double chargeElectricItem(ItemStack aStack, double aCharge, int aTier, boolean aIgnoreLimit,
        boolean aSimulate) {
        try {
     ...
     */
    public static double dischargeElectricItem(ItemStack aStack, double aCharge, int aTier, boolean aIgnoreLimit,
        boolean externally, boolean aSimulate, boolean aIgnoreDischargability) {
        try {
            if (isElectricItem(aStack)) {
                if (aIgnoreDischargability
                    || ((ic2.api.item.IElectricItem) aStack.getItem()).canProvideEnergy(aStack)) {
                    return ic2.api.item.ElectricItem.manager
                        .discharge(aStack, aCharge, aTier, aIgnoreLimit, externally, aSimulate);
                }
            }
        } catch (Throwable e) {/* Do nothing */}
        return 0;
        ...
        if (GT_ModHandler.isElectricItem(aStack)) {
            if (canUseElectricItem(aStack, aDecharge)) {
                if (aPlayer != null && aPlayer instanceof EntityPlayer) {
                    ...
                    return GT_ModHandler
                        .dischargeElectricItem(aStack, aDecharge, Integer.MAX_VALUE, true, false, false, true)
                        >= aDecharge;
                }
            }
            ...
            .isDamageable()) {
                ...
                if (aStack.getItemDamage() >= aStack.getMaxDamage()) {
                    ...
                    if (aStack.getItem()
                        .hasContainerItem() && aStack.getItem() instanceof GT_Tool_Item) {
        ...
        if (GT_Utility.isItemStackInList(aStack, GregTech_API.sSolderingToolList, true)) {
            if (aPlayer instanceof EntityPlayer) {
                ...
                for (int i = 0; i < tPlayer.inventory.mainInventory.length; i++) {
                    if (GT_Utility.isItemStackInList(
                        tPlayer.inventory.mainInventory[i],
                        GregTech_API.sSolderingMetalList,
                        true)) {
                        if (damageOrDechargeItem(aStack, 1, 1000, tPlayer)) {
                            if (tPlayer.inventory.mainInventory[i].getItemDamage()
                                >= tPlayer.inventory.mainInventory[i].getMaxDamage())
    ...
    public static int getCapsuleCellContainerCount(ItemStack aStack) {
        ...
        if (GT_Utility.areStacksEqual(GT_Utility.getContainerForFilledItem(aStack), GT_Items.Cell_Empty.get(1))
            || OrePrefixes.cell.contains(aStack)
            || OrePrefixes.cellPlasma.contains(aStack)) return 1;
        if (GT_Utility.areStacksEqual(aStack, getIC2Item("hydratedCoalCell", 1, GregTech_API.ITEM_WILDCARD_DAMAGE)))
            return 1;
        return 0;
    }
}
