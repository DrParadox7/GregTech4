...
public class GT_BaseCrop extends CropCard {
    ...
    public GT_BaseCrop(int aID, String aCropName, String aDiscoveredBy, ItemStack aDrop, ItemStack[] aSpecialDrops,
    ...
    public ItemStack getGain(ICropTile aCrop) {
        int tDrop = 0;
        if (mSpecialDrops != null && (tDrop = new Random().nextInt(mSpecialDrops.length + 4)) < mSpecialDrops.length
            && mSpecialDrops[tDrop] != null) {
            return GT_Utility.copy(mSpecialDrops[tDrop]);
        }
        if (mDrop.getItem()
            .getContainerItem(mDrop) == null) return GT_Utility.copy(mDrop);
        return null;
    }
    ...
    public boolean rightclick(ICropTile aCrop, EntityPlayer aPlayer) {
        ...
        ItemStack tContainerItem = mDrop.getItem()
            .getContainerItem(mDrop);
        if (tContainerItem == null) {
            return aCrop.harvest(aPlayer == null ? false : aPlayer instanceof EntityPlayerMP);
        }
        if (aPlayer != null && GT_Utility.areStacksEqual(tContainerItem, aPlayer.getCurrentEquippedItem())
            && aPlayer.getCurrentEquippedItem().stackSize >= mDrop.stackSize
            && aPlayer.inventory.addItemStackToInventory(GT_Utility.copy(mDrop))) {
            aPlayer.getCurrentEquippedItem().stackSize -= mDrop.stackSize;
