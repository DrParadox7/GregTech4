 ...
 * For faster inslot update in GT_Container, using a List of this slots. And added to usual slot list to activate
 * onSlotClicked
 * 
 * @author TheDarkDnKTv
 *
...
public class GT_FluidSlot extends Slot {
    ...
    public GT_FluidSlot(IGregTechTileEntity te, int slotIndex, int x, int y, int fluidIdx, boolean canDrain,
        boolean canFill) {
    ...
    public void draw(int uiLeft, int uiTop, boolean isMouseOver) {
        ...
        if (GT_Utility.isFluidStackValid(fluid)) {
            IIcon text = fluid.getFluid()
                .getIcon(fluid);
            if (text != null) {
                GTRenderHelper.bindTexture(TextureMap.locationBlocksTexture);
                GTRenderHelper
                    .drawQuad(posX, posY, 50, 16, 16, text.getMinU(), text.getMaxU(), text.getMinV(), text.getMaxV());
            }

            // Amount render
            if (renderAmount) {
                int amount = fluid.amount / 1000;
                if (amount > 0) GTRenderHelper.drawStackAmount(posX, posY, 0xFFFFFF, Integer.toString(amount));
            }
        }

        if (isMouseOver) GTRenderHelper.drawSlotDim(posX, posY, 51, 16, 16);
    ...
    public boolean onClick(int mouseClick, int shiftHold, EntityPlayer player) {
        ...
        if (GT_Utility.isStackValid(held)) {
            ...
            } else if (mouseClick == 0 && canFill && GT_Utility.isFluidStackValid(fluidInv)) { // Epmty (Left click to
                                                                                               // drain liquid from
                                                                                               // container)
                if ((amount = this.tryFill(inv, held, fluidInv.copy())) > 0) {
                    fluidInv.amount -= amount;
                    if (fluidInv.amount <= 0) setFluid(null);
    ...
    protected int tryFill(InventoryPlayer inv, ItemStack empty, FluidStack fluid) {
        ...
        if (empty.getItem() instanceof IFluidContainerItem) {
            if (empty.stackSize > 1) {
                ...
                if (amount > 0) {
                    if (!this.exchangeStacks(inv, filled)) return 0;
                }
            } else {
    ...
    protected int getSpace() {
        ...
        } else availableSpace = this.getSlotStackLimit() * 1000;

        FluidStack fluid = getFluid();
        if (GT_Utility.isFluidStackValid(fluid)) availableSpace -= fluid.amount;
