...
public class GT_GUIContainer extends GuiContainer {
                        ...
                        if (GT_Utility.isFluidStackValid(fluid)) {
                            ...
                            tooltipData.add(
                                EnumChatFormatting.GRAY + I18n.format(
                                    "metatileentity.fluid.amount",
                                    GT_Utility.parseNumberToString(fluid.amount)));
                        } else {
                            tooltipData.add(I18n.format("metatileentity.fluid.empty"));
                        }

                        if (slot.canFill) tooltipData.add(
                            EnumChatFormatting.GRAY.toString() + EnumChatFormatting.ITALIC.toString()
                                + I18n.format("metatileentity.fluid.fill"));
                        if (slot.canDrain) tooltipData.add(
                            EnumChatFormatting.GRAY.toString() + EnumChatFormatting.ITALIC.toString()
                                + I18n.format("metatileentity.fluid.drain"));
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
        if (inventorySlots instanceof GT_Container) {
            for (GT_FluidSlot slot : ((GT_Container) inventorySlots).fluidSlots) slot.draw(
                this.guiLeft,
                this.guiTop,
                this.func_146978_c(slot.xDisplayPosition, slot.yDisplayPosition, 16, 16, par2, par3));
        }
