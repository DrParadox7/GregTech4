...
public class GT_GUIContainer_MultiMachine extends GT_GUIContainerMetaTile_Machine {
    ...
    public GT_GUIContainer_MultiMachine(InventoryPlayer aInventoryPlayer, IGregTechTileEntity aTileEntity) {
        super(
            ...
            GregTech_API.GUI_PATH + "MultiblockDisplay.png");
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        if (mContainer != null) {
            ...
            if ((errorCode & 1) != 0)
                fontRenderer.drawString(I18n.format("metatileentity.status.malfunction.0"), 10, 16, 16448255);
            if ((errorCode & 2) != 0)
                fontRenderer.drawString(I18n.format("metatileentity.status.malfunction.1"), 10, 24, 16448255);
            if ((errorCode & 4) != 0)
                fontRenderer.drawString(I18n.format("metatileentity.status.malfunction.2"), 10, 32, 16448255);
            if ((errorCode & 8) != 0)
                fontRenderer.drawString(I18n.format("metatileentity.status.malfunction.3"), 10, 40, 16448255);
            if ((errorCode & 16) != 0)
                fontRenderer.drawString(I18n.format("metatileentity.status.malfunction.4"), 10, 48, 16448255);
            if ((errorCode & 32) != 0)
                fontRenderer.drawString(I18n.format("metatileentity.status.malfunction.5"), 10, 56, 16448255);
            if ((errorCode & 64) != 0)
                fontRenderer.drawString(I18n.format("metatileentity.status.malfunction.6"), 10, 64, 16448255);
            ...
            if (errorCode == 0) {
                if (m.mActive.get()) {
                    ...
                    fontRenderer
                        .drawSplitString(I18n.format("metatileentity.status.multiblock.idle"), 10, 16, 150, 16448255);
                }
            }
        }
    }
