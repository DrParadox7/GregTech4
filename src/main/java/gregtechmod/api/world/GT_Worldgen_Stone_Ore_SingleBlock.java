...
public class GT_Worldgen_Stone_Ore_SingleBlock extends GT_Worldgen_Ore {
    ...
    public GT_Worldgen_Stone_Ore_SingleBlock(String aName, boolean aDefault, Block aBlock, int aBlockMeta,
        int aDimensionType, int aAmount, int aSize, int aProbability, int aMinY, int aMaxY,
        Collection<String> aBiomeList, boolean aAllowToGenerateinVoid, Block[] aOres, int[] aOreMetas,
        int[] aOreChances) {
        super(
            aName,
            aDefault,
    ...
    public boolean executeWorldgen(World aWorld, Random aRandom, String aBiome, int aDimensionType, int aChunkX,
        ...
        if (aDimensionType == mDimensionType && (mBiomeList.isEmpty() || mBiomeList.contains(aBiome))
                                ...
                                for (int var41 = var33; var41 <= var36; ++var41) {
                                    ...
                                    if (var39 * var39 + var42 * var42 < 1.0D) {
                                        for (int var44 = var34; var44 <= var37; ++var44) {
                                            ...
                                            for (byte x = 0; x < mOreChances.length; x++)
                                                if (aRandom.nextInt(mOreChances[x]) == 0) {
                                                    tBlock = mOres[x];
                                                    tBlockMeta = mOreMetas[x];
                                                    break;
                                                }
                                            if (var39 * var39 + var42 * var42 + var45 * var45 < 1.0D
                                                && ((mAllowToGenerateinVoid && aWorld.isAirBlock(var38, var41, var44))
                                                    || (block != null && (block
