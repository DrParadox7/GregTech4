...
/**
 * This File contains the functions used to get Items and add Recipes. Please do not include this File in your
 * Moddownload as it maybe ruins compatiblity, like with the IC2-API
 ...
 * I change things quite often so please don't include any File inside your Mod, unless it's an Interface. Interfaces
 * are not going to be changed during a MC-Version
...
public class GregTech_API {
    ...
    public static List<Runnable> sBeforeGTPreload = new ArrayList<Runnable>(),
        ...
        sAfterGTServerstop = new ArrayList<Runnable>(), sGTCoverload = new ArrayList<Runnable>(),
    ...
    /** Configured Booleans */
    public static boolean DEBUG_MODE = false, SECONDARY_DEBUG_MODE = false, IC_ENERGY_COMPATIBILITY = true,
        RF_ENERGY_COMPATIBILITY = true, OREDICT_DEBUG_MODE = false;

    /** The Configuration Objects */
    public static GT_Config sMachineFile = null, sWorldgenFile = null, sMaterialProperties = null, sUnification = null,
        sSpecialFile = null, sRecipeFile = null, sIDFile = null;
    ...
    /** My Creative Tab */
    public static final CreativeTabs TAB_GREGTECH = new GT_CreativeTab("gregtech"),
        TAB_GREGTECH_MATERIALS = new GT_CreativeTab("gregtech_materials");
     ...
     * 4096 - 8191 are reserved for mostly decorative GregTech Blocks.
     * 8192 - 32766 are currently free.
     * 
    ...
    public static final String GENERIC_CHANNEL = "gregtech", SOUND_PACKET_CHANNEL = "GTSound",
        TILEENTITY_PACKET_CHANNEL = "GTTile", IC2_MOD_ID = "ic2", IC2_TEXTURE_PATH = IC2_MOD_ID + ":",
        MOD_ID = "gregtech_addon", TEXTURE_FOLDER = "textures/", TEXTURE_PATH_ITEM = MOD_ID + ":",
        TEXTURE_PATH_BLOCK = MOD_ID + ":", GUI_PATH = MOD_ID + ":" + TEXTURE_FOLDER + "gui/";
    ...
    public static boolean sDoShowAllItemsInCreative = false, sColoredGUI = true, sConstantEnergy = true,
    ...
    public static final List<ItemStackKey> sToolList = new ArrayList<ItemStackKey>(),
        sCrowbarList = new ArrayList<ItemStackKey>(), sScrewdriverList = new ArrayList<ItemStackKey>(),
        sWrenchList = new ArrayList<ItemStackKey>(), sSoftHammerList = new ArrayList<ItemStackKey>(),
        sHardHammerList = new ArrayList<ItemStackKey>(), sSolderingToolList = new ArrayList<ItemStackKey>(),
     ...
     */
    public static final List<String> sContinentalWaterList = new ArrayList<String>(),
        sTetrahedriteList = new ArrayList<String>(), sCassiteriteList = new ArrayList<String>(),
        sNickelList = new ArrayList<String>(), sRubyList = new ArrayList<String>(),
        sSapphireList = new ArrayList<String>(), sBauxiteList = new ArrayList<String>();
    ...
    public static final int VOLTAGE_ULTRALOW = 8, VOLTAGE_LOW = 32, VOLTAGE_MEDIUM = 128, VOLTAGE_HIGH = 512,
        VOLTAGE_EXTREME = 2048, VOLTAGE_INSANE = 8192, VOLTAGE_LUDICROUS = 32768, VOLTAGE_ZPM = 131072,
        VOLTAGE_ULTIMATE = 1000000;
    public static final int[] VOLTAGES = new int[] { VOLTAGE_ULTRALOW, VOLTAGE_LOW, VOLTAGE_MEDIUM, VOLTAGE_HIGH,
        VOLTAGE_EXTREME, VOLTAGE_INSANE, VOLTAGE_LUDICROUS, VOLTAGE_ZPM, VOLTAGE_ULTIMATE };
    ...
    public static ItemStack getGregTechBlock(int aIndex, int aAmount, int aMeta) {
        if (aIndex < GregTech_API.sBlockList.length && aIndex >= 0) {
            ...
            GT_Log.log.error(
                "GregTech_API ERROR: This Block Index " + aIndex
     ...
     * You could also check if the targetItem implements ICapsuleCellContainer, to not be relent on the Code in my
     * Addon, but this also outputs values for IC2-Items
     * 
    ...
    public static ItemStack getGregTechItem(int aIndex, int aAmount, int aMeta) {
        if (aIndex < GregTech_API.sItemList.length && aIndex >= 0) {
            ...
            GT_Log.log.error(
                "GregTech_API ERROR: This Item Index " + aIndex
                    + " wasn't initialized, you either called it before the Init-Phase (Only Post-Init or later will work), or this Item doesn't exist now");
    ...
    public static ItemStack getUnificatedOreDictStack(ItemStack aOreStack) {
        if (!GregTech_API.sPreloadFinished) GT_Log.log.error(
    ...
    public static boolean causeMachineUpdate(World aWorld, int aX, int aY, int aZ) {
        if (!aWorld.isRemote) stepToUpdateMachine(aWorld, aX, aY, aZ, new ArrayList<ChunkPosition>());
        return true;
    }
    ...
    public static Item constructCoolantCellItem(String aUnlocalized, int aMaxStore) {
        try {
            return (Item) Class.forName("gregtechmod.api.items.GT_CoolantCellIC_Item")
                .getConstructors()[0].newInstance(aUnlocalized, aMaxStore);
        } catch (Throwable e) {/* Do nothing */}
        try {
            return (Item) Class.forName("gregtechmod.api.items.GT_CoolantCell_Item")
                .getConstructors()[0].newInstance(aUnlocalized, aMaxStore);
        } catch (Throwable e) {/* Do nothing */}
     ...
     */
    public static Item constructElectricArmorItem(String aUnlocalized, int aCharge, int aTransfer, int aTier,
     ...
     */
    public static Item constructElectricEnergyStorageItem(String aUnlocalized, int aCharge, int aTransfer, int aTier,
        int aEmptyID, int aFullID) {
        try {
            return (Item) Class.forName("gregtechmod.api.items.GT_EnergyStoreIC_Item")
                .getConstructors()[0].newInstance(aUnlocalized, aCharge, aTransfer, aTier, aEmptyID, aFullID);
        } catch (Throwable e) {/* Do nothing */}
        try {
     ...
     */
    public static GT_Tool_Item constructHardHammerItem(String aUnlocalized, int aMaxDamage, int aEntityDamage) {
        try {
            return (GT_Tool_Item) Class.forName("gregtechmod.api.items.GT_HardHammer_Item")
                .getConstructors()[0].newInstance(aUnlocalized, aMaxDamage, aEntityDamage);
        } catch (Throwable e) {/* Do nothing */}
        return new gregtechmod.api.items.GT_Tool_Item(
            aUnlocalized,
            "item.bug.tooltip",
            aMaxDamage,
            aEntityDamage,
     ...
     */
    public static GT_Tool_Item constructCrowbarItem(String aUnlocalized, int aMaxDamage, int aEntityDamage) {
        try {
     ...
     */
    public static GT_Tool_Item constructWrenchItem(String aUnlocalized, int aMaxDamage, int aEntityDamage,
        int aDisChargedGTID) {
        try {
            return (GT_Tool_Item) Class.forName("gregtechmod.api.items.GT_Wrench_Item")
                .getConstructors()[0].newInstance(aUnlocalized, aMaxDamage, aEntityDamage, aDisChargedGTID);
        } catch (Throwable e) {/* Do nothing */}
        return new gregtechmod.api.items.GT_Tool_Item(
            aUnlocalized,
            "item.bug.tooltip",
     ...
     */
    public static GT_Tool_Item constructElectricScrewdriverItem(String aUnlocalized, int aMaxDamage, int aEntityDamage,
        int aDisChargedGTID) {
        try {
     ...
     */
    public static GT_Tool_Item constructElectricWrenchItem(String aUnlocalized, int aMaxDamage, int aEntityDamage,
        int aDisChargedGTID) {
        try {
     ...
     */
    public static GT_Tool_Item constructElectricSawItem(String aUnlocalized, int aMaxDamage, int aEntityDamage,
        int aToolQuality, float aToolStrength, int aEnergyConsumptionPerBlockBreak, int aDisChargedGTID) {
        try {
     ...
     */
    public static GT_Tool_Item constructElectricDrillItem(String aUnlocalized, int aMaxDamage, int aEntityDamage,
        int aToolQuality, float aToolStrength, int aEnergyConsumptionPerBlockBreak, int aDisChargedGTID) {
        try {
     ...
     */
    public static GT_Tool_Item constructElectricSolderingToolItem(String aUnlocalized, int aMaxDamage,
     ...
     */
    public static GT_Tool_Item constructEmptyElectricToolItem(String aUnlocalized, int aMaxDamage, int aChargedGTID) {
        try {
            return (GT_Tool_Item) Class.forName("gregtechmod.api.items.GT_EmptyToolIC_Item")
                .getConstructors()[0].newInstance(aUnlocalized, aMaxDamage, aChargedGTID);
        } catch (Throwable e) {/* Do nothing */}
    ...
    public static BaseMetaTileEntity constructBaseMetaTileEntity() {
        if (sBaseMetaTileEntityClass == null) {
            try {
                if (RF_ENERGY_COMPATIBILITY && IC_ENERGY_COMPATIBILITY)
                    return (sBaseMetaTileEntityClass = gregtechmod.api.metatileentity.BaseMetaTileEntityICRF.class)
                        .newInstance();
            } catch (Throwable e) {/* Do nothing */}
            try {
                if (IC_ENERGY_COMPATIBILITY)
                    return (sBaseMetaTileEntityClass = gregtechmod.api.metatileentity.BaseMetaTileEntityIC.class)
                        .newInstance();
            } catch (Throwable e) {/* Do nothing */}
            try {
                if (RF_ENERGY_COMPATIBILITY)
                    return (sBaseMetaTileEntityClass = gregtechmod.api.metatileentity.BaseMetaTileEntityRF.class)
                        .newInstance();
            } catch (Throwable e) {/* Do nothing */}
            try {
     ...
     * 
     * Best is you make a Runnable with all Cover Registrations, and add it to the Cover Registration ArrayList ontop of
     * this File.
     */
    public static void registerCover(ItemStack aStack, IIcon aCover) {
        int tStack = GT_Utility.stackUniversalId(aStack);
    ...
    public static GT_CoverBehavior getCoverBehavior(ItemStack aStack) {
        return getCoverBehavior(GT_Utility.stackUniversalId(aStack));
    }
    ...
    public static boolean registerTool(ItemStack aTool, Collection<ItemStackKey> aToolList) {
        if (aTool != null && aTool.getItem() != null) {
            ...
            if (!aToolList.contains(toolKey) && (aTool.getItem()
    ...
    // sToolsRegiter.add(() -> {
    // if (aTool == null || GT_Utility.isItemStackInList(aTool, sToolList) || (!aTool.getItem().isDamageable() &&
    // !GT_ModHandler.isElectricItem(aTool))) return;
