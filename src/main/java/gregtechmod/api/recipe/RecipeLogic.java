...
public class RecipeLogic {
    ...
    /**
     * Custom fucntion called every recipe progress time update, if you want to speed up machine because of some factor,
     * just increase the value up
     */
    ...
    public RecipeLogic(RecipeMap<?> recipeMap, IRecipeWorkable machine) {
        int inputs = machine.getInputItems()
            .size();
        int outputs = machine.getOutputItems()
            .size();
        int fluidIn = machine.getFluidInputs()
            .size();
        int fluidOut = machine.getFluidOutputs()
            .size();

        if (recipeMap != null) {
            if (inputs < recipeMap.minInputs || inputs > recipeMap.maxInputs
                || outputs < recipeMap.minOutputs
                || outputs > recipeMap.maxOutputs
                || fluidIn < recipeMap.minFluidInputs
                || fluidIn > recipeMap.maxFluidInputs
                || fluidOut < recipeMap.minFluidOutputs
                || fluidOut > recipeMap.maxFluidOutputs) {
                throw new IllegalArgumentException(
                    "Wrong recipe map was supplied to machine!\n" + "inputs: "
                        + inputs
                        + "; outputs: "
                        + outputs
                        + "\n"
            ...
            GT_Log.log.warn(
                "RecipeMap for machine with class name " + machine.getClass()
                    .getSimpleName()
                    + " == null! Make sure you set up custom recipe provoder, otherwise machine will spam errors");
        }
    ...
    public boolean update() {
        ...
        if (base.isAllowedToWork()) {
            ...
            if (progressTime == 0) {
                if (base.hasInventoryBeenModified() || base.hasWorkJustBeenEnabled()
                    || success
                    || base.getTimer() % 600 == 0
                    || wasNoEnergy) {// || isInputNonEmpty()) {
    ...
    protected boolean updateRecipeProgress() {
        if (getMachine().getBaseMetaTileEntity()
            .decreaseStoredEnergyUnits(EUt * (int) Math.pow(4, overclockersCount), false)) {
    ...
    protected boolean match(Recipe recipe) {
        return metadataVerifier.test(recipe)
            && recipe.matches(false, getMachine().getInputItems(), getMachine().getFluidInputs());
    }
    ...
    protected void startRecipe(Recipe recipe) {
        if (getMachine().spaceForOutput(recipe) && getMachine().getBaseMetaTileEntity()
            .decreaseStoredEnergyUnits(recipe.getEUtoStart(), false)) {
    ...
    protected void endRecipe(Recipe recipe) {
        ...
        for (ItemStack recipeOut : recipeOutputs) {
            ...
            if (amount > 0) {
                GT_Log.log.error(
                    String.format(
                        "Output overflow detected for machine (%s) left amount: %s, stack: %s",
                        getMachine().getClass()
                            .getName(),
                        amount,
                        recipeOut));
        ...
        for (FluidStack fluid : recipe.getFluidOutputs()) {
            ...
            if (amount > 0) {
                GT_Log.log.error(
                    String.format(
                        "Output overflow detected for machine (%s) left amount: %s, fluid: %s",
                        getMachine().getClass()
                            .getName(),
                        amount,
    ...
    protected boolean isInputNonEmpty() {
        return !getMachine().getInputItems()
            .isEmpty()
            || !getMachine().getFluidInputs()
                .isEmpty();
    }
    ...
    public boolean isActive() {
        return maxProgressTime > 0 && getMachine().getBaseMetaTileEntity()
            .isAllowedToWork();
    }
