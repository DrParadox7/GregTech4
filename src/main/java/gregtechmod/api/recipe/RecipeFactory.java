...
public abstract class RecipeFactory<F extends RecipeFactory<F>> {
    ...
    public RecipeFactory<F> input(Ingredient ingredient) {
        ...
        if (ingredient.getVariants()
            .isEmpty()) {
            errors.append(
                " - Supplied wrong ingredient: no variants! Current: '" + ingredient.getVariants()
                    + "', idx: "
                    + (inputItems.size() + 1)
                    + "\n");
        }
        ...
        if (ingredient.getCount() < 0) {
            errors.append(
                " - Supplied wrong ingredient: count is less than 0! Current: '" + ingredient.getCount()
                    + "', idx: "
                    + (inputItems.size() + 1)
                    + "\n");
        }
    ...
    public RecipeFactory<F> input(FluidStack fluid) {
        if (fluid == null)
            errors.append(" - Invalid FluidStack: '" + fluid + "', idx: " + (inputFluids.size() + 1) + "\n");
        else {
            if (fluid.getFluid() == null)
                errors.append(" - Invalid Fluid in stack: '" + fluid + "', idx: " + (inputFluids.size() + 1) + "\n");
            if (fluid.amount <= 0)
                errors.append(" - Invalid FluidStack size: '" + fluid + "', idx: " + (inputFluids.size() + 1) + "\n");
        }
    ...
    public RecipeFactory<F> output(ItemStack stack) {
        if (GT_Utility.isStackInvalid(stack)) errors
            .append("Found non-valid stack on output items: '" + stack + "', idx: " + (outputItems.size() + 1) + "\n");
        this.outputItems.add(stack);
        return this;
    }

    public RecipeFactory<F> outputs(ItemStack... stacks) {
        for (ItemStack stack : stacks) this.output(stack);
        return this;
    }

    public RecipeFactory<F> output(FluidStack fluid) {
        if (fluid == null)
            errors.append(" - Invalid FluidStack: '" + fluid + "', idx: " + (outputFluids.size() + 1) + "\n");
        else {
            if (fluid.getFluid() == null)
                errors.append(" - Invalid Fluid in stack: '" + fluid + "', idx: " + (outputFluids.size() + 1) + "\n");
            if (fluid.amount <= 0)
                errors.append(" - Invalid FluidStack size: '" + fluid + "', idx: " + (outputFluids.size() + 1) + "\n");
        }
    ...
    public RecipeFactory<F> chanced(ChancedOutput output) {
        if (output.getChance() < 0 || output.getChance() > MAX_CHANCE) {
            errors.append(
                " - Chance for '" + output.getStack()
                    + "' not valid. Shall be 0 <= chance <= 10000, idx: "
                    + (chancedOutput.size() + 1)
                    + "\n");
        }

        if (GT_Utility.isStackInvalid(output.getStack())) {
            errors.append(
                " - Stack for chanced output is invalid: '" + output.getStack()
                    + "', idx: "
                    + (chancedOutput.size() + 1)
                    + "\n");
        }
