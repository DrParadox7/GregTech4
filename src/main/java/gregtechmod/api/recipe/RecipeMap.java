...
public class RecipeMap<F extends RecipeFactory<F>> {
    ...
    public RecipeMap(int minInputs, int maxInputs, int minOutputs, int maxOutputs, int minFluidInputs,
        int maxFluidInputs, int minFluidOutputs, int maxFluidOutputs, F facorty) {
    ...
    public Recipe findRecipe(List<ItemStack> input, List<FluidStack> fluidInputs, Predicate<Recipe> metaChecker) {
        ...
        Recipe result = null;
        if (!recipesTotal.isEmpty()) result = findRecipe(recipesTotal, input, fluidInputs, metaChecker);
        return result != null && result.enabled ? result : null;
    }

    protected Recipe findRecipe(Collection<Recipe> recipes, List<ItemStack> input, List<FluidStack> fluidInputs,
        Predicate<Recipe> metaChecker) {
        if (recipes != null) {
    ...
    public Set<Recipe> getMappedRecipes(List<ItemStack> input, List<FluidStack> fluidInputs) {
        ...
        for (FluidStack fluid : fluidInputs) {
            if (GT_Utility.isFluidStackValid(fluid)) {
                ...
                if (recipesFluid != null) recipesTotal.addAll(recipesFluid);
            }
        }
        ...
        for (ItemStack item : input) {
            if (GT_Utility.isStackValid(item)) {
                ...
                if (recipesItems != null) recipesTotal.addAll(recipesItems);
                if (recipesItemsWild != null) recipesTotal.addAll(recipesItemsWild);
            }
        }
    ...
    protected void assertValidRecipe(Recipe recipe) {
        ...
        if (recipe.getInputs()
            .size() < minInputs)
            error += " - Inputs size less than minimum required(" + minInputs
                + "), current: "
                + recipe.getInputs()
                    .size();
        if (recipe.getInputs()
            .size() > maxInputs)
            error += " - Inputs size bigger than maximum allowed(" + maxInputs
                + "), current: "
                + recipe.getInputs()
                    .size();
        if (recipe.getAllOutputs()
            .size() < minOutputs)
            error += " - Total possible outputs less than minimum required(" + minOutputs
                + "), current: "
                + recipe.getAllOutputs()
                    .size();
        if (recipe.getAllOutputs()
            .size() > maxOutputs)
            error += " - Total possible outputs bigger than maximum allowed(" + maxOutputs
                + "), current: "
                + recipe.getAllOutputs()
                    .size();

        if (recipe.getFluidInputs()
            .size() < minFluidInputs)
            error += " - Fluid inputs size less than minimum required(" + minFluidInputs
                + "), current: "
                + recipe.getFluidInputs()
                    .size();
        if (recipe.getFluidInputs()
            .size() > maxFluidInputs)
            error += " - Fluid inputs size bigger than maximum allowed(" + maxFluidInputs
                + "), current: "
                + recipe.getFluidInputs()
                    .size();
        if (recipe.getFluidOutputs()
            .size() < minFluidOutputs)
            error += " - Fluid outputs size less than minimum required(" + minFluidOutputs
                + "), current: "
                + recipe.getFluidOutputs()
                    .size();
        if (recipe.getFluidOutputs()
            .size() > maxFluidOutputs)
            error += " - Fluid outputs outputs bigger than maximum allowed(" + maxFluidOutputs
                + "), current: "
                + recipe.getFluidOutputs()
                    .size();
    ...
    protected void createMappings(Recipe toMap) {
        ...
        for (Ingredient ingr : toMap.getInputs()) { // Can eat additional time of loading, but may improove recipe
                                                    // searching speed
    ...
    public boolean remove(Recipe recipe) {
        ...
        for (Ingredient ingr : recipe.getInputs()) {
            for (ItemStack var : ingr.getVariants()) {
                ...
                if (var1 != null) var1.remove(recipe);
                if (var2 != null) var2.remove(recipe);
    ...
    public boolean remove(CTRecipe recipe) {
        return recipe == null ? true : this.remove(recipe.backingRecipe);
    }
    ...
    public int hashCode() {
        return minInputs + maxInputs
            ...
            + maxFluidOutputs
            + factory.hashCode()
            + System.identityHashCode(recipeList); // To be sure, it is same list
    }

    @Override
    public String toString() {
        return String.format(
            "RecipeMap(item-in: %d-%d, item-out: %d-%d, fluid-in: %d-%d, fluid-out: %d-%d)%s",
            minInputs,
            maxInputs,
            minOutputs,
            maxOutputs,
