...
public class Recipe {
    ...
    public Recipe(int startEU, int EUt, int duration, boolean shaped, Collection<Ingredient> inputs,
        Collection<ItemStack> outputs, Collection<ChancedOutput> chancedOutputs) {
        this(
            startEU,
            EUt,
            duration,
    ...
    public Recipe(int startEU, int EUt, int duration, boolean shaped, Collection<Ingredient> itemInputs,
        Collection<ItemStack> itemOutputs, Collection<ChancedOutput> chancedOutputs, Collection<FluidStack> fluidInputs,
        Collection<FluidStack> fluidOutputs, Map<String, Object> meta) {
    ...
    public boolean matches(boolean decrease, List<ItemStack> input, List<FluidStack> fluidInputs) {
        ...
        boolean result;
        if (shaped) result = checkShaped(decrease, input, fluidInputs);
        else result = checkShapeless(decrease, input, fluidInputs);

        return result;
    }

    /**
     * Matching recipe to following input, in case of successfully found item in recipe input map will execute
     * actionPerfomer
     */
    private boolean checkShapeless(boolean decrease, List<ItemStack> inputs, List<FluidStack> fluidInputs) {
        ...
        if (items.getKey() && fluids.getKey()) {
            for (int i = 0; decrease && i < inputs.size() && !inputs.isEmpty(); i++) {
                ...
                if (newSize > 0) current.stackSize = newSize;
                else inputs.remove(i);
            }
            ...
            for (int i = 0; decrease && i < fluidInputs.size() && !fluidInputs.isEmpty(); i++) {
                ...
                if (newSize > 0) current.amount = newSize;
                else fluidInputs.remove(i);
            }
    ...
    private boolean checkShaped(boolean decrease, List<ItemStack> inputs, List<FluidStack> fluidInputs) {
        for (int i = 0; i < this.itemInputs.size(); i++) {
            ...
            ItemStack current = inputs.get(i);
            if (!ingr.match(current) || current.stackSize < ingr.getCount()) return false;
        }

        for (int i = 0; i < this.fluidInputs.size(); i++) {
            FluidStack current = fluidInputs.get(i);
            FluidStack rCurrent = this.fluidInputs.get(i);
            if (!rCurrent.isFluidEqual(current) || current.amount < rCurrent.amount) return false;
        }

        for (int i = 0; decrease && i < this.itemInputs.size(); i++) { // Check this, not sure about working
            int toConsume = this.itemInputs.get(i)
                .getCount();
            ItemStack current = inputs.get(i);
    ...
    private Pair<Boolean, Integer[]> matchItems(List<ItemStack> input) {
        ...
        for (Ingredient ingr : getInputs()) {
            ...
            for (int i = 0; i < input.size(); i++) {
                ...
                if (inputStack == null || !ingr.match(inputStack)) continue;
                int toConsume = Math.min(itemAmountInSlots[i], ingrAmount);
                ingrAmount -= toConsume;
                if (!consumed) itemAmountInSlots[i] -= toConsume;
                if (ingrAmount == 0) break;
            }
            if (ingrAmount > 0) return Pair.of(false, itemAmountInSlots);
        }
    ...
    private Pair<Boolean, Integer[]> matchFluids(List<FluidStack> input) {
        ...
        for (FluidStack ingr : getFluidInputs()) {
            ...
            for (int i = 0; i < input.size(); i++) {
                ...
                if (inputStack == null || !ingr.isFluidEqual(inputStack)) continue;
                int toConsume = Math.min(fluidAmountInSlots[i], ingrAmount);
                ingrAmount -= toConsume;
                if (!consumed) fluidAmountInSlots[i] -= toConsume;
                if (ingrAmount == 0) break;
            }
            if (ingrAmount > 0) return Pair.of(false, fluidAmountInSlots);
        }
    ...
    public void writeToNBT(NBTTagCompound data) {
        ...
        for (ItemStack out : this.getResults(new Random())) itemOut.appendTag(out.writeToNBT(new NBTTagCompound()));
        for (FluidStack out : this.getFluidOutputs()) fluidOut.appendTag(out.writeToNBT(new NBTTagCompound()));
    ...
    public int hashCode() {
        int hash = (3 * startEU) + (3 * EUt)
            ...
            + metadata.hashCode();
        for (FluidStack fluid : fluidInputs) hash += GT_Utility.hashFluidStack(fluid);
        for (FluidStack fluid : fluidOutputs) hash += GT_Utility.hashFluidStack(fluid);
        return hash;
    }
    ...
    public boolean equals(Object o) {
        if (o instanceof Recipe) {
            Recipe r = (Recipe) o;
            return r == this || (r.duration == this.duration && r.EUt == this.EUt
                && r.startEU == this.startEU
                && r.itemInputs.equals(itemInputs)) && doesListSame(r.itemOutputs, itemOutputs)
    ...
    public String toString() {
        return String.format(
            "Recipe(startEU=%d, EUt=%d, duration=%d, shaped=%s, enabled=%s)\nin=%s\nout=%s\nchancedOut=%s\nfluidIn=%s\nfluidOut=%s\nmeta=%s",
            startEU,
            EUt,
            duration,
