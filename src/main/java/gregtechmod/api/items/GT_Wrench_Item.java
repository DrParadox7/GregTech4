...
public class GT_Wrench_Item extends GT_Tool_Item {
    ...
    public boolean onItemUseFirst(ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY, int aZ,
        ...
        byte aMeta = (byte) aWorld.getBlockMetadata(aX, aY, aZ),
            aTargetSide = GT_Utility.determineWrenchingSide((byte) aSide, hitX, hitY, hitZ);
                    ...
                    if (GT_ModHandler.damageOrDechargeItem(aStack, tDamage, tDamage * 1000, aPlayer)) {
                        ...
                        for (ItemStack tStack : aBlock.getDrops(aWorld, aX, aY, aZ, aMeta, 0)) {
                            if (tOutput == null) {
                                ...
                                aWorld
                                    .spawnEntityInWorld(new EntityItem(aWorld, aX + 0.5, aY + 0.5, aZ + 0.5, tOutput));
                                tOutput = null;
                            }
                        }
        ...
        if (aBlock == Blocks.crafting_table || aBlock == Blocks.bookshelf) {
            if (GT_ModHandler.damageOrDechargeItem(aStack, 1, 1000, aPlayer)) {
                aWorld.spawnEntityInWorld(
                    new EntityItem(aWorld, aX + 0.5, aY + 0.5, aZ + 0.5, new ItemStack(aBlock, 1, aMeta)));
                aWorld.setBlockToAir(aX, aY, aZ);
                GT_Utility.sendSoundToPlayers(aWorld, GregTech_API.sSoundList.get(100), 1.0F, -1, aX, aY, aZ);
            }
            return true;
        ...
        if (aMeta == aTargetSide) {
            if (aBlock == Blocks.piston || aBlock == Blocks.sticky_piston
                ...
                if (GT_ModHandler.damageOrDechargeItem(aStack, 1, 1000, aPlayer)) {
                    aWorld.spawnEntityInWorld(
                        new EntityItem(aWorld, aX + 0.5, aY + 0.5, aZ + 0.5, new ItemStack(aBlock, 1, 0)));
                    aWorld.setBlockToAir(aX, aY, aZ);
                    GT_Utility.sendSoundToPlayers(aWorld, GregTech_API.sSoundList.get(100), 1.0F, -1, aX, aY, aZ);
                }
                return true;
