...
public class BaseMetaPipeEntity extends BaseTileEntity implements IGregTechTileEntity {
    ...
    public final GT_TileEntityPacket getMetaTileEntityData() {
        ...
        tOut.aRedstoneData = (oRedstoneData = (byte) (((mSidedRedstone[0] > 0) ? 1 : 0)
    ...
    public final void receiveMetaTileEntityData(short aID, int[] aCover, byte aTextureData, byte aUpdateData,
    ...
    public void setInventorySlotContents(int aIndex, ItemStack aStack) {
        ...
        if (hasValidMetaTileEntity())
            mMetaTileEntity.setInventorySlotContents(aIndex, GT_OreDictUnificator.setStack(true, aStack));
    }
    ...
    public int getTextureIndex(byte aSide, byte aMeta) {
        return getUncoveredIndex(aSide, aMeta);
    }
    ...
    public IIcon getTextureIcon(byte aSide, byte aMeta) {
        ...
        return getUncoveredIcon(aSide, aMeta);
    }

    public IIcon getUncoveredIcon(byte aSide, byte aMeta) {
        if ((mConnections & -64) != 0) return null;
        ...
        if (hasValidMetaTileEntity()) return mMetaTileEntity.getTextureIcon(
            aSide,
            tConnections,
    ...
    public int getUncoveredIndex(byte aSide, byte aMeta) {
        if ((mConnections & 64) != 0) return 368;
        if ((mConnections & -128) != 0) return 369;
        byte tConnections = mConnections;
        if (tConnections == 1 || tConnections == 2) tConnections = 3;
        else if (tConnections == 4 || tConnections == 8) tConnections = 12;
        else if (tConnections == 16 || tConnections == 32) tConnections = 48;
        if (hasValidMetaTileEntity()) return mMetaTileEntity.getTextureIndex(
            aSide,
            tConnections,
            tConnections == 0 || (tConnections & (1 << aSide)) != 0,
            getOutputRedstoneSignal(aSide) > 0);
    ...
    public boolean onRightclick(EntityPlayer aPlayer, byte aSide, float aX, float aY, float aZ) {
        ...
        if (isServerSide()) {
            ...
            if (GT_Utility.isItemStackInList(aPlayer.inventory.getCurrentItem(), GregTech_API.sSoftHammerList, true)) {
                // if (GT_ModHandler.damageOrDechargeItem(aPlayer.inventory.getCurrentItem(), 1, 1000, aPlayer)) {
                // GT_Utility.sendChatToPlayer(aPlayer, "Machine Processing: " +
                // (isAllowedToWork()?"Enabled":"Disabled"));
                // GT_Utility.sendSoundToPlayers(worldObj, GregTech_API.sSoundList.get(101), 1.0F, -1, xCoord, yCoord,
                // zCoord);
                // }
                return true;
            }
                ...
                if (GT_ModHandler.useSolderingIron(aPlayer.inventory.getCurrentItem(), aPlayer)) {
                    ...
                    GT_Utility.sendChatToPlayer(
                        ...
                        new ChatComponentTranslation(
                            ...
                            tSide,
                            new ChatComponentTranslation(
                                "metatileentity.status."
                                    + ((mStrongRedstone & (1 << tSide)) != 0 ? "redstone_strong" : "redstone_weak"))));
                    GT_Utility.sendSoundToPlayers(
                        worldObj,
            ...
            if (getCoverIDAtSide(cSide) == 0) {
                if (GT_Utility
                    ...
                    if (GregTech_API.getCoverBehavior(aPlayer.inventory.getCurrentItem())
                        .isCoverPlaceable(cSide, cover, this) && mMetaTileEntity.allowCoverOnSide(cSide, cover)) {
    ...
    public int fill(ForgeDirection aSide, FluidStack aFluid, boolean doFill) {
        if (mTickTimer > 5 && hasValidMetaTileEntity()
