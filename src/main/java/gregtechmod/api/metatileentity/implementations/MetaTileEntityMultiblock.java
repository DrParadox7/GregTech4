...
public abstract class MetaTileEntityMultiblock extends MetaTileEntity {
    ...
    public void onPostTick() {
        if (getBaseMetaTileEntity().isServerSide()) {
            ...
            if (structComplete) {
                ...
                if (getRepairStatus() <= 0) stopMachine();
            } else stopMachine();

            getBaseMetaTileEntity().setErrorDisplayID(
                (getBaseMetaTileEntity().getErrorDisplayID() & ~127) | (mWrench ? 0 : 1)
                    | (mScrewdriver ? 0 : 2)
                    | (mSoftHammer ? 0 : 4)
                    | (mHardHammer ? 0 : 8)
                    | (mSolderingTool ? 0 : 16)
    ...
    public void updateEfficiency() {
        mEfficiency = (int) (this.getMaxEfficiency(mInventory[1])
            * (this.getRepairStatus() * 1.0D / this.getIdealStatus()));
    }
    ...
    public static boolean isValidMetaTileEntity(MetaTileEntity aMetaTileEntity) {
        return aMetaTileEntity != null && aMetaTileEntity.getBaseMetaTileEntity() != null
    ...
    public boolean addToMachineList(IGregTechTileEntity aTileEntity) {
        ...
        if (aMetaTileEntity instanceof GT_MetaTileEntity_Hatch_EnergyInput)
            return mEnergyHatches.add((GT_MetaTileEntity_Hatch_EnergyInput) aMetaTileEntity);
    ...
    protected void findInventories() {
        ...
        for (GT_MetaTileEntity_Hatch_InputBus bus : mInputBusses) items.add(new ListAdapter<>(bus.mInventory));
        itemInputs = new InventoryHandlerList<>(items);
        items.clear();
        for (GT_MetaTileEntity_Hatch_OutputBus bus : mOutputBusses) items.add(new ListAdapter<>(bus.mInventory));
        itemOutputs = new InventoryHandlerList<>(items);
        items.clear();

        for (GT_MetaTileEntity_BasicTank hatch : mInputHatches) fluids.add(new ListAdapter<>(hatch.mFluid));
        fluidInputs = new InventoryHandlerList<>(fluids);
        fluids.clear();
        for (GT_MetaTileEntity_BasicTank hatch : mOutputHatches) fluids.add(new ListAdapter<>(hatch.mFluid));
                ...
                if (mInventory[1].getItemDamage() >= mInventory[1].getMaxDamage()) {
                    if (explodesOnComponentBreak(mInventory[1])) {
                        ...
                        for (MetaTileEntity tTileEntity : mInputHatches) tTileEntity.getBaseMetaTileEntity()
                            .doExplosion(GregTech_API.VOLTAGE_ULTIMATE);
                        ...
                        for (MetaTileEntity tTileEntity : mEnergyHatches) tTileEntity.getBaseMetaTileEntity()
                            .doExplosion(GregTech_API.VOLTAGE_ULTIMATE);
                        if (mufflerHatch.get() != null) mufflerHatch.get()
                            .getBaseMetaTileEntity()
                            .doExplosion(GregTech_API.VOLTAGE_ULTIMATE);
                        if (maintenanceHatch.get() != null) maintenanceHatch.get()
                            .getBaseMetaTileEntity()
                            .doExplosion(GregTech_API.VOLTAGE_ULTIMATE);
        ...
        if (GT_Utility.isFluidStackValid(resource)) {
            ...
            for (int i = 0; i < fluidInputs.size(); i++) {
                ...
                if (GT_Utility.isFluidStackValid(stackInSlot)) {
                    ...
                    copy.amount = amount;
                    if (doFill) fluidInputs.set(i, copy);
    ...
    public FluidStack drain(ForgeDirection aSide, FluidStack aFluid, boolean doDrain) {
        if (GT_Utility.isFluidStackValid(aFluid)) for (int i = 0; i < fluidOutputs.size(); i++) {
        ...
        for (GT_MetaTileEntity_Hatch_Dynamo tHatch : mDynamoHatches) {
            if (isValidMetaTileEntity(tHatch)) {
                if (tHatch.getBaseMetaTileEntity()
                    .increaseStoredEnergyUnits(aEU, false)) {
                    return true;
                }
            }
        }
