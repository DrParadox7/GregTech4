...
public class GT_MetaTileEntity_Hatch_Dynamo extends MetaTileEntity {
    ...
    public GT_MetaTileEntity_Hatch_Dynamo(int aID, String aName) {
        super(aID, aName);
    }
    ...
    public int maxEUOutput() {
        return Math.max(0, Math.min(getEUVar() - getMinimumStoredEU(), GregTech_API.VOLTAGE_INSANE));
    }

    @Override
    public int maxEUStore() {
        return GregTech_API.VOLTAGE_INSANE * 4 + getMinimumStoredEU();
    }

    @Override
    public String getSpecialVoltageToolTip() {
        return "metatileentity.TileEntity_EUp_OUT.Dynamo_Hatch.tooltip";
    }
