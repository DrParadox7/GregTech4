...
public class GT_MetaTileEntity_Hatch_Maintenance extends MetaTileEntity {
    ...
    public boolean mDuctTape = false, mWrench = false, mScrewdriver = false, mSoftHammer = false, mHardHammer = false,
    ...
    public boolean onRightclick(EntityPlayer aPlayer, byte aSide, float aX, float aY, float aZ) {
        if (aSide == getBaseMetaTileEntity().getFrontFacing())
            getBaseMetaTileEntity().openGUI(aPlayer, 155, GregTech_API.gregtechmod);
        return true;
    }
    ...
    public void onToolClick(ItemStack aStack, EntityLivingBase aPlayer) {
        ...
        if (GT_Utility.isItemStackInList(aStack, GregTech_API.sWrenchList, true)
            && GT_ModHandler.damageOrDechargeItem(aStack, 1, 1000, aPlayer)) mWrench = true;
        if (GT_Utility.isItemStackInList(aStack, GregTech_API.sScrewdriverList, true)
            && GT_ModHandler.damageOrDechargeItem(aStack, 1, 1000, aPlayer)) mScrewdriver = true;
        if (GT_Utility.isItemStackInList(aStack, GregTech_API.sSoftHammerList, true)
            && GT_ModHandler.damageOrDechargeItem(aStack, 1, 1000, aPlayer)) mSoftHammer = true;
        if (GT_Utility.isItemStackInList(aStack, GregTech_API.sHardHammerList, true)
            && GT_ModHandler.damageOrDechargeItem(aStack, 1, 1000, aPlayer)) mHardHammer = true;
        if (GT_Utility.isItemStackInList(aStack, GregTech_API.sCrowbarList, true)
