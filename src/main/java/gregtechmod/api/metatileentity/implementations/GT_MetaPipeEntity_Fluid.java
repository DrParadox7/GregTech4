...
public abstract class GT_MetaPipeEntity_Fluid extends MetaPipeEntity {
    ...
    public GT_MetaPipeEntity_Fluid(int aID, String aName) {
        super(aID, aName);
    }
            ...
            if (mLastReceivedFrom == oLastReceivedFrom) {
                ...
                for (byte i = 0; i < 6; i++) {
                    ...
                    if (tTileEntity != null) {
                        ...
                        FluidTankInfo[] tInfo = tTileEntity.getTankInfo(
                            ForgeDirection.getOrientation(i)
                                .getOpposite());
                        if (tInfo != null && tInfo.length > 0) {
                            if (getBaseMetaTileEntity().getCoverBehaviorAtSide(i)
                                .letsLiquidIn(
                                    i,
                                    getBaseMetaTileEntity().getCoverIDAtSide(i),
                                    getBaseMetaTileEntity().getCoverDataAtSide(i),
                            ...
                            if (getBaseMetaTileEntity().getCoverBehaviorAtSide(i)
                                .letsLiquidOut(
                                    ...
                                    getBaseMetaTileEntity())) {
                                mConnections |= (1 << i);
                                if (((1 << i) & mLastReceivedFrom) == 0) tTanks.put(
                                    tTileEntity,
                                    ForgeDirection.getOrientation(i)
                                        .getOpposite());
                            }
                    ...
                    if (tAmount >= tTanks.size()) {
                        for (IFluidHandler tTileEntity : tTanks.keySet()) {
                            int tFilledAmount = tTileEntity
                                .fill(tTanks.get(tTileEntity), drain(tAmount / tTanks.size(), false), false);
                            if (tFilledAmount > 0) {
                        ...
                        if (mFluid != null && mFluid.amount > 0) {
                            for (IFluidHandler tTileEntity : tTanks.keySet()) {
                                int tFilledAmount = tTileEntity
                                    .fill(tTanks.get(tTileEntity), drain(mFluid.amount, false), false);
                                if (tFilledAmount > 0) {
                                    tTileEntity.fill(tTanks.get(tTileEntity), drain(tFilledAmount, true), true);
                                }
    ...
    public final int fill_default(ForgeDirection aSide, FluidStack aFluid, boolean doFill) {
        ...
        if (mFluid == null || mFluid.getFluidID() <= 0) {
            ...
            if (doFill) {
                ...
                if (getBaseMetaTileEntity() != null) FluidEvent.fireEvent(
    ...
    public final FluidStack drain(int maxDrain, boolean doDrain) {
        ...
        int used = maxDrain;
        if (mFluid.amount < used) used = mFluid.amount;
        ...
        if (doDrain && getBaseMetaTileEntity() != null) FluidEvent.fireEvent(
