                ...
                if (tFluid != null && isFluidInputAllowed(tFluid)) {
                    if (getFillableStack() == null) {
                        if (isFluidInputAllowed(tFluid) && tFluid.amount <= getCapacity()) {
                            if (getBaseMetaTileEntity().addStackToSlot(
                                getOutputSlot(),
                                GT_Utility.getContainerForFilledItem(mInventory[getInputSlot()]),
                                1)) {
    ...
    public int fill(FluidStack aFluid, boolean doFill) {
        ...
        if (getFillableStack() == null || getFillableStack().getFluidID() <= 0) {
            if (aFluid.amount <= getCapacity()) {
                if (doFill) setFillableStack(aFluid.copy());
        ...
        if (aFluid.amount <= space) {
            if (doFill) getFillableStack().amount += aFluid.amount;
            return aFluid.amount;
        }
