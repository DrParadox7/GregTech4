...
public class BaseMetaTileEntity extends BaseTileEntity implements IGregTechTileEntity {
    ...
    protected int mStoredRF = 0, mStoredEnergy = 0, mStoredSteam = 0, mAverageEUInputIndex = 0,
    ...
    private boolean mHasEnoughEnergy = true, mNeedsBatteryUpgrade = false, mRunningThroughTick = false,
        mInputDisabled = false, mOutputDisabled = false, mMuffler = false, mLockUpgrade = false, mActive = false,
        mRedstone = false, mWorkUpdate = false, mSteamConverter = false, mRFConverter = false,
        mInventoryChanged = false, mWorks = true, mNeedsUpdate = true, mNeedsBlockUpdate = true,
        mSendClientData = false, oRedstone = false;
    private byte mColor = 0, oColor = 0, mStrongRedstone = 0, oRedstoneData = 63, oTextureData = 0, oUpdateData = 0,
        oLightValueClient = 0, oLightValue = 0, mLightValue = 0, mRSEnergyCells = 0, mSteamTanks = 0, mOverclockers = 0,
        mTransformers = 0, mOtherUpgrades = 0, mFacing = 0, oFacing = 0, mWorkData = 0;
    private int mDisplayErrorCode = 0, oOutput = 0, oX = 0, oY = 0, oZ = 0, mUpgradedStorage = 0,
        mTimeStatisticsIndex = 0, mLagWarningCount = 0;
    private short mID = 0;
    ...
    public void setInitialValuesAsNBT(NBTTagCompound aNBT, short aID) {
        if (aNBT == null) {
            ...
            mUpgradedStorage = aNBT.getInteger("mUpgradedStorage") + aNBT.getByte("mBatteries") * 10000
                + aNBT.getByte("mLiBatteries") * 100000;
    ...
    private boolean createNewMetatileEntity(short aID) {
        if (aID < 16 || aID >= GregTech_API.MAXIMUM_METATILE_IDS || GregTech_API.mMetaTileList[aID] == null) {
            ...
            if (aID != 0) {
                ...
                GregTech_API.mMetaTileList[aID].newMetaEntity(this)
    ...
    public void chargeItem(ItemStack aStack) {
        decreaseStoredEU(
            (int) GT_ModHandler
                .chargeElectricItem(aStack, getStoredEU(), mMetaTileEntity.getOutputTier(), false, false),
            true);
    }
    ...
    public void dischargeItem(ItemStack aStack) {
        increaseStoredEnergyUnits(
            (int) GT_ModHandler.dischargeElectricItem(
                aStack,
                getEUCapacity() - getStoredEU(),
    ...
    public void updateEntity() {
        ...
        try {
            ...
            if (hasValidMetaTileEntity()) {
                if (mTickTimer++ == 0) {
                    ...
                    if (isServerSide()) for (byte i = 0; i < 6; i++) if (getCoverIDAtSide(i) != 0)
                        if (!mMetaTileEntity.allowCoverOnSide(i, getCoverIDAtSide(i))) dropCover(i, i, true);

                    try {
                        mMetaTileEntity.onFirstTick();
                    } catch (Throwable e) {
                        GT_Log.log.error(
                            "Encountered Exception while ticking MetaTileEntity, the Game should've crashed now, but I prevented that. Please report immidietly to GregTech Intergalactical!!!");
                        GT_Log.log.catching(e);
                    }
                ...
                if (isServerSide()) {
                    for (byte i = 0; i < 6; i++) {
                        short tCoverTickRate = getCoverBehaviorAtSide(i)
                            .getTickRate(i, getCoverIDAtSide(i), mCoverData[i], this);
                        if (tCoverTickRate > 0 && mTickTimer % tCoverTickRate == 0) {
                            try {
                                mCoverData[i] = getCoverBehaviorAtSide(i).doCoverThings(
                                    i,
                                ...
                                GT_Log.log.error(
                                    "Encountered Exception while ticking Cover, the Game should've crashed now, but I prevented that. Please report immidietly to GregTech Intergalactical!!!");
                                GT_Log.log.catching(e);
                            }
                        }
                    }
                ...
                if (isServerSide()) {
                    ...
                    if (mMetaTileEntity.isElectric()
                        && (mMetaTileEntity.isEnetOutput() || mMetaTileEntity.isEnetInput())) {
                        for (byte i = 0; i < 6; i++) {
                    ...
                    if (mIsAddedToEnet && mMetaTileEntity.isElectric()
                        && mMetaTileEntity.isEnetOutput()
                        && getOutputVoltage() > 0) {
                        for (int i = 0; i < getOutputAmperage()
                            && getStoredEU() >= getOutputVoltage() + mMetaTileEntity.getMinimumStoredEU(); i++) {
                            int tEU = GT_ModHandler.emitEnergyToEnet(getOutputVoltage(), worldObj, this)
                                - getOutputVoltage();
                    ...
                    try {
                        if (getEUCapacity() > 0) {
                            if (GregTech_API.sMachineFireExplosions && getRandomNumber(1000) == 0) {
                                ...
                                if (tBlock != null && (tBlock == Blocks.fire || tBlock instanceof BlockFire))
                                    doEnergyExplosion();
                            }
                            ...
                            if (getRandomNumber(1000) == 0) {
                                if ((getCoverIDAtSide((byte) 1) == 0
                                    ...
                                    if (GregTech_API.sMachineThunderExplosions && worldObj.getWorldInfo()
                                        .isThundering() && getRandomNumber(3) == 0) {
                                        doEnergyExplosion();
                                    }
                                }
                            }
                ...
                if (isServerSide()) {
                    ...
                    if (mTickTimer > 10) {
                        byte tData = (byte) ((getFrontFacing() & 7) | (mActive ? 8 : 0)
                            | (mRedstone ? 16 : 0)
                            | (mLockUpgrade ? 32 : 0));
            ...
            GT_Log.log.error(
                "Encountered Exception while ticking TileEntity, the Game should've crashed now, but I prevented that. Please report immidietly to GregTech Intergalactical!!!");
            GT_Log.log.catching(e);
        }
    ...
    public void onDataPacket(NetworkManager manager, S35PacketUpdateTileEntity packet) {
        ...
        this.receiveMetaTileEntityData(
            data.getShort("mID"),
    ...
    public Packet getDescriptionPacket() {
        ...
        data.setByte(
            "oTextureData",
            oTextureData = (byte) ((getFrontFacing() & 7) | (mActive ? 8 : 0)
                | (mRedstone ? 16 : 0)
                | (mLockUpgrade ? 32 : 0)));
    ...
    public final GT_TileEntityPacket getMetaTileEntityData() {
        ...
        tOut.aTextureData = (byte) ((getFrontFacing() & 7) | (mActive ? 8 : 0)
            | (mRedstone ? 16 : 0)
            | (mLockUpgrade ? 32 : 0));
        tOut.aUpdateData = (oUpdateData = hasValidMetaTileEntity() ? mMetaTileEntity.getUpdateData() : 0);
        tOut.aRedstoneData = ((byte) (((mSidedRedstone[0] > 0) ? 1 : 0) | ((mSidedRedstone[1] > 0) ? 2 : 0)
            | ((mSidedRedstone[2] > 0) ? 4 : 0)
            | ((mSidedRedstone[3] > 0) ? 8 : 0)
            | ((mSidedRedstone[4] > 0) ? 16 : 0)
            | ((mSidedRedstone[5] > 0) ? 32 : 0)));
    ...
    public final void receiveMetaTileEntityData(short aID, int[] aCovers, byte aTextureData, byte aUpdateData,
        byte aRedstoneData, byte aColorData) {
        issueTextureUpdate();
        if (mID != aID && aID > 0) {
            mID = aID;
    ...
    public ArrayList<String> getDebugInfo(EntityPlayer aPlayer, int aLogLevel) {
        ...
        if (aLogLevel > 0) {
            if (getRFCapacity() > 0 && hasRFConverterUpgrade())
                tList.add("RF energy strored: " + getStoredRF() + " of " + getRFCapacity() + " RF");
            if (getSteamCapacity() > 0 && hasSteamEngineUpgrade())
                tList.add("Steam stored: " + getStoredSteam() + " of " + getSteamCapacity() + " Steam");
            tList.add("Machine is " + (mActive ? "active" : "inactive"));
            tList.add("Working is " + (mWorks ? "enabled" : "disabled"));
            if (mNeedsBatteryUpgrade && isBatteryUpgradable(10000, (byte) 1))
    ...
    public boolean isGivingInformation() {
        if (hasValidMetaTileEntity()) return mMetaTileEntity.isGivingInformation();
        return false;
    }

    @Override
    public boolean isValidFacing(byte aSide) {
        if (hasValidMetaTileEntity()) return mMetaTileEntity.isFacingValid(aSide);
        return false;
    }
    ...
    public int getSizeInventory() {
        if (hasValidMetaTileEntity()) return mMetaTileEntity.getSizeInventory();
        return 0;
    }

    @Override
    public ItemStack getStackInSlot(int aIndex) {
        if (hasValidMetaTileEntity()) return mMetaTileEntity.getStackInSlot(aIndex);
        return null;
    }
    ...
    public void setInventorySlotContents(int aIndex, ItemStack aStack) {
        mInventoryChanged = true;
        if (hasValidMetaTileEntity()) mMetaTileEntity.setInventorySlotContents(aIndex, aStack);
    }

    @Override
    public String getInventoryName() {
        if (hasValidMetaTileEntity()) return mMetaTileEntity.getInventoryName();
        if (GregTech_API.mMetaTileList[mID] != null) return GregTech_API.mMetaTileList[mID].getInventoryName();
        return "";
    }
    ...
    public void openInventory() {
        if (hasValidMetaTileEntity()) mMetaTileEntity.onOpenGUI();
    }

    @Override
    public void closeInventory() {
        if (hasValidMetaTileEntity()) mMetaTileEntity.onCloseGUI();
    }

    @Override
    public boolean isUseableByPlayer(EntityPlayer aPlayer) {
        return hasValidMetaTileEntity() && playerOwnsThis(aPlayer, false)
    ...
    public int getProgress() {
        return hasValidMetaTileEntity() ? mMetaTileEntity.getRecipeLogic() != null ? mMetaTileEntity.getRecipeLogic()
            .getProgressTime() : 0 : 0;
    }
    ...
    public int getMaxProgress() {
        return hasValidMetaTileEntity() ? mMetaTileEntity.getRecipeLogic() != null ? mMetaTileEntity.getRecipeLogic()
            .getMaxProgressTime() : 0 : 0;
    }

    @Override
    public boolean increaseProgress(int aProgressAmountInTicks) {
        return hasValidMetaTileEntity()
            ? mMetaTileEntity.increaseProgress(aProgressAmountInTicks) != aProgressAmountInTicks
            : false;
    }
    ...
    public boolean decreaseStoredEnergyUnits(int aEnergy, boolean aIgnoreTooLessEnergy) {
        ...
        if (getUniversalEnergyCapacity() < aEnergy) mNeedsBatteryUpgrade = true;
        return mHasEnoughEnergy = decreaseStoredRF(aEnergy * 8, false) || decreaseStoredSteam(aEnergy, false)
            || decreaseStoredEU(aEnergy, aIgnoreTooLessEnergy)
    ...
    public int getOutputAmperage() {
        if (hasValidMetaTileEntity() && mMetaTileEntity.isElectric())
            return mMetaTileEntity.maxEUPulses() == 1 && mMetaTileEntity.isTransformingLowEnergy() && mTransformers > 0
                ? 4
                : mMetaTileEntity.maxEUPulses();
        return 0;
    }
    ...
    public int getOutputVoltage() {
        if (hasValidMetaTileEntity() && mMetaTileEntity.isElectric() && mMetaTileEntity.isEnetOutput())
            return mMetaTileEntity.maxEUOutput() * (mTransformers > 0
                ? (int) Math.pow(4, mTransformers - (mMetaTileEntity.isTransformingLowEnergy() ? 1 : 0))
                : 1);
        return 0;
    }
    ...
    public int getInputVoltage() {
        if (hasValidMetaTileEntity() && mMetaTileEntity.isElectric())
            return mMetaTileEntity.maxEUInput() * (int) Math.pow(4, mTransformers);
        return mMetaTileEntity.isElectric() ? Integer.MAX_VALUE : 0;
    }

    @Override
    public boolean increaseStoredRF(int aEnergy, boolean aIgnoreTooMuchEnergy) {
        if (!hasValidMetaTileEntity()) return false;
        if (mMetaTileEntity.getRFVar() < getRFCapacity() || aIgnoreTooMuchEnergy) {
            setStoredRF(mMetaTileEntity.getRFVar() + aEnergy);
            return true;
        }
        return false;
    }
    ...
    public int getUniversalEnergyStored() {
        return Math.max(Math.max(getStoredEU(), getStoredRF()), getStoredSteam());
    }

    @Override
    public int getUniversalEnergyCapacity() {
        return Math.max(Math.max(getEUCapacity(), getRFCapacity()), getSteamCapacity());
    }

    @Override
    public int getStoredRF() {
        if (hasValidMetaTileEntity()) return Math.min(mMetaTileEntity.getRFVar(), getRFCapacity());
        return 0;
    }

    @Override
    public int getRFCapacity() {
        if (hasValidMetaTileEntity()) return mMetaTileEntity.maxRFStore() + mRSEnergyCells * 100000;
        return 0;
    }
    ...
    public int getEUCapacity() {
        if (hasValidMetaTileEntity()) return mMetaTileEntity.maxEUStore() + getUpgradeStorageVolume();
        return 0;
    }
    ...
    public int getSteamCapacity() {
        if (hasValidMetaTileEntity()) return mMetaTileEntity.maxSteamStore() + mSteamTanks * 32000;
        return 0;
    }
    ...
    public int getTextureIndex(byte aSide, byte aMeta) {
        if (hasValidMetaTileEntity()) return mMetaTileEntity
            .getTextureIndex(aSide, getFrontFacing(), mActive, getOutputRedstoneSignal(aSide) > 0);
        return -2;
    }
    ...
    public boolean setStoredRF(int aEnergy) {
        if (!hasValidMetaTileEntity()) return false;
        if (aEnergy < 0) aEnergy = 0;
        mMetaTileEntity.setRFVar(aEnergy);
        return true;
    }
    ...
    public boolean decreaseStoredRF(int aEnergy, boolean aIgnoreTooLessEnergy) {
        if (!hasValidMetaTileEntity()) return false;
        if (mMetaTileEntity.getRFVar() - aEnergy >= 0 || aIgnoreTooLessEnergy) {
            setStoredRF(mMetaTileEntity.getRFVar() - aEnergy);
            if (mMetaTileEntity.getRFVar() < 0) {
                setStoredRF(0);
                return false;
            }
            return true;
    ...
    public boolean playerOwnsThis(EntityPlayer aPlayer, boolean aCheckPrecicely) {
        if (!hasValidMetaTileEntity()) return false;
        if (aCheckPrecicely || unbreakable() || privateAccess() || mOwnerName.equals(""))
    ...
    public boolean unbreakable() {
        if (!hasValidMetaTileEntity()) return mLockUpgrade;
        return mLockUpgrade || mMetaTileEntity.unbreakable();
    }

    public void doEnergyExplosion() {
        if (getUniversalEnergyCapacity() > 0 && getUniversalEnergyStored() >= mMetaTileEntity.getMinimumStoredEU())
            doExplosion(
                getOutputVoltage() * (getUniversalEnergyStored() >= getUniversalEnergyCapacity() ? 4
    ...
    public void doExplosion(int aAmount) {
        ...
        float tStrength = aAmount < GregTech_API.VOLTAGE_ULTRALOW ? 1.0F
            : aAmount < GregTech_API.VOLTAGE_LOW ? 2.0F
                : aAmount < GregTech_API.VOLTAGE_MEDIUM ? 3.0F
                    : aAmount < GregTech_API.VOLTAGE_HIGH ? 4.0F
                        : aAmount < GregTech_API.VOLTAGE_EXTREME ? 5.0F
                            : aAmount < GregTech_API.VOLTAGE_EXTREME * 2 ? 6.0F
                                : aAmount < GregTech_API.VOLTAGE_INSANE ? 7.0F
                                    : aAmount < GregTech_API.VOLTAGE_LUDICROUS ? 8.0F
                                        : aAmount < GregTech_API.VOLTAGE_ULTIMATE ? 9.0F : 10.0F;
    ...
    public boolean onRightclick(EntityPlayer aPlayer, byte aSide, float aX, float aY, float aZ) {
        ...
        if (isServerSide()) {
            if (!unbreakable() || aPlayer.getDisplayName()
                .equalsIgnoreCase(getOwnerName())) {
                if (getColorization() >= 0 && GT_Utility
                    .areStacksEqual(new ItemStack(Items.water_bucket, 1), aPlayer.inventory.getCurrentItem())) {
                    aPlayer.inventory.getCurrentItem()
                ...
                if (GT_Utility.isItemStackInList(aPlayer.inventory.getCurrentItem(), GregTech_API.sWrenchList, true)) {
                    ...
                    if (!isValidFacing(tSide) || tSide == getFrontFacing()) {
                        if (mMetaTileEntity.isWrenchable()) {
                            if (GT_ModHandler.damageOrDechargeItem(
                                aPlayer.inventory.getCurrentItem(),
                                mMetaTileEntity.isSimpleMachine() ? 3 : 10,
                                mMetaTileEntity.isSimpleMachine() ? 3000 : 10000,
                                aPlayer)) {
                                worldObj.spawnEntityInWorld(
                                    new EntityItem(worldObj, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, getDrop()));
                ...
                if (GT_Utility
                    .isItemStackInList(aPlayer.inventory.getCurrentItem(), GregTech_API.sScrewdriverList, true)) {
                    if (GT_ModHandler.damageOrDechargeItem(aPlayer.inventory.getCurrentItem(), 1, 200, aPlayer)) {
                        ...
                        else if (getCoverIDAtSide(aSide) == -1 && mMetaTileEntity.allowCoverOnSide(aSide, -2))
                            setCoverIDAtSide(aSide, -2);
                        else if (getCoverIDAtSide(aSide) == 0 && mMetaTileEntity.allowCoverOnSide(aSide, -1))
                            setCoverIDAtSide(aSide, -1);
                        else {
                            setCoverDataAtSide(
                                aSide,
                ...
                if (GT_Utility
                    .isItemStackInList(aPlayer.inventory.getCurrentItem(), GregTech_API.sHardHammerList, true)) {
                    if (GT_ModHandler.damageOrDechargeItem(aPlayer.inventory.getCurrentItem(), 1, 1000, aPlayer)) {
                        ...
                        IChatComponent component = new ChatComponentTranslation(
                            "metatileentity.status.auto_io",
                            new ChatComponentTranslation(
                                mInputDisabled ? "metatileentity.disabled" : "metatileentity.enabled"),
                            new ChatComponentTranslation(
                                mOutputDisabled ? "metatileentity.disabled" : "metatileentity.enabled"));
                ...
                if (GT_Utility
                    .isItemStackInList(aPlayer.inventory.getCurrentItem(), GregTech_API.sSoftHammerList, true)) {
                    if (GT_ModHandler.damageOrDechargeItem(aPlayer.inventory.getCurrentItem(), 1, 1000, aPlayer)) {
                        mWorks = !mWorks;
                        mWorkUpdate = true;
                        GT_Utility.sendChatToPlayer(
                            aPlayer,
                            new ChatComponentTranslation(
                                "metatileentity.status.working",
                                new ChatComponentTranslation(
                                    isAllowedToWork() ? "metatileentity.enabled" : "metatileentity.disabled")));
                        GT_Utility.sendSoundToPlayers(
                            worldObj,
                ...
                if (GT_Utility
                    .isItemStackInList(aPlayer.inventory.getCurrentItem(), GregTech_API.sSolderingToolList, true)) {
                    byte tSide = GT_Utility.determineWrenchingSide(aSide, aX, aY, aZ);
                    if (GT_ModHandler.useSolderingIron(aPlayer.inventory.getCurrentItem(), aPlayer)) {
                        mStrongRedstone ^= (1 << tSide);
                        IChatComponent component = new ChatComponentTranslation(
                            "metatileentity.status.redstone",
                            tSide,
                            new ChatComponentTranslation(
                                "metatileentity.status.redstone_"
                ...
                if (getCoverIDAtSide(aSide) == 0) {
                    if (GT_Utility
                        .isItemStackInIntList(aPlayer.inventory.getCurrentItem(), GregTech_API.sCovers.keySet())) {
                        int cover = GT_Utility.stackUniversalId(aPlayer.inventory.getCurrentItem());
                        if (GregTech_API.getCoverBehavior(aPlayer.inventory.getCurrentItem())
                            .isCoverPlaceable(aSide, cover, this) && mMetaTileEntity.allowCoverOnSide(aSide, cover)) {
                    ...
                    if (GT_Utility
                        .isItemStackInList(aPlayer.inventory.getCurrentItem(), GregTech_API.sCrowbarList, true)) {
                        if (GT_ModHandler.damageOrDechargeItem(aPlayer.inventory.getCurrentItem(), 1, 1000, aPlayer)) {
                            GT_Utility.sendSoundToPlayers(
                                worldObj,
                ...
                if (isUpgradable() && aPlayer.inventory.getCurrentItem() != null) {
                    if (GT_Utility
                        .areStacksEqual(aPlayer.inventory.getCurrentItem(), GregTech_API.getGregTechComponent(25, 1))) {
                    ...
                    if (GT_Utility
                        .areStacksEqual(aPlayer.inventory.getCurrentItem(), GregTech_API.getGregTechComponent(80, 1))) {
                    ...
                    if (GT_Utility
                        .areStacksEqual(aPlayer.inventory.getCurrentItem(), GregTech_API.getGregTechComponent(89, 1))) {
                    ...
                    if (GT_Utility
                        .areStacksEqual(aPlayer.inventory.getCurrentItem(), GregTech_API.getGregTechComponent(88, 1))) {
                    ...
                    if (getInputVoltage() < GregTech_API.VOLTAGE_HIGH
                        && getOutputVoltage() * getOutputAmperage() < GregTech_API.VOLTAGE_HIGH) {
                    ...
                    if (GT_Utility
                        .areStacksEqual(aPlayer.inventory.getCurrentItem(), GregTech_API.getGregTechComponent(26, 1))) {
                    ...
                    if (GT_Utility
                        .areStacksEqual(aPlayer.inventory.getCurrentItem(), GregTech_API.getGregTechComponent(12, 1))) {
                    ...
                    if (GT_Utility
                        .areStacksEqual(aPlayer.inventory.getCurrentItem(), GregTech_API.getGregTechComponent(13, 1))) {
                    ...
                    if (GT_Utility
                        .areStacksEqual(aPlayer.inventory.getCurrentItem(), GregTech_API.getGregTechComponent(14, 1))) {
                    ...
                    if (hasRFConverterUpgrade() && GT_Utility
                        .areStacksEqual(aPlayer.inventory.getCurrentItem(), GregTech_API.getGregTechComponent(28, 1))) {
                        mRSEnergyCells++;
                    ...
                    if (hasSteamEngineUpgrade() && GT_Utility
                        .areStacksEqual(aPlayer.inventory.getCurrentItem(), GregTech_API.getGregTechComponent(81, 1))) {
                        mSteamTanks++;
                        mNeedsBatteryUpgrade = false;
                        GT_Utility.sendSoundToPlayers(
                            worldObj,
            ...
            try {
                if (hasValidMetaTileEntity() && mID > 15)
                    return mMetaTileEntity.onRightclick(aPlayer, aSide, aX, aY, aZ);
            } catch (Throwable e) {
                GT_Log.log.error(
    ...
    public int[] getAccessibleSlotsFromSide(int aSide) {
        if (hasValidMetaTileEntity() && (getCoverBehaviorAtSide((byte) aSide)
            .letsItemsOut((byte) aSide, getCoverIDAtSide((byte) aSide), getCoverDataAtSide((byte) aSide), this)
            || getCoverBehaviorAtSide((byte) aSide)
                .letsItemsIn((byte) aSide, getCoverIDAtSide((byte) aSide), getCoverDataAtSide((byte) aSide), this)))
    ...
    public boolean canInsertItem(int aIndex, ItemStack aStack, int aSide) {
        return hasValidMetaTileEntity() && (mRunningThroughTick || !mInputDisabled)
            && getCoverBehaviorAtSide((byte) aSide)
                .letsItemsIn((byte) aSide, getCoverIDAtSide((byte) aSide), getCoverDataAtSide((byte) aSide), this)
    ...
    public boolean canExtractItem(int aIndex, ItemStack aStack, int aSide) {
        return hasValidMetaTileEntity() && (mRunningThroughTick || !mOutputDisabled)
            && getCoverBehaviorAtSide((byte) aSide)
                .letsItemsOut((byte) aSide, getCoverIDAtSide((byte) aSide), getCoverDataAtSide((byte) aSide), this)
    ...
    public boolean isTransformerUpgradable() {
        return isUpgradable() && mMetaTileEntity.isElectric()
            && mMetaTileEntity.isTransformerUpgradable()
            && getInputVoltage() < GregTech_API.VOLTAGE_INSANE
            && getOutputVoltage() * getOutputAmperage() < GregTech_API.VOLTAGE_INSANE;
    }

    @Override
    public boolean isBatteryUpgradable(int aStorage, byte aTier) {
        return isUpgradable() && mMetaTileEntity.isElectric()
            && mMetaTileEntity.isBatteryUpgradable()
            && GT_Utility.getTier(getInputVoltage()) >= aTier
            && aStorage + getEUCapacity() > 0;
    }
    ...
    public byte getOutputRedstoneSignal(byte aSide) {
        return (byte) (getCoverBehaviorAtSide(aSide)
            ...
            || (mRedstone && getCoverBehaviorAtSide(aSide)
    ...
    public void setOnFire() {
        for (byte i = 0; i < 6; i++) {
            ...
            if (tBlock == null || null == tBlock
                .getCollisionBoundingBoxFromPool(worldObj, getOffsetX(i, 1), getOffsetY(i, 1), getOffsetZ(i, 1))) {
    ...
    public int fill(ForgeDirection aSide, FluidStack aFluid, boolean doFill) {
        if (hasValidMetaTileEntity() && (mRunningThroughTick || !mInputDisabled)
            && (aSide == ForgeDirection.UNKNOWN || (mMetaTileEntity.isLiquidInput((byte) aSide.ordinal())
                && getCoverBehaviorAtSide((byte) aSide.ordinal()).letsLiquidIn(
                    (byte) aSide.ordinal(),
    ...
    public FluidStack drain(ForgeDirection aSide, int maxDrain, boolean doDrain) {
        if (hasValidMetaTileEntity() && (mRunningThroughTick || !mOutputDisabled)
            && (aSide == ForgeDirection.UNKNOWN || (mMetaTileEntity.isLiquidOutput((byte) aSide.ordinal())
                && getCoverBehaviorAtSide((byte) aSide.ordinal()).letsLiquidOut(
                    (byte) aSide.ordinal(),
                    getCoverIDAtSide((byte) aSide.ordinal()),
                    getCoverDataAtSide((byte) aSide.ordinal()),
                    this)))) {
    ...
    public double injectEnergyUnits(ForgeDirection aDirection, double aAmount) {
        return injectEnergyUnits((byte) aDirection.ordinal(), (int) aAmount, 1) ? 0 : aAmount;
    }
    ...
    public double getOfferedEnergy() {
        return (hasValidMetaTileEntity() && getStoredEU() - mMetaTileEntity.getMinimumStoredEU() >= getOutputVoltage())
            ? Math.max(0, getOutputVoltage())
            : 0;
    }
    ...
    public int getStored() {
        return Math.min(getStoredEU(), getCapacity());
    }
