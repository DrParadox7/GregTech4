...
public class GT_Container_RedstoneCircuitBlock extends GT_ContainerMetaTile_Machine {
    ...
    public ItemStack slotClick(int aSlotIndex, int aMouseclick, int aShifthold, EntityPlayer aPlayer) {
        ...
        if (tSlot != null) {
            ...
            if (aSlotIndex < 4) {
                ...
                if (tStack == null) {
                    ((GT_MetaTileEntity_RedstoneCircuitBlock) mTileEntity.getMetaTileEntity()).changeGateData(
                        aSlotIndex,
                        aMouseclick == 0 ? aShifthold == 0 ? +1 : aShifthold == 1 ? +128 : +16
                ...
                if (aMouseclick == 0) ((GT_MetaTileEntity_RedstoneCircuitBlock) mTileEntity.getMetaTileEntity())
                    .switchGateForward(aShifthold != 0);
                else((GT_MetaTileEntity_RedstoneCircuitBlock) mTileEntity.getMetaTileEntity())
    ...
    public void prepareChanges(JsonObject data, boolean force) {
        ...
        GT_MetaTileEntity_RedstoneCircuitBlock m = (GT_MetaTileEntity_RedstoneCircuitBlock) mTileEntity
            .getMetaTileEntity();
        Integer[] arr = new Integer[8];
        for (int i = 0; i < arr.length; i++) arr[i] = m.mGateData[i];
