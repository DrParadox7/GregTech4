...
public class GT_Container_ElectricRegulatorAdvanced extends GT_ContainerMetaTile_Machine {
    ...
    public ItemStack slotClick(int aSlotIndex, int aMouseclick, int aShifthold, EntityPlayer aPlayer) {
        ...
        if (tSlot != null) {
            ...
            GT_MetaTileEntity_ElectricRegulatorAdvanced mte = (GT_MetaTileEntity_ElectricRegulatorAdvanced) mTileEntity
                .getMetaTileEntity();
            if (aSlotIndex == 27) {
                ...
                mte.mTargetSlots[aSlotIndex - 18] = Math.min(
                    99,
                    Math.max(
                        0,
                        mte.mTargetSlots[aSlotIndex - 18]
    ...
    public void prepareChanges(JsonObject data, boolean force) {
        ...
        GT_MetaTileEntity_ElectricRegulatorAdvanced m = (GT_MetaTileEntity_ElectricRegulatorAdvanced) mTileEntity
            .getMetaTileEntity();
        Integer[] arr = new Integer[9];
        for (int i = 0; i < arr.length; i++) arr[i] = m.mTargetSlots[i];
