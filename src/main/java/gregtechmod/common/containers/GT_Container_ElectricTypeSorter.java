...
public class GT_Container_ElectricTypeSorter extends GT_ContainerMetaTile_Machine {
    ...
    public ItemStack slotClick(int aSlotIndex, int aMouseclick, int aShifthold, EntityPlayer aPlayer) {
        ...
        if (tSlot != null) {
            ...
            GT_MetaTileEntity_ElectricBufferSmall mte = (GT_MetaTileEntity_ElectricBufferSmall) mTileEntity
                .getMetaTileEntity();
            if (aSlotIndex == 1) {
                if (aMouseclick == 0)
                    ((GT_MetaTileEntity_ElectricTypeSorter) mTileEntity.getMetaTileEntity()).switchModeForward();
                else((GT_MetaTileEntity_ElectricTypeSorter) mTileEntity.getMetaTileEntity()).switchModeBackward();
                return null;
            } else if (aSlotIndex == 2) {
                mte.bOutput = !mte.bOutput;
                if (aPlayer.worldObj.isRemote) GT_Utility.sendChatToPlayer(
                    aPlayer,
                    new ChatComponentTranslation("metatileentity.status.energy_out." + mte.bOutput));
                return null;
            } else if (aSlotIndex == 3) {
                mte.bRedstoneIfFull = !mte.bRedstoneIfFull;
                if (aPlayer.worldObj.isRemote) GT_Utility.sendChatToPlayer(
                    aPlayer,
                    new ChatComponentTranslation("metatileentity.status.redstone_if_full." + mte.bRedstoneIfFull));
                return null;
            } else if (aSlotIndex == 4) {
                mte.bInvert = !mte.bInvert;
                if (aPlayer.worldObj.isRemote) GT_Utility.sendChatToPlayer(
                    aPlayer,
                    new ChatComponentTranslation("metatileentity.status.redstone_invert." + mte.bInvert));
                return null;
            } else if (aSlotIndex == 5) {
                ((GT_MetaTileEntity_ElectricTypeSorter) mTileEntity
                    .getMetaTileEntity()).mTargetDirection = (byte) ((((GT_MetaTileEntity_ElectricTypeSorter) mTileEntity
                        .getMetaTileEntity()).mTargetDirection + 1) % 6);
            }
        }
    ...
    public void prepareChanges(JsonObject data, boolean force) {
        ...
        mMode.updateAndWriteChanges(
            data,
            force,
            ((GT_MetaTileEntity_ElectricTypeSorter) mTileEntity.getMetaTileEntity()).mMode);
        mTargetDirection.updateAndWriteChanges(
            data,
            force,
            ((GT_MetaTileEntity_ElectricTypeSorter) mTileEntity.getMetaTileEntity()).mTargetDirection);
    }
