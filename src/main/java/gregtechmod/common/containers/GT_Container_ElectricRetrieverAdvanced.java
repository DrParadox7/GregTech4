...
public class GT_Container_ElectricRetrieverAdvanced extends GT_ContainerMetaTile_Machine {
    ...
    public GT_Container_ElectricRetrieverAdvanced(final InventoryPlayer aInventoryPlayer,
    ...
    public ItemStack slotClick(final int aSlotIndex, final int aMouseclick, final int aShifthold,
        ...
        if (tSlot != null) {
            ...
            GT_MetaTileEntity_ElectricRetrieverAdvanced mte = (GT_MetaTileEntity_ElectricRetrieverAdvanced) this.mTileEntity
                .getMetaTileEntity();
            if (aSlotIndex == 18) {
            ...
            if (aSlotIndex == 19) {
                ...
                if (aPlayer.worldObj.isRemote) GT_Utility.sendChatToPlayer(
                    aPlayer,
                    new ChatComponentTranslation("metatileentity.status.partial_req." + mte.mPartialRequests));
                return null;
            }
            ...
            if (aSlotIndex >= 9 && aSlotIndex < 18) {
                GT_MetaTileEntity_ElectricRetrieverAdvanced m = (GT_MetaTileEntity_ElectricRetrieverAdvanced) this.mTileEntity
                    .getMetaTileEntity();
                m.mTargetSlots[aSlotIndex - 9] = Math.min(
                    99,
                    Math.max(
                        0,
                        m.mTargetSlots[aSlotIndex - 9] + ((aMouseclick == 0) ? -1 : 1) * ((aShifthold != 1) ? 1 : 16)));
                return null;
            }
        }
        return super.slotClick(aSlotIndex, aMouseclick, aShifthold, aPlayer);
    ...
    public void prepareChanges(JsonObject data, boolean force) {
        ...
        for (int i = 0; i < 9; i++) arr[i] = Integer.valueOf(m.mTargetSlots[i]);
