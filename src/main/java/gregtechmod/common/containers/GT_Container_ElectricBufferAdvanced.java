...
public class GT_Container_ElectricBufferAdvanced extends GT_ContainerMetaTile_Machine {
    ...
    public ItemStack slotClick(int aSlotIndex, int aMouseclick, int aShifthold, EntityPlayer aPlayer) {
        ...
        if (tSlot != null) {
            ...
            if (aSlotIndex == 1) {
                ...
                ((GT_MetaTileEntity_ElectricBufferAdvanced) mte).mTargetSlot = Math.max(
                    0,
                    ((GT_MetaTileEntity_ElectricBufferAdvanced) mte).mTargetSlot - ((aShifthold == 1) ? 16 : 1));
                return null;
            } else if (aSlotIndex == 5) {
                ((GT_MetaTileEntity_ElectricBufferAdvanced) mte).mTargetSlot = Math.min(
                    8192,
                    ((GT_MetaTileEntity_ElectricBufferAdvanced) mte).mTargetSlot + ((aShifthold == 1) ? 16 : 1));
                return null;
            }
    ...
    public void prepareChanges(JsonObject data, boolean force) {
        ...
        mTargetSlot.updateAndWriteChanges(
            data,
            force,
            ((GT_MetaTileEntity_ElectricBufferAdvanced) mTileEntity.getMetaTileEntity()).mTargetSlot);
    }
