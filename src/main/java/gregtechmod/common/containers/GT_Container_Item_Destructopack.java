...
public class GT_Container_Item_Destructopack extends Container {
    ...
    public ItemStack slotClick(int aSlotIndex, int aMouseclick, int aShifthold, EntityPlayer aPlayer) {
        ...
        if (aSlotIndex < 1) {
            if (aPlayer.inventory.getItemStack() != null) {
                ...
                if (aShifthold == 1
                    || (ret = emtpyContainer(aPlayer.inventory.getItemStack(), aPlayer, aMouseclick == 0)) == null) {
                    if (aMouseclick == 0) {
                        if (aShifthold == 1) {
    ...
    protected ItemStack emtpyContainer(ItemStack held, EntityPlayer player, boolean leftClick) {
        ...
        if (held.stackSize > 1) {
            ...
            if (result != null) player.inventory.setItemStack(result);
        }
    ...
    public ItemStack getContainer(ItemStack item) {
        if (FluidContainerRegistry.isContainer(item)) return FluidContainerRegistry.drainFluidContainer(item);
            ...
            if (fl != null && fl.amount > 0) return item;
        }
