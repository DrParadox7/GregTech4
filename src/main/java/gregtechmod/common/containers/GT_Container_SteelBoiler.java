...
public class GT_Container_SteelBoiler extends GT_Container_BronzeBoiler {
    ...
    protected void write(JsonObject data, boolean force) {
        ...
        mProcessingEnergy.updateAndWriteChanges(
            data,
            force,
            Math.min(14, Math.max(m.mProcessingEnergy > 0 ? 1 : 0, (m.mProcessingEnergy * 14) / 640)));
        mSteamAmount.updateAndWriteChanges(
            data,
            force,
            Math.min(54, Math.max(0, ((m.mSteam == null ? 0 : m.mSteam.amount) * 54) / 31900)));
        mWaterAmount.updateAndWriteChanges(
            data,
            force,
            Math.min(54, Math.max(0, ((m.mFluid[0] == null ? 0 : m.mFluid[0].amount) * 54) / 15900)));
    }
}
