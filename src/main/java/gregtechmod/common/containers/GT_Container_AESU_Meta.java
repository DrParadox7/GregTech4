...
public class GT_Container_AESU_Meta extends GT_ContainerMetaTile_Machine {
    ...
    public ItemStack slotClick(int aSlotIndex, int aMouseclick, int aShifthold, EntityPlayer aPlayer) {
        ...
        if (tSlot != null) {
            ...
            switch (aSlotIndex) {
                case 2:
                    ((GT_MetaTileEntity_AESU) mTileEntity.getMetaTileEntity()).mOutput = Math
                        .min(8192, tOutput + (aShifthold == 1 ? 256 : 64));
                    return null;
                case 3:
                    ((GT_MetaTileEntity_AESU) mTileEntity.getMetaTileEntity()).mOutput = Math
                        .min(8192, tOutput + (aShifthold == 1 ? 16 : 1));
                    return null;
                case 4:
                    ((GT_MetaTileEntity_AESU) mTileEntity.getMetaTileEntity()).mOutput = Math
                        .max(0, tOutput - (aShifthold == 1 ? 16 : 1));
                    return null;
                case 5:
                    ((GT_MetaTileEntity_AESU) mTileEntity.getMetaTileEntity()).mOutput = Math
                        .max(0, tOutput - (aShifthold == 1 ? 256 : 64));
                    return null;
            }
        }
