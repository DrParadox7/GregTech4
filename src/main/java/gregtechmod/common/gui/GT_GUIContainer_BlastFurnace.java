...
public class GT_GUIContainer_BlastFurnace extends GT_GUIContainerMetaTile_Machine {
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        if (((GT_Container_BlastFurnace) mContainer).mMachine.get()) fontRenderer.drawString(
            I18n.format(
                "metatileentity.GT_BlastFurnace.UI.heat_capacity",
                ((GT_Container_BlastFurnace) mContainer).mHeatCapacity.get()),
            8,
            ySize - 103,
            4210752);
        else fontRenderer
            .drawString(I18n.format("metatileentity.status.multiblock.incomplete"), 8, ySize - 103, 0xF00000);
        ...
        else fontRenderer.drawString(StatCollector.translateToLocal("container.inventory"), 8, ySize - 94, 4210752);
    }
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
        ...
        if (mContainer != null) {
            ...
            int tScale = Math.max(
                ...
                Math.min(
                    20,
                    (cont.mProgressTime.get() > 0 ? 1 : 0) + (cont.mProgressTime.get() * 20)
                        / (cont.mMaxProgressTime.get() < 1 ? 1 : cont.mMaxProgressTime.get())));
