...
public class GUI_UniversalMacerator extends GT_GUIContainerMetaTile_Machine {
     ...
     * @param aTextureFile
     */
    public GUI_UniversalMacerator(InventoryPlayer aInventoryPlayer, IGregTechTileEntity aTileEntity, String aName,
        String aTextureFile) {
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
        ...
        if (mContainer != null) {
            if (((UniversalMacerator) mContainer).mOutputting.get())
                drawTexturedModalRect(x + 7, y + 62, 176, 18, 18, 18);
            if (((UniversalMacerator) mContainer).mItemTransfer.get())
                drawTexturedModalRect(x + 25, y + 62, 176, 36, 18, 18);
            if (((UniversalMacerator) mContainer).mSeperatedInputs.get())
                drawTexturedModalRect(x + 43, y + 62, 176, 54, 18, 18);

            if (mContainer.mMaxProgressTime.get() > 0) {
                int tSize = 20, tProgress = Math.max(
                    1,
                    Math.min(
                        tSize,
                        (mContainer.mProgressTime.get() > 0 ? 1 : 0)
                            + (mContainer.mProgressTime.get() * tSize) / mContainer.mMaxProgressTime.get()))
                    % (tSize + 1);
