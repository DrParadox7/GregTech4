...
public class GT_GUIContainer_BronzeBlastFurnace extends GT_GUIContainerMetaTile_Machine {
    ...
    public GT_GUIContainer_BronzeBlastFurnace(InventoryPlayer aInventoryPlayer, IGregTechTileEntity aTileEntity) {
        super(
            ...
            GregTech_API.GUI_PATH + "BronzeBlastFurnace.png");
    }
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        if (mContainer != null) {
            if (((GT_Container_BronzeBlastFurnace) mContainer).mDisplayErrorCode.get() > 0) fontRenderer
                .drawString(I18n.format("metatileentity.status.multiblock.incomplete"), 8, ySize - 95, 0x9d2913);
        }
    }
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
        ...
        if (mContainer != null) {
            if (mContainer.mProgressTime.get() > 0) drawTexturedModalRect(
                x + 58,
                y + 28,
                176,
                0,
                Math.max(
                    ...
                    Math.min(
                        20,
                        (mContainer.mProgressTime.get() > 0 ? 1 : 0) + (mContainer.mProgressTime.get() * 20)
                            / (mContainer.mMaxProgressTime.get() < 1 ? 1 : mContainer.mMaxProgressTime.get()))),
                11);
        }
    }
}
