...
public class GT_GUIContainer_DistillationTower extends GT_GUIContainerMetaTile_Machine {
    ...
    public GT_GUIContainer_DistillationTower(InventoryPlayer aInventoryPlayer, IGregTechTileEntity aTileEntity) {
        super(
            new GT_Container_DistillationTower(aInventoryPlayer, aTileEntity),
            GregTech_API.GUI_PATH + "DistillationTower.png");
    }

    @Override
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        fontRenderer.drawSplitString(I18n.format("metatileentity.GT_DistillationTower.name"), 116, 4, 60, 4210752);
        if (!((GT_Container_DistillationTower) mContainer).mMachine.get()) fontRenderer
            .drawSplitString(I18n.format("metatileentity.status.multiblock.incomplete"), 116, 30, 60, 0xF00000);
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
        ...
        if (mContainer != null && mContainer.mProgressTime.get() > 0) {
            ...
            int tScale = Math.max(
                ...
                Math.min(
                    72,
                    (cont.mProgressTime.get() > 0 ? 1 : 0) + (cont.mProgressTime.get() * 72)
