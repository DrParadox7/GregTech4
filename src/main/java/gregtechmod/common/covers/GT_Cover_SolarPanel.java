            ...
            if (aTileEntity.getInputVoltage() * 32 >= mEUtDay) {
                if (!aTileEntity.getWorld()
                    ...
                    boolean bRain = aTileEntity.getWorld()
                        .isRaining()
                        && aTileEntity.getBiome(aTileEntity.getXCoord(), aTileEntity.getZCoord()).rainfall > 0;
                    if (!bRain || aTileEntity.getWorld().skylightSubtracted < 4) {
                        if (aTileEntity.getSkyAtSide((byte) aSide)) {
                            aTileEntity.increaseStoredEnergyUnits(
                                bRain || !aTileEntity.getWorld()
                                    .isDaytime() ? mEUtNight : mEUtDay,
                                false);
                        }
                    }
                }
