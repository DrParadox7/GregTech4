...
public class GT_Cover_Drain extends GT_CoverBehavior {
        ...
        if (aSide != 6) {
            ...
            if (aCoverVariable < 3 && aTileEntity instanceof IFluidHandler) {
                if (aSide == 1) {
                    if (aTileEntity.getWorld()
                        ...
                        if (aTileEntity.getWorld()
                            ...
                            int tAmount = (int) (aTileEntity
                                .getBiome(aTileEntity.getXCoord(), aTileEntity.getZCoord()).rainfall * 10);
                            if (tAmount > 0) {
                                ((IFluidHandler) aTileEntity).fill(
                                    ForgeDirection.getOrientation(aSide),
                                    GT_ModHandler.getWater(
                ...
                if (t != Blocks.air) {
                    if ((t == Blocks.water || t == Blocks.flowing_water) && aTileEntity.getMetaIDAtSide(aSide) == 0) {
                        ...
                        if ((t == Blocks.lava || t == Blocks.flowing_lava) && aTileEntity.getMetaIDAtSide(aSide) == 0) {
                            tLiquid = GT_ModHandler.getLava(1000);
                        } else if (t instanceof IFluidBlock) {
                            tLiquid = ((IFluidBlock) t).drain(
                                aTileEntity.getWorld(),
                    ...
                    if (tLiquid != null && tLiquid.getFluid() != null
                        && (aSide > 1 || (aSide == 0 && tLiquid.getFluid()
                            ...
                            || (aSide == 1 && tLiquid.getFluid()
                                .getDensity() >= 0))) {
                        if (((IFluidHandler) aTileEntity).fill(ForgeDirection.getOrientation(aSide), tLiquid, false)
                            == tLiquid.amount) {
            ...
            if (aCoverVariable >= 3 && t != Blocks.air) {
                if (t == Blocks.lava || t == Blocks.flowing_lava
                    || t == Blocks.water
                    || t == Blocks.flowing_water
                    || t instanceof IFluidBlock) {
                    aTileEntity.getWorld()
                        .setBlock(
                            aTileEntity.getOffsetX(aSide, 1),
                            aTileEntity.getOffsetY(aSide, 1),
    ...
    public int onCoverScrewdriverclick(byte aSide, int aCoverID, int aCoverVariable, ICoverable aTileEntity,
        ...
        if (aCoverVariable < 3) GT_Utility.sendChatToPlayer(
            aPlayer,
            new ChatComponentTranslation(
                "metatileentity.cover.io." + Math.max(1, aCoverVariable + (aCoverVariable * 2))));
        else GT_Utility.sendChatToPlayer(
            aPlayer,
            new ChatComponentTranslation("metatileentity.cover.drain." + (aCoverVariable - 3)));
        return aCoverVariable;
    }
    ...
    public boolean letsLiquidIn(byte aSide, int aCoverID, int aCoverVariable, ICoverable aTileEntity) {
        return !(aCoverVariable > 1 && aTileEntity instanceof IMachineProgress
            && (((IMachineProgress) aTileEntity).isAllowedToWork() != aCoverVariable < 2));
    }
