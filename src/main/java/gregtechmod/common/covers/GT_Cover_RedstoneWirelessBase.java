...
public abstract class GT_Cover_RedstoneWirelessBase extends GT_CoverBehavior {
    ...
    public boolean onCoverRightclick(byte aSide, int aCoverID, int aCoverVariable, ICoverable aTileEntity,
        ...
        if (((aX > 0.375 && aX < 0.625) || aSide > 3) && ((aY > 0.375 && aY < 0.625) || aSide < 2)
            && ((aZ > 0.375 && aZ < 0.625) || aSide == 2 || aSide == 3)) {
            GregTech_API.sWirelessRedstone.put(aCoverVariable, (byte) 0);
            aCoverVariable = GT_Utility.stackUniversalId(aPlayer.inventory.getCurrentItem());
            aTileEntity.setCoverDataAtSide(aSide, aCoverVariable);
            GT_Utility.sendChatToPlayer(
                aPlayer,
                new ChatComponentTranslation("metatileentity.GT_Tesseract.message.1", aCoverVariable));
            return true;
        }
        return false;
    }
