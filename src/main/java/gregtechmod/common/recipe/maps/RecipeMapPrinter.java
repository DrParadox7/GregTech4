...
public class RecipeMapPrinter extends RecipeMap<SimpleRecipeFactory> {
    ...
    public Recipe findRecipe(List<ItemStack> itemInputs, List<FluidStack> fluidInputs, Predicate<Recipe> metaChecker) {
        if (GT_Utility.isStackValid(itemInputs.get(2)) && GT_Utility.isStackValid(itemInputs.get(0))
            && GT_Utility.isStackValid(itemInputs.get(1))) {
            if (itemInputs.get(2)
                ...
                RecipeFactory<?> fact = factory().EUt(1)
                    .duration(200)
                    .setShaped(true);
            ...
            if (Loader.isModLoaded("arsmagica2")) {
                if (itemInputs.get(2)
                    .getItem()
                    .getUnlocalizedName()
                    .equals("item.ArsMagica:spell_recipe")
                    && itemInputs.get(0)
                        .getItem()
                        .getUnlocalizedName()
                        .equals("item.ArsMagica:spell_parchment")
                    && itemInputs.get(1)
                        .getItem()
                        .getUnlocalizedName()
                        .equals("item.ArsMagica:rune_blank")) {
                    return factory().EUt(4)
                        .duration(500)
