        ...
        if (!Loader.isModLoaded("pokepatch")) {
            ...
            if (!GT_Mod.mAlreadyPlayed || aUserName.equalsIgnoreCase("richardg867")) {
                ...
                if (aUserName.equalsIgnoreCase("richardg867")) {
                    handler.sendPacket(
                        new S02PacketChat(new ChatComponentText("Is that enough of a disclaimer for you RichardG?")));
                }

                GT_Mod.mAlreadyPlayed = true;
            }
            if (GT_Mod.sMessage != null && GT_Mod.sMessage.length() > 5
                && GregTech_API.sSpecialFile.get(GT_ConfigCategories.news, GT_Mod.sMessage, true)) {
            ...
            try {
                ...
                if (e < 820) {
                    handler.sendPacket(
                        new S02PacketChat(new ChatComponentText("GregTech: Please update your IndustrialCraft here:")));
                    handler.sendPacket(
                        new S02PacketChat(
                            new ChatComponentText("http://ic2api.player.to:8080/job/IC2_experimental/?")));
                }
            } catch (Throwable e) {
                handler.sendPacket(
                    new S02PacketChat(new ChatComponentText("GregTech: Please update your IndustrialCraft here:")));
                handler.sendPacket(
                    new S02PacketChat(new ChatComponentText("http://ic2api.player.to:8080/job/IC2_experimental/?")));
            }
        }
    }
}
