...
public class GT_TileEntityPacket extends GT_Packet {
    ...
    public void fromBytes(ByteBuf buf) {
        ...
        aZ = buf.readInt();
        aID = buf.readShort();
        for (int i = 0; i < 6; i++) aCovers[i] = buf.readInt();
    ...
    public void toBytes(ByteBuf buf) {
        ...
        buf.writeInt(aZ);
        buf.writeShort(aID);
        for (int i = 0; i < 6; i++) buf.writeInt(aCovers[i]);
    ...
    public String toString() {
        return String.format(
            "GT_TileEntityPacket[x=%d, y=%d, z=%d, ID=%d, TextureData=%d, UpdateData=%d, RedstoneData=%d, ColorData=%d, RFUpgrade=%s, CoverData=%s]",
            aX,
            aY,
            aZ,
            (int) aID,
        ...
        if (context.side == Side.CLIENT) {
            if (Minecraft.getMinecraft().thePlayer != null && Minecraft.getMinecraft().thePlayer.worldObj != null) {
                ...
                if (GregTech_API.DEBUG_MODE
                    && (name.equals("Player") || name.equals("GregoriusT") || name.equals("The_DnK")))
                    GT_Log.log.info(
                        String.format(
                            "Received initial MetaTileEntity Data: %s Bytes @ (%d;%d;%d) during Tick: %s",
                            data.capacity(),
                            aX,
                            aY,
                            aZ,
                ...
                if (tTile != null) {
                    if (tTile instanceof BaseMetaTileEntity) {
                        ((BaseMetaTileEntity) tTile).receiveMetaTileEntityData(
                            aID,
                            aCovers,
                            aTextureData,
                            aUpdateData,
                        ...
                        ((BaseMetaPipeEntity) tTile).receiveMetaTileEntityData(
                            aID,
                            aCovers,
                            aTextureData,
                            aUpdateData,
