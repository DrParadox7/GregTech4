...
public class FluidInventoryPacket extends GT_Packet {
    ...
    public IMessage handle(MessageContext context) {
        if (context.side != Side.CLIENT) throw new IllegalStateException("Somehow client packet handling on server!");
        if (Minecraft.getMinecraft().thePlayer == null) throw new IllegalStateException("Player client-side is null!");
