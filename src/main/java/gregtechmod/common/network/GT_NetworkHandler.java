...
public final class GT_NetworkHandler implements Runnable {
    ...
    public static <T extends GT_Packet> void sendPacket(T packet, EntityPlayerMP reciever) {
        if (GregTech_API.DEBUG_MODE && GregTech_API.SECONDARY_DEBUG_MODE) GT_Log.log.info(
            "Packet '" + packet.getClass()
                .getSimpleName() + "' been sent to '" + reciever.getDisplayName() + "' data: " + packet.toString());
    ...
    private static <T extends GT_Packet> void registerType(SimpleNetworkWrapper channel, Class<T> type) {
        try {
            SimpleIndexedCodec codec = ReflectionHelper
                .getPrivateValue(SimpleNetworkWrapper.class, channel, "packetCodec");
    ...
    private static void registerHandler(SimpleNetworkWrapper channel, Side side) {
        try {
            EnumMap<Side, FMLEmbeddedChannel> channels = ReflectionHelper
                .getPrivateValue(SimpleNetworkWrapper.class, channel, "channels");
            FMLEmbeddedChannel realChannel = channels.get(side);
            SimpleChannelHandlerWrapper<GT_Packet, IMessage> handler = new SimpleChannelHandlerWrapper<>(
                GT_PacketHandler.INSTANCE,
                side,
                GT_Packet.class);
            String type1 = realChannel.findChannelHandlerNameForType(SimpleIndexedCodec.class);
            realChannel.pipeline()
                .addAfter(type1, "GTNetHandler", handler);
        } catch (Throwable e) {
