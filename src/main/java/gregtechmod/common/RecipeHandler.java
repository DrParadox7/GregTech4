...
public class RecipeHandler {
    ...
    private static void proceedTasks() {
        ...
        GT_Log.log.info(
            String
                .format("\tOnFinish tasks completed for %.3f seconds", (System.currentTimeMillis() - time) / 1000.0F));
    }
    ...
    private static void processSmelting() {
        ...
        Map<ItemStack, ItemStack> smelting = FurnaceRecipes.smelting()
            .getSmeltingList();
        Iterator<Entry<ItemStack, ItemStack>> iter = smelting.entrySet()
            .iterator();
        ...
        GT_Log.log.info(
            String.format(
                "\tSmelting processor finished for %.3f seconds",
    ...
    private static void processCrafting() {
        ...
        @SuppressWarnings("unchecked")
        Iterator<IRecipe> iter = CraftingManager.getInstance()
            .getRecipeList()
            .iterator();
        ...
        GT_Log.log.info(
            String.format(
                "\tCrafting processor finished for %.3f seconds",
                (System.currentTimeMillis() - time) / 1000.0F));
    }
    ...
    private static void processIC2() {
        ...
        for (Entry<Integer, Collection<IIC2Matcher>> recipeMap : IC2Remove.asMap()
            .entrySet()) {
            Collection<IIC2Matcher> tasks = new ArrayList<>(recipeMap.getValue());
            Iterator<Entry<IRecipeInput, RecipeOutput>> iter = IC2Mapping.get(recipeMap.getKey())
                .entrySet()
                .iterator();
        ...
        GT_Log.log.info(
            String.format(
                "\tIC2 Recipe processor finished for %.3f seconds",
                (System.currentTimeMillis() - time) / 1000.0F));
    }
    ...
    public static void activateHandler() {
        ...
        GT_Log.log.info(
            String.format("Recipe handler finished for %.3f seconds", (System.currentTimeMillis() - time) / 1000.0F));
    }
    ...
    public static class OutputMatcher implements IRecipeMatcher {
        ...
        public boolean matches(IRecipe recipe) {
            ...
            return recipeOut != null
                && (wildcard ? recipeOut.getItem() == output.getItem() : recipeOut.isItemEqual(output));
        }
