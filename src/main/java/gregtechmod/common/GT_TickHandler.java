...
public class GT_TickHandler {
    ...
    public void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        this.mAnimationTick++;
        if (this.mAnimationTick % 50L == 0L) this.mAnimationDirection = !this.mAnimationDirection;
    ...
    public void onPlayerTick(TickEvent.PlayerTickEvent event) {
        if (event.phase != Phase.START && event.player != null && !event.player.isDead) {
            if (event.player.worldObj.isRemote) {
                ...
                if (event.player.getDisplayName()
                    .equalsIgnoreCase("immibis")) {
                    for (int i = 1; i <= 42; i++) {
                        if (GregTech_API.sSpecialFile
                            .get("PumpkinOfShame", "Config.Random.Number." + new Random(i).nextInt(4200), true)) {
                            break;
                        }
                    }
                ...
                if (GregTech_API.sServerTickCounter % 200 == 0
                    && (event.player.capabilities.allowEdit
                        || event.player.isCurrentToolAdventureModeExempt(0, -10000, 0))
                    && !event.player.capabilities.isCreativeMode
                    && GT_Mod.sSurvivalIntoAdventure) {
                    event.player.setGameType(GameType.ADVENTURE);
                    boolean temp = event.player.capabilities.allowEdit
                        || event.player.isCurrentToolAdventureModeExempt(0, -10000, 0);
                    ...
                    if (GT_Mod.sAxeWhenAdventure) {
                        if (!temp) GT_Utility
                            .sendChatToPlayer(event.player, new ChatComponentTranslation("gregtech.adventure.2"));
                ...
                if (GregTech_API.sServerTickCounter % 120 == 0) {
                    ...
                    for (int i = 0; i < 36; i++) {
                        if (event.player.inventory.getStackInSlot(i) != null) {
                            tCount += (event.player.inventory.getStackInSlot(i).stackSize * 64) / Math.max(
                                1,
                                event.player.inventory.getStackInSlot(i)
                                    .getMaxStackSize());
                            if (GT_Mod.sInventoryUnification)
                                GT_OreDictUnificator.setStack(true, event.player.inventory.getStackInSlot(i));
                        }
                    }
                    for (int i = 0; i < 4; i++) {
                    ...
                    if (GT_Mod.sHungerEffect && GregTech_API.sServerTickCounter % 2400 == 1200)
                        event.player.addExhaustion(Math.max(1.0F, tCount / 666.6F));
                }
            }
        }
    }
