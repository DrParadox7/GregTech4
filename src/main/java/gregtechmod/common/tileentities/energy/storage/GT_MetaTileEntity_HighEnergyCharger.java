    ...
    public void onPreTick() {
        ...
        if (this.getBaseMetaTileEntity()
            ...
            this.getBaseMetaTileEntity()
                .setActive(
                    GT_ModHandler.isElectricItem(super.mInventory[0], (byte) 7)
                        && GT_ModHandler.canUseElectricItem(super.mInventory[0], 1));
        }

    }

    public int getTextureIndex(byte aSide, byte aFacing, boolean aActive, boolean aRedstone) {
        return aSide == aFacing ? (aActive ? 412 : 411)
            : (aSide == GT_Utility.getOppositeSide(aFacing) ? 103 : (aSide < 2 ? 409 : 410));
    }
