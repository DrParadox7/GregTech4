...
public class GT_MetaTileEntity_DragonEggEnergySiphon extends MetaTileEntity {
    ...
    public int maxEUStore() {
        return getMinimumStoredEU() + sDragonEggEnergyPerTick * 2;
    }
    ...
    public void onConfigLoad(GT_Config aConfig) {
        sDragonEggEnergyPerTick = aConfig
            .get(GT_ConfigCategories.machineconfig, "DragonEggEnergySiphon.EnergyPerTick", 1024);
        sAllowMultipleEggs = aConfig
            .get(GT_ConfigCategories.machineconfig, "DragonEggEnergySiphon.AllowMultipleEggs", false);
            ...
            if (getBaseMetaTileEntity().isAllowedToWork() && hasEgg()) {
                ...
                if (getBaseMetaTileEntity().increaseStoredEnergyUnits(sDragonEggEnergyPerTick, false)) {
                    if (sAllowFlux && GT_Mod.TC_loaded) {
                        try {
                            if (this.findNode()) {
                                thaumcraft.common.tiles.TileNode node = (thaumcraft.common.tiles.TileNode) cachedNode
                                    .get();
                                ...
                                switch (getBaseMetaTileEntity().getRandomNumber(5000)) {
                                    case 0:
                                        aspects.add(thaumcraft.api.aspects.Aspect.MECHANISM, 3);
                                        break;
                                    case 1:
                                        aspects.add(thaumcraft.api.aspects.Aspect.VOID, 1);
                                        break;
                                    case 2:
                                        aspects.add(thaumcraft.api.aspects.Aspect.ELDRITCH, 2);
                                        break;
                                    case 3:
                                        aspects.add(thaumcraft.api.aspects.Aspect.EXCHANGE, 1);
                                        break;
                                    case 4:
                                        aspects.add(thaumcraft.api.aspects.Aspect.MAGIC, 1);
                                        break;
                                    case 5:
                                        aspects.add(thaumcraft.api.aspects.Aspect.MOTION, 3);
                                        break;
                                    case 6:
                                        aspects.add(thaumcraft.api.aspects.Aspect.AIR, 2);
                                        break;
                                    case 7:
                                        aspects.add(thaumcraft.api.aspects.Aspect.EARTH, 2);
                                        break;
                                    case 8:
                                        aspects.add(thaumcraft.api.aspects.Aspect.FIRE, 2);
                                        break;
                                    case 9:
                                        aspects.add(thaumcraft.api.aspects.Aspect.WATER, 2);
                                        break;
                                    case 10:
                                        aspects.add(thaumcraft.api.aspects.Aspect.ORDER, 2);
                                        break;
                                    case 11:
                                        aspects.add(thaumcraft.api.aspects.Aspect.ENTROPY, 2);
                                        break;
                                }
                                ...
                                if (!aspects.aspects.isEmpty()) {
                                    ...
                                    thaumcraft.common.lib.network.PacketHandler.INSTANCE.sendToAllAround(
                                        new thaumcraft.common.lib.network.fx.PacketFXEssentiaSource(
                                            node.xCoord,
                                            node.yCoord,
                                            node.zCoord,
                                            (byte) (node.xCoord - tThis.xCoord),
                                            (byte) (node.yCoord - tThis.yCoord),
                                            (byte) (node.zCoord - tThis.zCoord),
                                            toAdd.getColor()),
                                        new NetworkRegistry.TargetPoint(
                                            node.getWorldObj().provider.dimensionId,
                                            node.xCoord,
                                            node.yCoord,
                                            node.zCoord,
                                            32.0));
                ...
                if (mActiveSiphon.get() != this && !sAllowMultipleEggs) if (mActiveSiphon.get() == null
                    || mActiveSiphon.get()
                ...
                else getBaseMetaTileEntity().doExplosion(Integer.MAX_VALUE);
            } else {
    ...
    private boolean findNode() {
        List<ArrayList<Integer>> nodesCoords = thaumcraft.common.tiles.TileNode.locations.entrySet()
            .stream()
        ...
        if (coords != null && !coords.isEmpty()) {
            ...
            if (tr != null && tr instanceof thaumcraft.common.tiles.TileNode) {
                ...
                thaumcraft.common.tiles.TileNode.locations
                    .remove(String.format("%d:%d:%d:%d", coords.get(0), coords.get(1), coords.get(2), coords.get(3)));
            }
        }
