...
public class GT_MetaTileEntity_MagicEnergyAbsorber extends MetaTileEntity {
    ...
    public void onRightclick(EntityPlayer aPlayer) {
        getBaseMetaTileEntity().openGUI(aPlayer, 126);
    }
    ...
    public void onConfigLoad(GT_Config aConfig) {
        sEnergyPerEnderCrystal = aConfig
            .get(GT_ConfigCategories.machineconfig, "MagicEnergyAbsorber.EnergyPerTick.EnderCrystal", 32);
    ...
    public void onPostTick() {
        if (getBaseMetaTileEntity().isServerSide() && energySource == null
        ...
        if (getBaseMetaTileEntity().isServerSide() && getBaseMetaTileEntity().isAllowedToWork()
            && getBaseMetaTileEntity().getTimer() % 10 == 0) {
            if (getBaseMetaTileEntity().getUniversalEnergyStored()
                < (getBaseMetaTileEntity().getOutputVoltage() * 10 + getMinimumStoredEU())) {
                try {
                    if (mInventory[0] != null && mInventory[1] == null) {
                        if (mInventory[0].isItemEnchanted() && mInventory[0].getItem()
                                    ...
                                    if (tID > -1 && tID < Enchantment.enchantmentsList.length) {
                                        ...
                                        if (tEnchantment != null) {
                                            getBaseMetaTileEntity().increaseStoredEnergyUnits(
                                                (1_000_000 * tLevel)
                                                    / (tEnchantment.getMaxLevel() * tEnchantment.getWeight()),
                                                true);
                                        }
                                    }
                                }
                            ...
                            NBTTagList tEnchantments = ((ItemEnchantedBook) mInventory[0].getItem())
                                .func_92110_g(mInventory[0]);
                            if (tEnchantments != null) {
                                for (int i = 0; i < tEnchantments.tagCount(); i++) {
                                    short tID = ((NBTTagCompound) tEnchantments.getCompoundTagAt(i)).getShort("id");
                                    short tLevel = ((NBTTagCompound) tEnchantments.getCompoundTagAt(i)).getShort("lvl");
            ...
            if (sEnergyPerEnderCrystal > 0 && isActive1) {
                if (mTargetedCrystal == null) {
                    ArrayList<EntityEnderCrystal> tList = (ArrayList<EntityEnderCrystal>) getBaseMetaTileEntity()
                        .getWorld()
            ...
            if (sEnergyFromVis > 0 && isActive2
                && getBaseMetaTileEntity().getUniversalEnergyStored() < sEnergyFromVis
                && GT_Mod.TC_loaded) {
                try {

                    if (energySource != null) {
                        if (thaumcraft.api.aspects.AspectSourceHelper.drainEssentia(
                            (TileEntity) this.getBaseMetaTileEntity(),
                            (thaumcraft.api.aspects.Aspect) energySource,
                            ForgeDirection.UNKNOWN,
                            20)) {
                            getBaseMetaTileEntity().increaseStoredEnergyUnits(sEnergyFromVis, true);
                            List<?> tList = getBaseMetaTileEntity().getWorld()
                                .getEntitiesWithinAABB(
                                    thaumcraft.common.entities.monster.EntityWisp.class,
                                    AxisAlignedBB.getBoundingBox(
                                        getBaseMetaTileEntity().getXCoord() - 8,
                                        getBaseMetaTileEntity().getYCoord() - 8,
                                        getBaseMetaTileEntity().getZCoord() - 8,
                                        getBaseMetaTileEntity().getXCoord() + 8,
                                        getBaseMetaTileEntity().getYCoord() + 8,
                                        getBaseMetaTileEntity().getZCoord() + 8));
            ...
            if (getBaseMetaTileEntity() != null) getBaseMetaTileEntity().setActive(
                getBaseMetaTileEntity().getUniversalEnergyStored()
                    >= getBaseMetaTileEntity().getOutputVoltage() + getMinimumStoredEU());
        }
    }
                ...
                for (int i = 0; i < iterations; i++) {
                    ...
                    if (bg.biomeID != thaumcraft.common.lib.world.ThaumcraftWorldGenerator.biomeTaint.biomeID) {
                        thaumcraft.common.lib.utils.Utils
                            .setBiomeAt(tWorld, x, z, thaumcraft.common.lib.world.ThaumcraftWorldGenerator.biomeTaint);
                    }
