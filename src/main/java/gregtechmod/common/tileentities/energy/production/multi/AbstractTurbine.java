...
public abstract class AbstractTurbine extends MTEWorkableMultiblock {
    ...
    public int maxEUOutput() {
        return TURBINE_OUTPUT_EU;
    }

    @Override
    public void onRightclick(EntityPlayer aPlayer) {
        getBaseMetaTileEntity().openGUI(aPlayer, 156, GregTech_API.gregtechmod);
    }
    ...
    protected void onRecipeUpdateTick() {
        int newEfficiency = Math.min(
            mEfficiency + turbine.getEfficiencyGrowRate(),
    ...
    protected final boolean checkMachine(ItemStack aStack) {
        ...
        if (getBaseMetaTileEntity().getAirAtSideAndDistance(getBaseMetaTileEntity().getBackFacing(), 1)
            ...
            TileEntity tTileEntity = getBaseMetaTileEntity()
                .getTileEntityAtSideAndDistance(getBaseMetaTileEntity().getBackFacing(), 3);
            if (tTileEntity != null && tTileEntity instanceof IGregTechTileEntity)
                if (((IGregTechTileEntity) tTileEntity).getMetaTileEntity() != null)
                    if (((IGregTechTileEntity) tTileEntity)
                        .getMetaTileEntity() instanceof GT_MetaTileEntity_Hatch_Dynamo) {
                            mDynamoHatches.add(
                                (GT_MetaTileEntity_Hatch_Dynamo) ((IGregTechTileEntity) tTileEntity)
                                    .getMetaTileEntity());
                        } else {
                            return false;
                        }
            ...
            for (byte i = 2; i < 6; i++) {
                if (null != (tTileEntity = getBaseMetaTileEntity().getTileEntityAtSideAndDistance(i, 2))
                    && tTileEntity instanceof IGregTechTileEntity) {
                    if (((IGregTechTileEntity) tTileEntity).getFrontFacing() == getBaseMetaTileEntity().getFrontFacing()
                        && ((IGregTechTileEntity) tTileEntity).getMetaTileEntity() != null) {
                        if (((IGregTechTileEntity) tTileEntity).getMetaTileEntity()
                            .getClass() == this.getClass()) return false;
                    }
                }
            }
            ...
            for (byte i = -1; i < 2; i++)
                for (byte j = -1; j < 2; j++) if (i != 0 || j != 0) for (byte k = 0; k < 4; k++) {
                    Pair<Block, Integer> hull = this.getHull();

                    if ((i == 0 || j == 0) && (k == 1 || k == 2)) {
                        if (getBaseMetaTileEntity().getBlock(
                            tX + (tSide < 4 ? i : tSide == 5 ? +k : -k),
                            tY + j,
                            tZ + (tSide < 4 ? tSide == 3 ? +k : -k : i)) != hull.getKey()
                            || getBaseMetaTileEntity().getMetaID(
                                tX + (tSide < 4 ? i : tSide == 5 ? +k : -k),
                                tY + j,
                                tZ + (tSide < 4 ? tSide == 3 ? +k : -k : i)) != hull.getValue()) {
                            tTileEntity = getBaseMetaTileEntity().getTileEntity(
                                tX + (tSide < 4 ? i : tSide == 5 ? +k : -k),
                                tY + j,
                                tZ + (tSide < 4 ? tSide == 3 ? +k : -k : i));
                            if (tTileEntity != null && tTileEntity instanceof IGregTechTileEntity
                                && ((IGregTechTileEntity) tTileEntity).getMetaTileEntity() != null) {
    ...
    public Map<String, List<Object>> getInfoData() {
        InfoBuilder b = InfoBuilder.create()
            .newKey(
                "metatileentity.turbine.speed",
                GT_Utility.parseNumberToString(mEfficiency * 100.0D / MAX_EFFICIENCY))
            .newKey(
                "metatileentity.multiblock.dynamo_output",
                (int) (TURBINE_OUTPUT_EU * mEfficiency * 1.0D / MAX_EFFICIENCY))
        ...
        if (isCorrectMachinePart(mInventory[1])) {
            ...
            if (this.turbine == Turbines.RC) damage /= 2;
    ...
    public static class MultiTurbineLogic extends GeneratorRecipeLogic {
        ...
        protected MultiTurbineLogic(IntSupplier efficiency, RecipeMap<?> recipeMap, IRecipeWorkable machine) {
            ...
            metadataVerifier = rec -> rec.getInputs()
                .isEmpty()
                && rec.getAllOutputs()
                    .isEmpty(); // only fluid recipes allow
        }
        ...
        protected boolean depleteInputs() {
            for (FluidStack fluid : previousRecipe.getFluidInputs()) {
                ...
                if (!getMachine().depleteInput(dummy)) return false;
            }
        ...
        public boolean update() {
            ...
            if (base.isAllowedToWork()) {
                ...
                if (leftEU <= 0) {
                    if (getMachine().hasInventoryBeenModified() || base.hasWorkJustBeenEnabled()
                        || success
                        || base.getTimer() % 600 == 0) {
                        trySerachRecipe();
                    }
        ...
        protected boolean updateRecipeProgress() {
            if ((getMachine().getBaseMetaTileEntity()
                .getTimer() % 10 != 0 || getMachine().checkAir())) {
