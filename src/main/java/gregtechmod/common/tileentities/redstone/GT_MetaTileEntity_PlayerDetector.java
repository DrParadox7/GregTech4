...
public class GT_MetaTileEntity_PlayerDetector extends MetaTileEntity {
    ...
    public boolean onRightclick(EntityPlayer aPlayer, byte aSide, float aX, float aY, float aZ) {
        ...
        if (GT_Utility.areStacksEqual(tStack, GT_OreDictUnificator.get(OrePrefixes.plate, Materials.Paper, 1L), true)) {
            NBTTagCompound tMessage = tStack.getTagCompound()
        ...
        switch (this.mMode) {
            case 0:
                ...
                break;
            case 1:
                tMessage = new ChatComponentTranslation(
                    "tile.BlockMetaID_Machine.Playerdetector.message.2",
                    mDetectedPlayer);
                break;
            case 2:
                tMessage = new ChatComponentTranslation(
                    "tile.BlockMetaID_Machine.Playerdetector.message.3",
                    mDetectedPlayer);
        }
    ...
    public void onPostTick() {
        if (this.getBaseMetaTileEntity()
            ...
            if (this.mDetectedPlayer != null && !this.mDetectedPlayer.equals("")) {
                byte tRedstone = 0;
                if (this.getBaseMetaTileEntity()
                    .decreaseStoredEnergyUnits(50, false)) {
                    Iterator<?> i = this.getBaseMetaTileEntity()
                        .getWorld().loadedEntityList.iterator();
                        ...
                        if (tObject instanceof EntityPlayer) {
                            ...
                            if (dist < 16) {
                                ...
                                if (tEntity.getDisplayName()
                                    .equalsIgnoreCase(this.mDetectedPlayer)) {
                                    if (this.mMode == 1) {
                                        tRedstone = (byte) (16 - dist);
                                        break;
                                    }
                ...
                this.getBaseMetaTileEntity()
                    .setActive(tRedstone > 0);

                for (byte var6 = 0; var6 < 6; ++var6) {
                    this.getBaseMetaTileEntity()
                        .setOutputRedstoneSignal(var6, tRedstone);
                }
            } else {
                this.mDetectedPlayer = this.getBaseMetaTileEntity()
                    .getOwnerName();
            }
        }
    ...
    public byte getComparatorValue(byte aSide) {
        return (byte) (15 - this.getBaseMetaTileEntity()
            .getOutputRedstoneSignal(aSide));
    }
