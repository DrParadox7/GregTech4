...
public class GT_MetaTileEntity_RedstoneButtonPanel extends MetaTileEntity {
    ...
    public void onPreTick() {
        if (getBaseMetaTileEntity().isServerSide()) {
            ...
            if (mUpdate > 0) mUpdate--;
            else if (getBaseMetaTileEntity().isAllowedToWork()) mRedstoneStrength = 0;
            for (byte i = 0; i < 6; i++) getBaseMetaTileEntity().setInternalOutputRedstoneSignal(
                i,
    ...
    public boolean allowCoverOnSide(byte aSide, int aCoverID) {
        return aSide != getBaseMetaTileEntity().getFrontFacing() || GregTech_API.getCoverBehavior(aCoverID)
