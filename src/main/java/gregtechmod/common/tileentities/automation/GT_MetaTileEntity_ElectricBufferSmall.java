...
public class GT_MetaTileEntity_ElectricBufferSmall extends MetaTileEntity {
    ...
    public void onRightclick(EntityPlayer aPlayer) {
        getBaseMetaTileEntity().openGUI(aPlayer, 102);
    }
    ...
    public void onPostTick() {
        if (getBaseMetaTileEntity().isAllowedToWork() && getBaseMetaTileEntity().isServerSide()
            && getBaseMetaTileEntity().getUniversalEnergyStored() >= 500
            && (getBaseMetaTileEntity().hasWorkJustBeenEnabled() || getBaseMetaTileEntity().getTimer() % 200 == 0
                || (getBaseMetaTileEntity().getTimer() % 5 == 0 && (mSuccess > 0
                    || (mInventory[0] != null && getBaseMetaTileEntity().getTimer() % 10 == 0 && getInvSize() <= 2)))
                || mSuccess >= 20
    ...
    public int getTextureIndex(byte aSide, byte aFacing, boolean aActive, boolean aRedstone) {
        if (aSide == aFacing) return 130 + (aRedstone ? 8 : 0);
