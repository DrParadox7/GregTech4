...
public class GT_MetaTileEntity_CropHarvestor extends GT_MetaTileEntity_ElectricBufferSmall {
    ...
    public int getMinimumStoredEU() {
        return 1000 + (int) Math.pow(4, getBaseMetaTileEntity().getOverclockerUpgradeCount()) * 100;
    }
    ...
    public void onRightclick(EntityPlayer aPlayer) {
        getBaseMetaTileEntity().openGUI(aPlayer, 110);
    }
    ...
    public void onPostTick() {
        if (getBaseMetaTileEntity().isServerSide() && getBaseMetaTileEntity().isAllowedToWork()
            ...
            if (getBaseMetaTileEntity().getUniversalEnergyStored() >= 100 * tOverclockerFactor) {
                for (int i = 0, j = (int) Math.pow(2, getBaseMetaTileEntity().getOverclockerUpgradeCount()); i
                    < j; i++) {
                    TileEntity tTileEntity = getBaseMetaTileEntity()
                        .getTileEntityAtSideAndDistance(getBaseMetaTileEntity().getFrontFacing(), i + 1);
                    try {
                        if (tTileEntity != null && tTileEntity instanceof ic2.api.crops.ICropTile) {
                            if (!ic2.api.crops.Crops.instance.getCropList()[((ic2.api.crops.ICropTile) tTileEntity)
                                .getID()].canGrow((ic2.api.crops.ICropTile) tTileEntity)
                                && ic2.api.crops.Crops.instance.getCropList()[((ic2.api.crops.ICropTile) tTileEntity)
                                    .getID()].canBeHarvested((ic2.api.crops.ICropTile) tTileEntity)) {
                    ...
                    if (mInventory[0] == null
                        && getBaseMetaTileEntity().getUniversalEnergyStored() >= 64 * tOverclockerFactor
                        && (mInventory[0] = GT_Utility.suckOneItemStackAt(
                            ...
                            getBaseMetaTileEntity().getOffsetZ(getBaseMetaTileEntity().getFrontFacing(), 1 + i),
                            1,
                            1,
                            1)) != null) {
                        getBaseMetaTileEntity()
                            .decreaseStoredEnergyUnits(mInventory[0].stackSize * tOverclockerFactor, true);
                        break;
                    }
                }
            }
    ...
    public int getTextureIndex(byte aSide, byte aFacing, boolean aActive, boolean aRedstone) {
        if (aSide == aFacing) return 118 + (aRedstone ? 8 : 0);
