...
public class GT_MetaTileEntity_ElectricTypeFilter extends GT_MetaTileEntity_ElectricBufferSmall {
    ...
    public void clickTypeIcon(boolean aRightClick) {
        if (this.getBaseMetaTileEntity()
            .isServerSide()) {
            label37: for (int i = 0; i < OrePrefixes.values().length; ++i) {
    ...
    public void onPreTick() {
        if (this.getBaseMetaTileEntity()
            ...
            if (this.mPrefix.mPrefixedItems.isEmpty()) {
                ...
                super.mInventory[9] = GT_Utility.copyAmount(
                    1L,
    ...
    public boolean allowPutStack(int aIndex, byte aSide, ItemStack aStack) {
        return aSide != this.getBaseMetaTileEntity()
            .getBackFacing() && (this.bNBTAllowed || !aStack.hasTagCompound())
            && this.mPrefix.contains(aStack) != this.bInvertFilter;
    }
