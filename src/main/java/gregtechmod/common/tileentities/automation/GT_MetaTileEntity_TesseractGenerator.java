...
public class GT_MetaTileEntity_TesseractGenerator extends MetaTileEntity {
    ...
    public int getProgresstime() {
        return sTesseractGenerators.get(mFrequency) == this && isWorking >= 20 ? 999 : 0;
    }
    ...
    public void onConfigLoad(GT_Config aConfig) {
        ...
        TESSERACT_ENERGY_COST_DIMENSIONAL = aConfig
            .get(GT_ConfigCategories.machineconfig, "Tesseract.InterDimensionalEnergyPerTick", 32);
    }
    ...
    public boolean onRightclick(EntityPlayer aPlayer, byte aSide, float aX, float aY, float aZ) {
        if (aSide == getBaseMetaTileEntity().getFrontFacing()) {
            ...
            int var = (sTesseractGenerators.get(mFrequency) != null && sTesseractGenerators.get(mFrequency) != this) ? 2
                : 1;
            GT_Utility.sendChatToPlayer(
                aPlayer,
                new ChatComponentTranslation("metatileentity.GT_Tesseract.message." + var, mFrequency));
        }
    ...
    public void onScrewdriverRightClick(byte aSide, EntityPlayer aPlayer, float aX, float aY, float aZ) {
        if (aSide == getBaseMetaTileEntity().getFrontFacing()) {
            ...
            int type = sTesseractGenerators.get(mFrequency) != null && sTesseractGenerators.get(mFrequency) != this ? 2
                : 1;
            GT_Utility.sendChatToPlayer(
                aPlayer,
                new ChatComponentTranslation("metatileentity.GT_Tesseract.message." + type, mFrequency));
        }
    }
    ...
    public Map<String, List<Object>> getInfoData() {
        return InfoBuilder.create()
            ...
            .newKey(
                "metatileentity.GT_Tesseract.status",
                "metatileentity.GT_Tesseract.status." + (sTesseractGenerators.get(mFrequency) == this ? 1 : 2))
            .build();
    }
    ...
    public boolean canFill(ForgeDirection aSide, Fluid aFluid) {
        IFluidHandler tTileEntity = getBaseMetaTileEntity()
            .getITankContainerAtSide(getBaseMetaTileEntity().getBackFacing());
        if (tTileEntity == null || !getBaseMetaTileEntity().isAllowedToWork()) return false;
    ...
    public boolean addEnergyConsumption(GT_MetaTileEntity_TesseractTerminal aTerminal) {
        ...
        tNeededEnergy += aTerminal.getBaseMetaTileEntity()
            .getWorld() == getBaseMetaTileEntity().getWorld() ? TESSERACT_ENERGY_COST
                : TESSERACT_ENERGY_COST_DIMENSIONAL;
        return true;
    }

    public boolean isValidTesseractGenerator(String aOwnerName, boolean aWorkIrrelevant) {
        return getBaseMetaTileEntity() != null && !getBaseMetaTileEntity().isInvalidTileEntity()
            && getBaseMetaTileEntity().isAllowedToWork()
            && (aOwnerName == null || getBaseMetaTileEntity().getOwnerName()
                .equals(aOwnerName))
            && (aWorkIrrelevant || isWorking >= 20);
    }
    ...
    public void onPostTick() {
        if (getBaseMetaTileEntity().isServerSide()) {
            ...
            if (getBaseMetaTileEntity().isAllowedToWork()
                && getBaseMetaTileEntity().decreaseStoredEnergyUnits(tNeededEnergy, false)) {
                if (sTesseractGenerators.get(mFrequency) == null || !sTesseractGenerators.get(mFrequency)
