...
public class GT_MetaTileEntity_ElectricAutoWorkbench extends GT_MetaTileEntity_BasicTank {
    ...
    public void onRightclick(EntityPlayer aPlayer) {
        getBaseMetaTileEntity().openGUI(aPlayer, 100);
    }
                    ...
                    if (tOutput != null) {
                        for (byte j = 0; j < 9; j++) {
                            if (mInventory[j] == null || (GT_Utility.areStacksEqual(tOutput, mInventory[j])
                                ...
                                mFluid[0].amount -= GT_Utility.getFluidForFilledItem(tOutput).amount
                                    * tOutput.stackSize;
    ...
    public int getTextureIndex(byte aSide, byte aFacing, boolean aActive, boolean aRedstone) {
        if (aSide == aFacing) return 112;
        if (GT_Utility.getOppositeSide(aSide) == aFacing) return 113;
        return 114;
    }
