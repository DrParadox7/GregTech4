...
public class GT_MetaTileEntity_ElectricRetrieverAdvanced extends MetaTileEntity {
    ...
    public boolean isOutputFacing(byte aSide) {
        return aSide == this.getBaseMetaTileEntity()
            .getBackFacing();
    }
    ...
    public void onPostTick() {
        if (this.getBaseMetaTileEntity()
            ...
            && this.getBaseMetaTileEntity()
                .isServerSide()
            && this.getBaseMetaTileEntity()
                .isUniversalEnergyStored(2000)
            && (this.getBaseMetaTileEntity()
                .hasWorkJustBeenEnabled()
                || this.getBaseMetaTileEntity()
                    .getTimer() % 50L == 0L)) {
            IInventory tInventory = this.getBaseMetaTileEntity()
                .getIInventoryAtSide(
                    this.getBaseMetaTileEntity()
                        .getBackFacing());
            IGregTechTileEntity tConnectedPipe = this.getBaseMetaTileEntity()
                .getIGregTechTileEntityAtSide(
                    this.getBaseMetaTileEntity()
                        .getFrontFacing());
            if (tInventory != null && tConnectedPipe != null
                && tConnectedPipe.getMetaTileEntity() != null
                && tConnectedPipe.getMetaTileEntity() instanceof GT_MetaPipeEntity_Item) {
                boolean temp = false;
                ...
                for (int tMap = 0; tMap < 9; ++tMap) {
                    ...
                    if (super.mInventory[tMap] != null) {
                        ...
                        if (tList == null || GT_Utility.areStacksEqual(tList, super.mInventory[tMap])
                            && tList.stackSize < super.mInventory[tMap].stackSize) {
                            temp = true;
                ...
                if (temp) {
                    ...
                    LinkedHashMap<GT_MetaPipeEntity_Item, Long> pipes = GT_Utility.sortMapByValuesAcending(
                        GT_Utility.scanPipes(
                            (GT_MetaPipeEntity_Item) tConnectedPipe.getMetaTileEntity(),
                            new HashMap<>(),
                            0,
                            true));
                    ...
                    for (GT_MetaPipeEntity_Item tPipe : pipes.keySet()) {
                        if (!temp) {
                            ...
                            for (Iterator<GT_MetaPipeEntity_Item> iter = pipes1.iterator(); iter
                                .hasNext(); ++tmpPipe.mTransferredItems) {
                                tmpPipe = iter.next();
                            }
                        ...
                        for (byte tSide = 0; temp && tSide < 6; ++tSide) {
                            if (tPipe.getBaseMetaTileEntity()
                                .getCoverBehaviorAtSide(tSide)
                                .letsItemsIn(
                                    tSide,
                                    tPipe.getBaseMetaTileEntity()
                                        .getCoverIDAtSide(tSide),
                                    tPipe.getBaseMetaTileEntity()
                                        .getCoverDataAtSide(tSide),
                                    tPipe.getBaseMetaTileEntity())) {
                                IInventory tTileEntity = tPipe.getBaseMetaTileEntity()
                                    .getIInventoryAtSide(tSide);
                                if (tTileEntity != tInventory) {
                                    for (int i = 0; temp && i < 9; ++i) {
                                        if (tNeedsItems[i]) {
                                            int itemsMoved = GT_Utility.moveOneItemStackIntoSlot(
                                                tTileEntity,
                                                tInventory,
                                                GT_Utility.getOppositeSide(tSide),
                                                this.mTargetSlots[i],
                                                Arrays.asList(new ItemStack[] { super.mInventory[i] }),
                                                false,
                                                (byte) super.mInventory[i].stackSize,
                                                this.mPartialRequests ? 1 : (byte) super.mInventory[i].stackSize,
                                                (byte) 64,
                                                (byte) 1) * 20;
                                            if (itemsMoved > 0) {
                                                this.getBaseMetaTileEntity()
                                                    .decreaseStoredEnergyUnits(itemsMoved, true);
                                                temp = false;
                                            }
                                        }
                                    }
