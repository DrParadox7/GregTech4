...
public class GT_MetaTileEntity_ElectricRegulatorAdvanced extends GT_MetaTileEntity_ElectricBufferSmall {
    ...
    public void onRightclick(EntityPlayer aPlayer) {
        getBaseMetaTileEntity().openGUI(aPlayer, 123);
    }
    ...
    public void onPostTick() {
        if (getBaseMetaTileEntity().isAllowedToWork() && getBaseMetaTileEntity().isServerSide()
            ...
            && (getBaseMetaTileEntity().hasWorkJustBeenEnabled() || getBaseMetaTileEntity().getTimer() % 10 == 0)) {
    ...
    public int getTextureIndex(byte aSide, byte aFacing, boolean aActive, boolean aRedstone) {
        if (aSide == aFacing) return 135 + (aRedstone ? 8 : 0);
