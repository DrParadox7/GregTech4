...
public class GT_MetaTileEntity_ElectricItemClearer extends GT_MetaTileEntity_ElectricBufferSmall {
    ...
    public void onRightclick(EntityPlayer aPlayer) {
        getBaseMetaTileEntity().openGUI(aPlayer, 108);
    }
    ...
    public void onPostTick() {
        if (getBaseMetaTileEntity().isAllowedToWork() && getBaseMetaTileEntity().isServerSide()
            ...
            && getBaseMetaTileEntity().getUniversalEnergyStored()
                >= 64 * (int) Math.pow(2, getBaseMetaTileEntity().getOverclockerUpgradeCount())
            && (getBaseMetaTileEntity().hasWorkJustBeenEnabled() || getBaseMetaTileEntity().getTimer() % 20 == 0
                || mSuccess == 20)) {
            if (null != (mInventory[0] = GT_Utility.suckOneItemStackAt(
                getBaseMetaTileEntity().getWorld(),
                getBaseMetaTileEntity().getOffsetX(
                    getBaseMetaTileEntity().getFrontFacing(),
                    (2 + getBaseMetaTileEntity().getOverclockerUpgradeCount()))
                    - (1 + getBaseMetaTileEntity().getOverclockerUpgradeCount()),
                ...
                getBaseMetaTileEntity().getOffsetZ(
                    ...
                    (2 + getBaseMetaTileEntity().getOverclockerUpgradeCount()))
                    - (1 + getBaseMetaTileEntity().getOverclockerUpgradeCount()),
                3 + getBaseMetaTileEntity().getOverclockerUpgradeCount() * 2,
                3 + getBaseMetaTileEntity().getOverclockerUpgradeCount() * 2,
                3 + getBaseMetaTileEntity().getOverclockerUpgradeCount() * 2))) {
                getBaseMetaTileEntity().decreaseStoredEnergyUnits(
                    mInventory[0].stackSize * (int) Math.pow(2, getBaseMetaTileEntity().getOverclockerUpgradeCount()),
                    true);
                mSuccess = 20;
            }
        }
    ...
    public int getTextureIndex(byte aSide, byte aFacing, boolean aActive, boolean aRedstone) {
        if (aSide == aFacing) return 117 + (aRedstone ? 8 : 0);
