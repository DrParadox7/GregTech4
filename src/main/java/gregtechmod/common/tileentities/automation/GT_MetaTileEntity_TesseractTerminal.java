...
public class GT_MetaTileEntity_TesseractTerminal extends MetaTileEntity {
    ...
    public int getProgresstime() {
        return getTesseract(mFrequency, false) != null ? 999 : 0;
    }
    ...
    public void onConfigLoad(GT_Config aConfig) {
        sInterDimensionalTesseractAllowed = aConfig
            .get(GT_ConfigCategories.machineconfig, "Tesseract.Interdimensional", true);
    }
        ...
        if (aSide == getBaseMetaTileEntity().getFrontFacing()) {
            ...
            GT_Utility.sendChatToPlayer(
                ...
                new ChatComponentTranslation(
                    "metatileentity.GT_Tesseract.message." + (getTesseract(mFrequency, false) == null ? 1 : 3),
                    mFrequency));
        }
    ...
    public GT_MetaTileEntity_TesseractGenerator getTesseract(int aFrequency, boolean aWorkIrrelevant) {
        GT_MetaTileEntity_TesseractGenerator rTesseract = GT_MetaTileEntity_TesseractGenerator.sTesseractGenerators
            .get(aFrequency);
        if (rTesseract == null
            || !rTesseract.isValidTesseractGenerator(getBaseMetaTileEntity().getOwnerName(), aWorkIrrelevant))
            return null;
        if (!sInterDimensionalTesseractAllowed && rTesseract.getBaseMetaTileEntity()
            .getWorld() != getBaseMetaTileEntity().getWorld()) return null;
        return rTesseract;
    }
    ...
    public Map<String, List<Object>> getInfoData() {
        return InfoBuilder.create()
            ...
            .newKey(
                ...
                "metatileentity.GT_Tesseract.status." + getTesseract(mFrequency, false) != null ? 1 : 2)
            .build();
    }
