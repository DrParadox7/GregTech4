...
public class GT_MetaTileEntity_AdvancedTranslocator extends MetaTileEntity {
    ...
    public boolean isOutputFacing(byte aSide) {
        return getBaseMetaTileEntity().getFrontFacing() == aSide || getBaseMetaTileEntity().getBackFacing() == aSide;
    }
    ...
    public void onRightclick(EntityPlayer aPlayer) {
        getBaseMetaTileEntity().openGUI(aPlayer, 104);
    }
    ...
    public void onPostTick() {
        if (getBaseMetaTileEntity().isAllowedToWork() && getBaseMetaTileEntity().isServerSide()
            && getBaseMetaTileEntity().getUniversalEnergyStored() >= 1800
            && (getBaseMetaTileEntity().hasWorkJustBeenEnabled()
                || getBaseMetaTileEntity().getTimer()
                    % (197 - (getBaseMetaTileEntity().getOverclockerUpgradeCount() * 49)) == 0
                || (getBaseMetaTileEntity().getTimer() % 5 == 0 && mSuccess > 0)
                || mSuccess >= 20)) {
            mSuccess--;
            ArrayList<ItemStack> tList = new ArrayList<ItemStack>();
            for (int i = 0; i < 9; i++) tList.add(mInventory[i]);
            int tCost = (getBaseMetaTileEntity().getOverclockerUpgradeCount() + 1)
                * GT_Utility.moveOneItemStack(
                    getBaseMetaTileEntity().getIInventoryAtSide(getBaseMetaTileEntity().getFrontFacing()),
                    ...
                    (byte) 64,
                    (byte) 1)
                * ((mInventory[0] == null && mInventory[1] == null
                    && mInventory[2] == null
                    && mInventory[3] == null
                    && mInventory[4] == null
                    && mInventory[5] == null
    ...
    public int getTextureIndex(byte aSide, byte aFacing, boolean aActive, boolean aRedstone) {
        if (aSide == aFacing) return 112 + (aRedstone ? 8 : 0);
