                ...
                if (tTileEntity != null) {
                    int tFilledAmount = tTileEntity.fill(
                        ForgeDirection.getOrientation(i)
                            .getOpposite(),
                        getBaseMetaTileEntity().drain(ForgeDirection.getOrientation(i), 1000, false),
                        false);
                    if (tFilledAmount > 0) {
                        tTileEntity.fill(
                            ForgeDirection.getOrientation(i)
                                .getOpposite(),
                            getBaseMetaTileEntity().drain(ForgeDirection.getOrientation(i), tFilledAmount, true),
                            true);
                    }
                }
            }
    ...
    public void doSound(byte aIndex, double aX, double aY, double aZ) {
        if (aIndex == 1) {
            ...
            for (int l = 0; l < 8; ++l) getBaseMetaTileEntity().getWorld()
                .spawnParticle("largesmoke", aX - 0.5 + Math.random(), aY, aZ - 0.5 + Math.random(), 0, 0, 0);
        }
    }
