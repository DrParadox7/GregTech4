...
public class GT_MetaTileEntity_Teleporter extends MetaTileEntity {
    ...
    public int maxEUInput() {
        return GregTech_API.VOLTAGE_ULTIMATE;
    }
    ...
    public void onConfigLoad(GT_Config aConfig) {
        sInterDimensionalTeleportAllowed = aConfig
            .get(GT_ConfigCategories.machineconfig, "Teleporter.Interdimensional", true);
    }
    ...
    public boolean checkForEgg() {
        for (byte i = -5; i <= 5; i++) for (byte j = -5; j <= 5; j++) for (byte k = -5; k <= 5; k++)
            if (getBaseMetaTileEntity().getBlockOffset(i, j, k) == Blocks.dragon_egg) return true;
        return false;
    }
    ...
    public boolean isDimensionalTeleportAvailable() {
        return mDebug || (hasDimensionalTeleportCapability() && GT_Utility.isRealDimension(mTargetD)
            ...
            if (getBaseMetaTileEntity().isAllowedToWork() && getBaseMetaTileEntity().getRedstone()) {
                ...
                for (Object tObject : getBaseMetaTileEntity().getWorld()
                    ...
                    if (tObject instanceof Entity && !((Entity) tObject).isDead) {
                        ...
                        if ((mCharge >= (tCost = (int) (tDistance * tDistance * weightCalculation(tEntity)))
                            && tCost >= 0) || mDebug) {
                            ...
                            if (mTargetD != getBaseMetaTileEntity().getWorld().provider.dimensionId
                                ...
                                if (tEntity instanceof EntityLiving) {
                                    ((EntityLiving) tEntity)
                                        .setPositionAndUpdate(mTargetX + 0.5, mTargetY + 0.5, mTargetZ + 0.5);
                                } else {
                                    if (tEntity instanceof EntityPlayer) {
                                        ((EntityPlayerMP) tEntity).playerNetServerHandler.setPlayerLocation(
                                            mTargetX + 0.5,
                                            mTargetY + 0.5,
                                            mTargetZ + 0.5,
                                            tEntity.rotationYaw,
                                            tEntity.rotationPitch);
                                    } else {
                            ...
                            GT_Log.log.warn(
                                String.format(
                                    "Teleported %s from (%s %s %s, DIM: %s) to (%s %s %s, DIM: %s)",
                                    tEntity.toString(),
                                    ...
                                    mTargetY,
                                    mTargetZ,
                                    DimensionManager.createProviderFor(mTargetD)
                                        .getDimensionName()));
                        } else {
    ...
    private int distanceCalculation() {
        return Math.abs(
            (mTargetD != getBaseMetaTileEntity().getWorld().provider.dimensionId && isDimensionalTeleportAvailable()
                ? 100
                : 1)
                * (int) Math.sqrt(
                    Math.pow(getBaseMetaTileEntity().getXCoord() - mTargetX, 2)
                        + Math.pow(getBaseMetaTileEntity().getYCoord() - mTargetY, 2)
                        + Math.pow(getBaseMetaTileEntity().getZCoord() - mTargetZ, 2)));
    }
    ...
    private float weightCalculation(Entity aEntity) {
        ...
        if (aEntity instanceof EntityPlayer) {
            ...
            int tCount = 64;
            for (int i = 0; i < 36; i++)
                if (tPlayer.inventory.getStackInSlot(i) != null) tCount += (tPlayer.inventory.getStackInSlot(i)
        ...
        if (GT_Utility.getClassName(aEntity)
            .equals("EntityItnt")) return 5.0F;
        if (GT_Utility.getClassName(aEntity)
            .equals("EntityNuke")) return 50.0F;
    ...
    public Map<String, List<Object>> getInfoData() {
        return InfoBuilder.create()
            ...
            .newKey(
                "metatileentity.GT_Teleporter.info.4",
                DimensionManager.createProviderFor(mTargetD)
                    .getDimensionName())
            .build();
    }
