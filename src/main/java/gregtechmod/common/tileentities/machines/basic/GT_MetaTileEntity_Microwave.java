...
public class GT_MetaTileEntity_Microwave extends GT_MetaTileEntity_BasicMachine {
                ...
                for (int i = 0; i < inputs.size(); i++) {
                    ...
                    if (GT_Utility.isStackValid(input) && (output = checkForExlosion(input, i)) != null) {
                        if (TileEntityFurnace.getItemBurnTime(input) > 0
                            || TileEntityFurnace.getItemBurnTime(output) > 0) {
                            inputs.set(i, null);
    ...
    private ItemStack checkForExlosion(ItemStack input, int inIdx) {
        ...
        if (input.getItem() == GT_Items.Spray_CFoam.getItem()
            ...
            ent.doExplosion(128);
            try {
                ItemStack foamSprayer = GT_ModHandler
                    .getIC2Item("constructionFoamSprayer", 1, new ItemStack(Blocks.sponge, 1));
                ...
                for (int i = 0; i < 6; i++) {
                    foamSprayer.getItem()
                        .onItemUse(
                            foamSprayer,
                            new FakePlayer((WorldServer) ent.getWorld(), new GameProfile(UUID.randomUUID(), "Foo")),
                            ent.getWorld(),
                            ent.getXCoord(),
                            ent.getYCoord(),
                            ent.getZCoord(),
            ...
            if (OrePrefixes.ingot.contains(input) || OrePrefixes.ingot.contains(tOutput)
