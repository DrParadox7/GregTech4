...
public class GT_MetaTileEntity_AdvancedPump extends GT_MetaTileEntity_BasicTank {
    ...
    public void onRightclick(EntityPlayer aPlayer) {
        getBaseMetaTileEntity().openGUI(aPlayer, 130);
    }
    ...
    public void onPostTick() {
        if (getBaseMetaTileEntity().isServerSide() && getBaseMetaTileEntity().getTimer() % 10 == 0) {
            ...
            if (getBaseMetaTileEntity().isAllowedToWork() && getBaseMetaTileEntity().getUniversalEnergyStored() > 10000
                && (mFluid[0] == null || mFluid[0].amount + 1000 <= getCapacity())) {
                ...
                if (mPumpedBlock1 == null || mPumpedBlock2 == null) {
                    ...
                    if (mPumpedBlock1 == null || mPumpedBlock2 == null) {
                        getFluidAt(
                            getBaseMetaTileEntity().getXCoord(),
                            getYOfPumpHead(),
                            getBaseMetaTileEntity().getZCoord() + 1);
                    }
                    if (mPumpedBlock1 == null || mPumpedBlock2 == null) {
                        getFluidAt(
                            getBaseMetaTileEntity().getXCoord(),
                    ...
                    if (mPumpedBlock1 == null || mPumpedBlock2 == null) {
                        getFluidAt(
                            getBaseMetaTileEntity().getXCoord() + 1,
                            getYOfPumpHead(),
                            getBaseMetaTileEntity().getZCoord());
                    }
                    if (mPumpedBlock1 == null || mPumpedBlock2 == null) {
                    ...
                    if (getYOfPumpHead() < getBaseMetaTileEntity().getYCoord()) {
                        if (tMovedOneDown || (mPumpList.isEmpty() && getBaseMetaTileEntity().getTimer() % 200 == 100)
                            || getBaseMetaTileEntity().getTimer() % 72000 == 100) {
                            mPumpList.clear();
                            for (int y = getBaseMetaTileEntity().getYCoord() - 1,
                                yHead = getYOfPumpHead(); mPumpList.isEmpty() && y >= yHead; y--) {
                                scanForFluid(
                                    getBaseMetaTileEntity().getXCoord(),
                                    y,
                                    getBaseMetaTileEntity().getZCoord(),
                        ...
                        if (!tMovedOneDown && !mPumpList.isEmpty()) {
                            consumeFluid(
                                mPumpList.get(mPumpList.size() - 1).chunkPosX,
                                mPumpList.get(mPumpList.size() - 1).chunkPosY,
                                mPumpList.get(mPumpList.size() - 1).chunkPosZ);
    ...
    private boolean moveOneDown() {
        if (mInventory[0] == null || mInventory[0].stackSize < 1
            || !GT_Utility.areStacksEqual(mInventory[0], GT_ModHandler.getIC2Item("miningPipe", 1))) return false;
        int yHead = getYOfPumpHead();
        if (yHead <= 0) return false;
        if (!consumeFluid(getBaseMetaTileEntity().getXCoord(), yHead - 1, getBaseMetaTileEntity().getZCoord())
            && !getBaseMetaTileEntity()
                .getAir(getBaseMetaTileEntity().getXCoord(), yHead - 1, getBaseMetaTileEntity().getZCoord()))
            return false;
        if (!getBaseMetaTileEntity().getWorld()
            .setBlock(
                ...
                Block.getBlockFromItem(
                    GT_ModHandler.getIC2Item("miningPipeTip", 1)
                        .getItem())))
            return false;
        if (yHead != getBaseMetaTileEntity().getYCoord()) getBaseMetaTileEntity().getWorld()
            .setBlock(
                getBaseMetaTileEntity().getXCoord(),
                yHead,
                getBaseMetaTileEntity().getZCoord(),
    ...
    private int getYOfPumpHead() {
        ...
        while (getBaseMetaTileEntity()
            .getBlock(getBaseMetaTileEntity().getXCoord(), y, getBaseMetaTileEntity().getZCoord())
            == Block.getBlockFromItem(
                GT_ModHandler.getIC2Item("miningPipe", 1)
                    .getItem()))
            y--;
    ...
    private void scanForFluid(int aX, int aY, int aZ, ArrayList<ChunkPosition> aList, int mX, int mZ, int mDist) {
        ...
        if (addIfFluidAndNotAlreadyAdded(aX, aY + 1, aZ, aList)
            || (aX == mX && aZ == mZ && aY < getBaseMetaTileEntity().getYCoord())) {
        ...
        if (mPumpedBlock1 == tBlock || mPumpedBlock2 == tBlock) {
            ...
            if (tBlock instanceof IFluidBlock) {
                if (mFluid[0] == null) {
                    ...
                    getBaseMetaTileEntity()
                        .decreaseStoredEnergyUnits(mFluid[0] == null ? 1000 : mFluid[0].amount, true);
                } else {
                    return false;
                }
            }
            ...
            getBaseMetaTileEntity().getWorld()
                .setBlock(aX, aY, aZ, Blocks.air, 0, 0);
            return true;
        }
        return false;
    }
