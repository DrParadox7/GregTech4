...
public class GT_MetaTileEntity_VacuumFreezer extends BaseMultiWorkable {
    ...
    public void onRightclick(EntityPlayer aPlayer) {
        getBaseMetaTileEntity().openGUI(aPlayer, 122);
    }

    @Override
    public List<ItemStack> getInputItems() {
        return new ListAdapter<>(mInventory, 0, 0);
    }

    @Override
    public List<ItemStack> getOutputItems() {
        return new ListAdapter<>(mInventory, 1, 1);
    }
    ...
    protected boolean checkMachine() {
        for (int i = -1; i < 2; i++) for (int j = -1; j < 2; j++) for (int k = -1; k < 2; k++) {
            if (i != 0 || j != 0 || k != 0) {
                ...
                if (getBaseMetaTileEntity().getMetaIDOffset(i, j - 2, k)
