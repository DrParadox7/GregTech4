...
public class GT_MetaTileEntity_Multi_ThermalBoiler extends MTEWorkableMultiblock {
    ...
    public void endProcess() {
        ...
        if (recipe != null) {
            if (GT_Utility.areStacksEqual(mInventory[1], GT_Items.Component_LavaFilter.getWildcard(1))
                && recipe.getFluidInputs()
                    .contains(new FluidStack(FluidRegistry.LAVA, 0))) {
    ...
    public boolean checkMachine(ItemStack aStack) {
        ...
        if (getBaseMetaTileEntity().getAirAtSideAndDistance(getBaseMetaTileEntity().getBackFacing(), 1)) {
            if (getBaseMetaTileEntity().getBlockAtSideAndDistance(getBaseMetaTileEntity().getBackFacing(), 2)
                != GregTech_API.sBlockList[0]
                || getBaseMetaTileEntity().getMetaIDAtSideAndDistance(getBaseMetaTileEntity().getBackFacing(), 2)
                    != 14) {
                TileEntity tTileEntity = getBaseMetaTileEntity()
                    .getTileEntityAtSideAndDistance(getBaseMetaTileEntity().getBackFacing(), 2);
            ...
            for (byte i = -1; i < 2; i++)
                for (byte j = -1; j < 2; j++) if (i != 0 || j != 0) for (byte k = 0; k < 3; k++) {
                    if ((i == 0 || j == 0) && (k == 1)) {
                        if (getBaseMetaTileEntity().getBlock(
                            tX + (tSide < 4 ? i : tSide == 5 ? +k : -k),
                            tY + j,
                            tZ + (tSide < 4 ? tSide == 3 ? +k : -k : i)) != GregTech_API.sBlockList[0]
                            || getBaseMetaTileEntity().getMetaID(
                                tX + (tSide < 4 ? i : tSide == 5 ? +k : -k),
                                tY + j,
                                tZ + (tSide < 4 ? tSide == 3 ? +k : -k : i)) != 14) {
                            TileEntity tTileEntity = getBaseMetaTileEntity().getTileEntity(
                                tX + (tSide < 4 ? i : tSide == 5 ? +k : -k),
                                tY + j,
                            ...
                            if (tTileEntity != null && tTileEntity instanceof IGregTechTileEntity) {
                                ...
                                if (mte != null) {
                                    if (mte instanceof GT_MetaTileEntity_Hatch_Maintenance) {
                                        maintenanceHatch = new WeakReference<>(
                                            (GT_MetaTileEntity_Hatch_Maintenance) mte);
                                    } else if (mte instanceof GT_MetaTileEntity_Hatch_Input) {
                                        mInputHatches.add((GT_MetaTileEntity_Hatch_Input) mte);
                                    } else if (mte instanceof GT_MetaTileEntity_Hatch_Output) {
                                        mOutputHatches.add((GT_MetaTileEntity_Hatch_Output) mte);
