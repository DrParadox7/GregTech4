...
public class GT_MetaTileEntity_FusionComputer extends MetaTileEntity implements IRecipeWorkable {
    ...
    public int maxEUStore() {
        return mEnergyInjectors.size() * 10000000;
    }
    ...
    public void onRightclick(EntityPlayer aPlayer) {
        getBaseMetaTileEntity().openGUI(aPlayer, 143);
    }
    ...
    private void setComputerOf(MetaTileEntity aMetaTileEntity, boolean setreset) {
        if (aMetaTileEntity != null) {
            if (aMetaTileEntity instanceof GT_MetaTileEntity_FusionInjector) {
                ((GT_MetaTileEntity_FusionInjector) aMetaTileEntity)
                    .setComputer(setreset ? getBaseMetaTileEntity() : null);
                if (setreset) mPlasmaInjectors.add((GT_MetaTileEntity_FusionInjector) aMetaTileEntity);
            }
            if (aMetaTileEntity instanceof GT_MetaTileEntity_FusionEnergyInjector) {
                ((GT_MetaTileEntity_FusionEnergyInjector) aMetaTileEntity)
                    .setComputer(setreset ? getBaseMetaTileEntity() : null);
                if (setreset) mEnergyInjectors.add(aMetaTileEntity.getBaseMetaTileEntity());
            }
            if (aMetaTileEntity instanceof GT_MetaTileEntity_FusionExtractor) {
                ((GT_MetaTileEntity_FusionExtractor) aMetaTileEntity)
                    .setComputer(setreset ? getBaseMetaTileEntity() : null);
                if (setreset) mPlasmaExtractors.add((GT_MetaTileEntity_FusionExtractor) aMetaTileEntity);
            }
        }
    }
    ...
    private void reset() {
        ...
        for (IGregTechTileEntity tTileEntity : mEnergyInjectors)
            setComputerOf((MetaTileEntity) tTileEntity.getMetaTileEntity(), false);
    ...
    public void onPostTick() {
        if (getBaseMetaTileEntity().isServerSide()) {
            if (needCheckStruct) {
                ...
                if (structComplete) {
                    fluidInputs = new InventoryHandlerList<>(
                        mPlasmaInjectors.stream()
                            .map(extr -> new ListAdapter<>(extr.mFluid))
                            .collect(Collectors.toList()));
                    fluidOutputs = new InventoryHandlerList<>(
                        mPlasmaExtractors.stream()
                            .map(extr -> new ListAdapter<>(extr.mFluid))
                            .collect(Collectors.toList()));
                    itemInputs = new InventoryHandlerList<>(
                        mPlasmaInjectors.stream()
                            .map(extr -> new ListAdapter<>(extr.mInventory, extr.getInputSlot(), extr.getInputSlot()))
                            .collect(Collectors.toList()));
                    itemOutputs = new InventoryHandlerList<>(
                        mPlasmaExtractors.stream()
                            .map(extr -> new ListAdapter<>(extr.mInventory, extr.getOutputSlot(), extr.getOutputSlot()))
                            .collect(Collectors.toList()));
    ...
    public Map<String, List<Object>> getInfoData() {
        return InfoBuilder.create()
            .newKey(
                ...
                recipeLogic.getDisplayProgress() * 100.0D / recipeLogic.getDisplayMaxProgress())
    ...
    private boolean checkMachine() {
        ...
        int xCenter = getBaseMetaTileEntity().getXCoord()
            ...
            yCenter = getBaseMetaTileEntity().getYCoord(), zCenter = getBaseMetaTileEntity().getZCoord()
                + ForgeDirection.getOrientation(getBaseMetaTileEntity().getFrontFacing()).offsetZ * 5;

        if (isAdvancedMachineCasing(xCenter + 5, yCenter, zCenter) || xCenter + 5 == getBaseMetaTileEntity()
            .getXCoord()) if (isAdvancedMachineCasing(xCenter - 5, yCenter, zCenter)
                || xCenter - 5 == getBaseMetaTileEntity().getXCoord())
                if (isAdvancedMachineCasing(xCenter, yCenter, zCenter + 5) || zCenter + 5 == getBaseMetaTileEntity()
                    .getZCoord()) if (isAdvancedMachineCasing(xCenter, yCenter, zCenter - 5)
                        || zCenter - 5 == getBaseMetaTileEntity().getZCoord())
                        if (checkCoils(xCenter, yCenter, zCenter)) if (checkHulls(xCenter, yCenter, zCenter))
                            if (checkUpperOrLowerHulls(xCenter, yCenter + 1, zCenter)) if (checkUpperOrLowerHulls(
                                xCenter,
                                yCenter - 1,
                                zCenter))
                                if (addIfEnergyInjector(xCenter + 4, yCenter, zCenter + 3)) if (addIfEnergyInjector(
                                    xCenter + 4,
                                    yCenter,
                                    zCenter - 3))
                                    if (addIfEnergyInjector(xCenter + 4, yCenter, zCenter + 5)) if (addIfEnergyInjector(
                                        xCenter + 4,
                                        yCenter,
                                        zCenter - 5))
    ...
    private boolean isAdvancedMachineCasing(int aX, int aY, int aZ) {
        return getBaseMetaTileEntity().getBlock(aX, aY, aZ) == GregTech_API.sBlockList[0]
            && getBaseMetaTileEntity().getMetaID(aX, aY, aZ) == 15;
    }

    private boolean isFusionCoil(int aX, int aY, int aZ) {
        return getBaseMetaTileEntity().getBlock(aX, aY, aZ) == GregTech_API.sBlockList[0]
            && getBaseMetaTileEntity().getMetaID(aX, aY, aZ) == 1;
    }
    ...
    private class FusionRecipeLogic extends RecipeLogic {
        ...
        public boolean update() {
            ...
            if (base.isAllowedToWork()) {
                if (progressTime == 0) {
                    if (machine.hasInventoryBeenModified() || base.hasWorkJustBeenEnabled()
                        || success
                        || base.getTimer() % 600 == 0
                        || wasNoEnergy) {
        ...
        protected boolean updateRecipeProgress() {
            if (((GT_MetaTileEntity_FusionComputer) getMachine()).decreaseStoredEU(EUt)) {
                if ((progressTime += progressTimeManipulator.applyAsInt((int) Math.pow(2, overclockersCount)))
                    >= maxProgressTime) {
                    progressTime = 0;
                    maxProgressTime = 0;
                    EUt = 0;
        ...
        protected void trySerachRecipe() {
            if (getMachine().allowToCheckRecipe()) {
                if (previousRecipe != null) {
                    ...
                    if (resRec != null) startRecipe(resRec);
                    firstStart = false;
                }
            }
        }
        ...
        protected void startRecipe(Recipe recipe) {
            if (getMachine().spaceForOutput(recipe) && firstStart
                ? ((GT_MetaTileEntity_FusionComputer) getMachine()).decreaseStoredEU(recipe.getEUtoStart())
                : true) {
                ...
                if (consumeInputs(recipe)) {
                    ...
                    GT_Log.log.catching(
                        new IllegalStateException(
                            "Error state detected! RecipeMap passed recipe, but it's not matching! Report about this!!!"));
                    EUt = 0;
                    progressTime = 0;
                    maxProgressTime = 0;
                    previousRecipe = null;
