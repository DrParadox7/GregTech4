...
public class GT_MetaTileEntity_ImplosionCompressor extends BaseMultiWorkable {
    ...
    public void onRightclick(EntityPlayer aPlayer) {
        getBaseMetaTileEntity().openGUI(aPlayer, 115);
    }
    ...
    protected boolean checkMachine() {
        for (int i = -1; i < 2; i++) for (int j = -1; j < 2; j++) for (int k = -1; k < 2; k++) {
            if (i != 0 || j != 0 || k != 0) {
                ...
                if (getBaseMetaTileEntity().getMetaIDOffset(i, j - 2, k) != (i == 0 || j == 0 || k == 0 ? 14 : 13))
                    return false;
            } else {
    ...
    public boolean allowPutStack(int aIndex, byte aSide, ItemStack aStack) {
        return GT_Utility
            .areStacksEqual(aStack, GT_ModHandler.getIC2Item("industrialTnt", 1, new ItemStack(Blocks.tnt, 1)))
                ? aIndex == 1
                : aIndex == 0;
    }
    ...
    public int getTextureIndex(byte aSide, byte aFacing, boolean aActive, boolean aRedstone) {
        if (aSide == 1) return 68 + (aActive ? 1 : 0);
        if (aSide == 0) return 71;
        if (aSide < 4) return 74;
        return 73;
    }
