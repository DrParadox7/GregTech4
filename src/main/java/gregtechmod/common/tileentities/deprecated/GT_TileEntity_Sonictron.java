...
public class GT_TileEntity_Sonictron extends GT_TileEntityMetaID_Machine {
        ...
        try {
            ...
            if (mTickTimer % 2 == 0 && mCurrentIndex > -1) {
                GT_Mod.gregtechproxy
                    .doSonictronSound(mInventory[mCurrentIndex], worldObj, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5);
    ...
    public short getSoundIndex(ItemStack aStack) {
        for (short i = 0; i < GT_Mod.mSoundItems.size(); i++)
            if (GT_Utility.areStacksEqual(GT_Mod.mSoundItems.get(i), aStack)) return i;
        ...
        try {
            if (worldObj.isRemote) {
                ...
                if (aEventID >= 20 && aEventID - 20 < getInventorySlotCount() && aValue >= 0) {
                    mInventory[aEventID - 20] = (aValue < GT_Mod.mSoundItems.size()
                        ? GT_Utility.copy(GT_Mod.mSoundItems.get(aValue))
                        : null);
                }
