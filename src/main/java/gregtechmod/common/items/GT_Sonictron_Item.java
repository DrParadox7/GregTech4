...
public class GT_Sonictron_Item extends GT_Generic_Item {
    ...
    @Override
    public boolean onItemUseFirst(ItemStack aStack, EntityPlayer par2EntityPlayer, World par3World, int par4, int par5,
        int par6, int par7, float par8, float par9, float par10) {
        if (!par3World.isRemote && par3World.getBlock(par4, par5, par6) == GregTech_API.sBlockList[1]
            && par3World.getBlockMetadata(par4, par5, par6) == 6) {
    ...
    private void copyInventory(ItemStack aInventory[], ItemStack aNewContent[]) {
        for (int i = 0; i < 64; i++) {
            ...
            else aInventory[i] = GT_Utility.copy(aNewContent[i]);
        }
    }
    ...
    public void onUpdate(ItemStack aStack, World par2World, Entity aPlayer, int aTimer, boolean aIsInHand) {
        ...
        if (tTickTimer++ % 2 == 0 && tCurrentIndex > -1) {
            ItemStack[] tInventory = getNBTInventory(aStack);
            GT_Mod.gregtechproxy.doSonictronSound(
