...
public class GT_SensorCard_Item extends GT_Generic_Item implements IRemoteSensor, IPanelDataSource {
    ...
    private ChunkCoordinates getCoordinates(ItemStack aStack) {
        if (aStack != null) {
            ...
            if (tNBT != null)
                return new ChunkCoordinates(tNBT.getInteger("x"), tNBT.getInteger("y"), tNBT.getInteger("z"));
        }
        return null;
    }
    ...
    public CardState update(World world, ICardWrapper aCard, int maxRange) {
        ...
        if (tTileEntity != null && tTileEntity instanceof IGregTechDeviceInformation
    ...
    public List<PanelString> getStringData(int displaySettings, ICardWrapper aCard, boolean showLabels) {
        ...
        try {
            JsonObject data = new JsonParser().parse(jsonData)
                .getAsJsonObject();
            ...
            for (Entry<String, JsonElement> entry : data.entrySet()) {
                ...
                for (JsonElement value : entry.getValue()
                    .getAsJsonArray()) {
                    if (value.isJsonObject()) {
                        JsonObject stackData = value.getAsJsonObject();
                        ItemStack stack = new ItemStack(
                            Item.getItemById(
                                stackData.get("itemID")
                                    .getAsInt()),
                            stackData.get("amount")
                                .getAsInt(),
                            stackData.get("damage")
                                .getAsInt());
