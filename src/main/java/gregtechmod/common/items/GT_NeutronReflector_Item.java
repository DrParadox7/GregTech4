...
public class GT_NeutronReflector_Item extends GT_Generic_Item implements IReactorComponent {
    ...
    @Override
    public boolean acceptUraniumPulse(IReactor aReactor, ItemStack aStack, ItemStack pulsingStack, int x, int y,
        int pulseX, int pulseY, boolean heatrun) {
        if (aStack.stackSize > 1) return false;
        ((IReactorComponent) pulsingStack.getItem())
            .acceptUraniumPulse(aReactor, pulsingStack, aStack, pulseX, pulseY, x, y, heatrun);
        if (getMaxDamage() > 0) if (aStack.getItemDamage() + 1 >= getMaxDamage()) aReactor.setItemAt(x, y, null);
        else aStack.setItemDamage(aStack.getItemDamage() + 1);
        return true;
    }
