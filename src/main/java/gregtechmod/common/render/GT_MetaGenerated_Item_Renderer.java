...
public class GT_MetaGenerated_Item_Renderer implements IItemRenderer {
    ...
    protected static final ResourceLocation ENCHANT_GLITH = new ResourceLocation(
    ...
    public boolean handleRenderType(ItemStack aStack, ItemRenderType aType) {
        ...
        return !GT_Utility.isStackInvalid(aStack) && aStack.getItemDamage() >= 0
            ? aType != ItemRenderType.FIRST_PERSON_MAP
            : false;
    }
    ...
    public void renderItem(ItemRenderType type, ItemStack aStack, Object... data) {
        if (!GT_Utility.isStackInvalid(aStack)) {
            ...
            GL11.glPushMatrix();
            {
                ...
                if (meta < 32000) {
                    ...
                    if (tStats == null) {
                        ...
                        if ((secondIcon = tStats.getOverlayIcon()) != null) {
                            ...
                            if (tModulation != null) {
                                fluidIcon = tModulation.getFluid()
                                    .getIcon(tModulation);
                            }
                        }
                    }
                    ...
                    if (fluidIcon == null) {
                        ...
                        GL11.glColor3f(
                            (float) tModulation1[0] / 255.0F,
                            (float) tModulation1[1] / 255.0F,
                            (float) tModulation1[2] / 255.0F);
                    }
                    ...
                    if (tStats1 != null && tStats1[3] < 0) {
                        ...
                        if (tCharge1 <= 0L) {
                            ...
                            icon = item.mIconList[meta - 32000][7
                                - (int) Math.max(0, Math.min(5, (tStats1[0] - tCharge1) * 6 / tStats1[0]))];
                        }
                    } else {
                ...
                if (aStack.hasEffect(0)) this.renderEffect(type);
            }
            GL11.glPopMatrix();
            GL11.glPopAttrib();
        }
    ...
    protected void renderGlith() {
        ...
        for (int i = 0; i < 2; i++) {
            float scale = 1 / 16f;
            float uOffset = (float) (Minecraft.getSystemTime() % (long) (3000 + i * 1873))
                / (3000.0F + (float) (i * 1873))
                * 16.0F;
    ...
    protected void renderIcon(IIcon icon, double xStart, double yStart, double xEnd, double yEnd, double z, float nx,
        float ny, float nz) {
        if (icon != null) {
            Tessellator.instance.startDrawingQuads();
            ...
            if (nz > 0.0F) {
                Tessellator.instance
                    .addVertexWithUV(xStart, yStart, z, (double) icon.getMinU(), (double) icon.getMinV());
