...
public class IC2ReactorSlotTransformer implements IClassTransformer {
    ...
    private final static String TE_REACTOR = GT_CoreMod
        .getIC2CoreClass("block.reactor.tileentity.TileEntityNuclearReactorElectric");
    ...
    public byte[] transform(String name, String transformedName, byte[] basicClass) {
        if (TE_REACTOR.equals(name)) {
            ...
            for (MethodNode method : clz.methods) {
                if (method.name.equals("isUsefulItem") && method.access == ACC_PUBLIC
                    && method.desc.equals(String.format("(L%s;Z)Z", STACK_TYPE))) { // (Ladd;Z)Z
                    GT_CoreMod.log.info("Transforming " + TE_REACTOR);
                    try {
                        for (int i = 0; i < method.instructions.size(); i++) {
                            AbstractInsnNode ins = method.instructions.get(i);
                            if (ins.getOpcode() == ALOAD && ins.getNext()
                                .getOpcode() == INVOKEVIRTUAL
                                && ((MethodInsnNode) ins.getNext()).name.equals("getClass")) {
                                ins = ins.getNext()
                                    .getNext()
                                    .getNext(); // IF_ACMPNE L5
                                ...
                                InsnList list = new InsnList();
                                {
                                    ...
                                    list.add(
                                        new MethodInsnNode(
                                            ...
                                            GT,
                                            "getHeatStored",
                                            String.format("(L%s;)I", STACK_TYPE),
                                            true));
