...
public class AnvilTransformer implements IClassTransformer {
    ...
    private static final String[] names = { "updateRepairOutput", "getItem" };

    private static final String[] namesObf = { "func_82848_d", "func_77973_b" };
        ...
        for (MethodNode meth : node.methods) {
            if (meth.name.equals(names[0]) && meth.desc.equals("()V")) { // updateRepairOutput func_82848_d
                for (int i = 0; i < meth.instructions.size(); i++) {
                    ...
                    if (ins.getOpcode() == GETFIELD && ins.getNext()
                        .getOpcode() == GETFIELD
                        && ins.getNext()
                            .getNext()
                            .getOpcode() == IFNE
                        && ((FieldInsnNode) ins).owner.equals("net/minecraft/entity/player/EntityPlayer")) {
                        ins = ins.getNext()
                            .getNext();
                        ...
                        InsnList list = new InsnList();
                        {
                            ...
                            list.add(new JumpInsnNode(IFEQ, jmpAfter)); // jump after block if item not instanceof
                                                                        // GT_Tool_Item
                            ...
                            list.add(
                                new MethodInsnNode(
                                    INVOKEVIRTUAL,
                                    "gregtechmod/api/items/GT_Tool_Item",
                                    "checkEnchant",
